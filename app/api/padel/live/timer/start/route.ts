...
async function _POST(req: NextRequest) {
  ...
  const roundNumberRaw = typeof body.roundNumber === "number" ? body.roundNumber : Number(body.roundNumber);
  ...
  const roundNumber =
    Number.isFinite(roundNumberRaw) && roundNumberRaw > 0 ? Math.floor(roundNumberRaw) : previous.roundNumber || 1;
