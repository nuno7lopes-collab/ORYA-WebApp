...
type Props = { organizationId: number | null; embedded?: boolean };
type AgreementItem = {
  id: number;
...
export default function PartnershipsPageClient({
  ...
  const { data, mutate, isLoading } = useSWR<AgreementsResponse>(
    baseUrl,
    fetcher,
    { revalidateOnFocus: false },
  );
  const { data: tournamentRequestsData, mutate: mutateTournamentRequests } =
    useSWR<TournamentRequestsResponse>(requestsUrl, fetcher, {
      revalidateOnFocus: false,
    });
  const { data: organizationsData, isLoading: organizationsLoading } =
    useSWR<OrganizationSearchResponse>(organizationsUrl, fetcher, {
      revalidateOnFocus: false,
    });
  ...
  const tournamentRequests = Array.isArray(tournamentRequestsData?.items)
    ? tournamentRequestsData.items
    : [];
  const organizationSuggestions = Array.isArray(organizationsData?.items)
  ...
  const runTournamentRequestAction = async (
    requestId: number,
    ...
    try {
      const res = await fetch(
        `/api/padel/partnerships/tournament-requests/${requestId}/${action}`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      setFeedback(
        action === "approve"
          ? "Pedido de torneio aprovado."
          : "Pedido de torneio rejeitado.",
      );
  ...
  const wrapperClass = embedded
    ...
    : "mx-auto w-full max-w-6xl space-y-5 px-4 py-6 text-white";
  const sectionClass = "rounded-2xl border border-white/12 bg-white/[0.04] p-4";
  return (
      ...
      {!embedded && (
        <header className={sectionClass}>
          ...
          <p className="text-sm text-white/70">
            Pedidos entre organizações e gestão operacional partilhada.
          </p>{" "}
        </header>
      )}{" "}
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-3">
          ...
          <label className="relative space-y-1 md:col-span-2">
            ...
            {!selectedTarget && normalizedTargetQuery.length >= 2 && (
              <div className="absolute left-0 right-0 top-[72px] z-20 rounded-xl border border-white/15 bg-[#050b16]/95 p-2">
                {" "}
                {organizationsLoading ? (
                  <p className="px-2 py-1 text-xs text-white/65">
                    A pesquisar organizações...
                  </p>
                ) : organizationSuggestions.length === 0 ? (
                  <p className="px-2 py-1 text-xs text-white/65">
                    Sem organizações elegíveis com clube ativo.
                  </p>
                ) : (
                    ...
                    {organizationSuggestions.map((item) => (
                      ...
                      >
                        ...
                        <p className="truncate text-sm text-white">
                          {formatOrganizationLabel(item)}
                        </p>{" "}
                        <p className="truncate text-[11px] text-white/60">
                          Organização elegível para parceria.
                        </p>{" "}
                      </button>
                    ))}{" "}
                  </div>
            ...
            {selectedTarget ? (
              <p className="text-[11px] text-emerald-100">
                Destino selecionado: {formatOrganizationLabel(selectedTarget)}
              </p>
            ) : null}{" "}
          </label>{" "}
            ...
            {items.map((item) => (
              ...
              >
                ...
                <p className="mt-1 text-xs text-white/65">
                  {" "}
                  {item.activeWindowsCount != null
                    ? `Janelas ativas ${item.activeWindowsCount}`
                    : "Janelas ativas 0"}{" "}
                  {item.activeGrantsCount != null
                    ? ` · Grants ativos ${item.activeGrantsCount}`
                    : ""}{" "}
                </p>{" "}
        ...
        {tournamentRequests.length === 0 ? (
          ...
          <div className="space-y-3">
            ...
            {tournamentRequests.map((requestItem) => {
              const isOwnerSide =
                organizationId === requestItem.ownerOrganizationId;
              const canReview = isOwnerSide && requestItem.status === "PENDING";
              return (
                <article
                  key={`tournament-request-${requestItem.id}`}
                  className="rounded-xl border border-white/10 bg-black/30 p-3"
                >
                  {" "}
                  <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <p className="mt-1 text-xs text-white/70">
                    {" "}
                    {requestItem.ownerOrganizationName ||
                      "Organização origem"}{" "}
                    →{""}{" "}
                    {requestItem.partnerOrganizationName ||
                      "Organização destino"}{" "}
                  </p>{" "}
                  <p className="mt-1 text-xs text-white/65">
                    {" "}
                  ...
                  {canReview && (
                    <div className="mt-3 flex flex-wrap gap-2">
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          runTournamentRequestAction(requestItem.id, "approve")
                        }
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          runTournamentRequestAction(requestItem.id, "reject")
                        }
                        disabled={requestBusyId === requestItem.id}
