...
import { PadelFormat } from "@prisma/client";
...
const PADEL_FORMAT_ALIASES: Record<string, PadelFormat> = {
  KO: "QUADRO_ELIMINATORIO",
  GROUPS_KO: "GRUPOS_ELIMINATORIAS",
  LEAGUE: "TODOS_CONTRA_TODOS",
};

const normalizePadelFormat = (value: unknown): PadelFormat | null => {
  if (typeof value !== "string") return null;
  const normalized = value.toUpperCase();
  if (PADEL_FORMAT_ALIASES[normalized]) return PADEL_FORMAT_ALIASES[normalized];
  if (Object.values(PadelFormat).includes(normalized as PadelFormat)) {
    return normalized as PadelFormat;
  }
  return null;
};
...
export async function POST(req: NextRequest) {
  try {
    ...
    const orgCategoryRaw = (organizer as { organizationCategory?: string | null }).organizationCategory ?? null;
    const orgCategory = orgCategoryRaw?.toUpperCase() ?? "";
    ...
    const resaleModeRaw = body.resaleMode?.toUpperCase() as
      ...
      | undefined;
    const payoutMode =
      body.payoutMode?.toUpperCase() === "PLATFORM" ? "PLATFORM" : "ORGANIZER";
    ...
    if (isPadelOrg) {
      const isPadelRequest = templateTypeFromBody === "PADEL" || padelRequested || categories.includes("PADEL");
      if (!isPadelRequest) {
        return NextResponse.json(
          { ok: false, error: "A tua organização só pode criar torneios de padel." },
          { status: 400 },
        );
      }
    }
    ...
    if (templateType === "PADEL" && padelConfigInput && organizer) {
      ...
      const normalizedFormat = normalizePadelFormat(padelConfigInput.format) ?? "TODOS_CONTRA_TODOS";
      try {
