...
type ReportsResponse =
  | {
      ...
      frontDesk: { queue: FrontDeskContact[] };
      retention: {
        cohortMonths: number;
...
export default function CrmRelatoriosPage() {
  ...
  const setFilterParam = (
    key: "windowDays" | "cohortMonths",
    value: number,
  ) => {
    setFilterParams({ [key]: String(value) });
  };
  const setSortParam = (
    key: "segmentSort" | "frontDeskSort",
    value: string,
  ) => {
  ...
  useEffect(() => {
    ...
    const normalizedFrontDeskSort = normalizeFrontDeskSort(
      currentFrontDeskSortRaw,
    );
    const normalizedSections = serializeSections(
      parseSections(currentSectionsRaw),
    );
    ...
    const hasCanonicalSegmentSort =
      currentSegmentSortRaw === normalizedSegmentSort;
    const hasCanonicalFrontDeskSort =
      currentFrontDeskSortRaw === normalizedFrontDeskSort;
  ...
  const reportsUrl = useMemo(() => {
    ...
    return resolveCanonicalOrgApiPath(
      `/api/org/[orgId]/crm/relatorios?${params.toString()}`,
    );
  ...
  const topCampaignWinners = useMemo(
    () =>
      [...abTesting.campaignWinners]
        .sort((a, b) => b.winnerSent - a.winnerSent)
        .slice(0, 6),
    [abTesting.campaignWinners],
  );
    ...
    <div className="space-y-6">
      ...
      <header className="space-y-2">
        ...
        <p className={DASHBOARD_MUTED}>
          Dashboard avançado de CRM padel-first para operação diária.
        </p>{" "}
      </header>{" "}
      {data?.ok === false ? (
      ...
      <section className={cn(DASHBOARD_CARD, "p-4")}>
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-2">
          ...
          <label className="text-[12px] text-white/65">
            ...
            <select
              ...
              onChange={(event) =>
                setFilterParam("windowDays", Number(event.target.value))
              }
            >
              {" "}
          ...
          <label className="text-[12px] text-white/65">
            ...
            <select
              ...
              onChange={(event) =>
                setFilterParam("cohortMonths", Number(event.target.value))
              }
            >
              {" "}
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-2">
          ...
          <label className="text-[12px] text-white/65">
            ...
            <select
              ...
              value={segmentSort}
              onChange={(event) =>
                setSortParam("segmentSort", event.target.value)
              }
            >
              {" "}
          ...
          <label className="text-[12px] text-white/65">
            ...
            <select
              ...
              value={frontDeskSort}
              onChange={(event) =>
                setSortParam("frontDeskSort", event.target.value)
              }
            >
              {" "}
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2 lg:grid-cols-4">
          ...
          {[
            ...
            {
              label: `Interações (${windowDays}d)`,
              value: totals.interactions,
            },
            {
              label: `Receita CRM (${windowDays}d)`,
              value: formatCurrency(totals.amountCents, "EUR"),
            },
          ].map((item) => (
            <div
              key={item.label}
        ...
        <p className="mt-3 text-[11px] text-white/55">
          Campanhas enviadas: {campaignsSent}
        </p>{" "}
      </section>{" "}
        ...
        <section className="space-y-3">
          ...
          {!isLoading && !hasData ? (
            <div
              className={cn(
                DASHBOARD_CARD,
                "p-6 text-center text-[12px] text-white/60",
              )}
            >
              {" "}
            ...
            <div className="grid gap-3">
              ...
              {sortedCategories.map((category) => {
                ...
                return (
                  <div
                    key={category.id}
                    className={cn(DASHBOARD_CARD, "space-y-3 p-4")}
                  >
                    {" "}
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      {" "}
            ...
            <div className="space-y-2">
              ...
              {topCampaignVariants.map((variant) => (
                ...
                >
                  ...
                  <p>
                    {" "}
                    Sent {variant.sent} · Open{" "}
                    {(variant.openRate * 100).toFixed(1)}% · CTR{""}{" "}
              ...
              {topCampaignWinners.map((winner) => (
                ...
                >
                  ...
                  <p>
                    ...
                    {winner.upliftCtr !== null
                      ? ` · uplift ${formatRate(winner.upliftCtr)}`
                      : ""}{" "}
                  </p>{" "}
                </div>
              ))}{" "}
              {topCampaignWinners.length === 0 ? (
                <p className="text-[12px] text-white/55">
                  Sem winner definido.
                </p>
              ) : null}{" "}
            ...
            <div className="space-y-2">
              ...
              {topJourneyVariants.map((variant) => (
                ...
                >
                  ...
                  <p className="font-semibold text-white">
                    {" "}
                    {variant.journeyName} · {variant.stepKey} ·{" "}
                    {variant.variantId}{" "}
                  </p>{" "}
                  <p>
                    {" "}
                    OK {variant.completed} · Skip {variant.skipped} · Failed{" "}
                    {variant.failed}{" "}
                  </p>{" "}
                </div>
              ))}{" "}
              {topJourneyVariants.length === 0 ? (
                <p className="text-[12px] text-white/55">
                  Sem execuções A/B de journeys no período.
                </p>
              ) : null}{" "}
            </div>{" "}
            <div className="space-y-2 rounded-xl border border-white/10 bg-white/5 p-3">
              ...
              {topJourneyWinners.map((winner) => (
                <div
                  key={`${winner.journeyId}:${winner.stepKey}`}
                  className="text-[12px] text-white/75"
                >
                  {" "}
                  <p className="font-semibold text-white">
                    {" "}
                    {winner.journeyName} · {winner.stepKey} ·{" "}
      ...
      {isSectionVisible("retention") ? (
        <section className="grid gap-4 xl:grid-cols-2">
          ...
          <div className={cn(DASHBOARD_CARD, "space-y-3 p-4")}>
            {" "}
            <h2 className="text-sm font-semibold text-white">
              Coortes de retenção padel
            </h2>{" "}
            <p className="text-[12px] text-white/55">
              Últimos {retention.cohortMonths} meses de coorte.
            </p>{" "}
            <div className="grid gap-2 sm:grid-cols-3">
              {" "}
              <div className="rounded-xl border border-white/10 bg-white/5 px-3 py-2">
                ...
                <p className="text-lg font-semibold text-white">
                  {formatRate(retention.summary.avgRate30)}
                </p>{" "}
              </div>{" "}
              <div className="rounded-xl border border-white/10 bg-white/5 px-3 py-2">
                {" "}
                ...
                <p className="text-lg font-semibold text-white">
                  {formatRate(retention.summary.avgRate60)}
                </p>{" "}
              </div>{" "}
              <div className="rounded-xl border border-white/10 bg-white/5 px-3 py-2">
                {" "}
                ...
                <p className="text-lg font-semibold text-white">
                  {formatRate(retention.summary.avgRate90)}
                </p>{" "}
              </div>{" "}
            </div>{" "}
            <div className="space-y-2">
              ...
              {retention.cohorts.map((cohort) => (
                ...
                >
                  ...
                  <p>
                    {" "}
                    R30 {formatRate(cohort.rate30)} · R60{" "}
                    {formatRate(cohort.rate60)} · R90{" "}
                    {formatRate(cohort.rate90)}{" "}
                  </p>{" "}
                </div>
              ))}{" "}
              {retention.cohorts.length === 0 ? (
                <p className="text-[12px] text-white/55">
                  Sem dados suficientes para coortes.
                </p>
              ) : null}{" "}
            </div>{" "}
          </div>{" "}
          <div className={cn(DASHBOARD_CARD, "space-y-3 p-4")}>
            ...
            <div className="grid gap-2 sm:grid-cols-3">
              ...
              <div className="rounded-xl border border-white/10 bg-white/5 px-3 py-2">
                ...
                <p className="text-lg font-semibold text-white">
                  {segmentPerformance.summary.totalSegments}
                </p>{" "}
              </div>{" "}
              <div className="rounded-xl border border-white/10 bg-white/5 px-3 py-2">
                {" "}
                ...
                <p className="text-lg font-semibold text-white">
                  {segmentPerformance.summary.withCampaigns}
                </p>{" "}
              </div>{" "}
              <div className="rounded-xl border border-white/10 bg-white/5 px-3 py-2">
                {" "}
                ...
                <p className="text-lg font-semibold text-white">
                  {segmentPerformance.summary.sent}
                </p>{" "}
              </div>{" "}
            </div>{" "}
            <div className="space-y-2">
              ...
              {sortedSegmentRows.slice(0, 8).map((segment) => (
                ...
                >
                  ...
                  <p>
                    {" "}
                    Score {segment.performanceScore} · Sent {segment.sent} ·
                    Open{""} {(segment.openRate * 100).toFixed(1)}% · CTR{" "}
                    {(segment.ctr * 100).toFixed(1)}%{" "}
                  </p>{" "}
                </div>
              ))}{" "}
              {sortedSegmentRows.length === 0 ? (
                <p className="text-[12px] text-white/55">
                  Sem desempenho de segmentos no período.
                </p>
              ) : null}{" "}
            </div>{" "}
          </div>{" "}
        ...
        <section className={cn(DASHBOARD_CARD, "space-y-3 p-4")}>
          ...
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <span className="text-[11px] text-white/50">
              {" "}
              {loyalty.enabled
                ? `${loyalty.programName ?? "Programa"} · ${loyalty.programStatus}`
                : "Programa inativo"}{" "}
            </span>{" "}
          </div>{" "}
          ...
          <div className="space-y-2">
            ...
            {loyalty.topMembers.map((member) => (
              ...
              >
                ...
                <p>
                  {" "}
                  Saldo {member.netPoints} · Earn {member.earnedPoints} · Spend{" "}
                  {member.spentPoints}{" "}
                </p>{" "}
              </div>
            ))}{" "}
            {loyalty.topMembers.length === 0 ? (
              <p className="text-[12px] text-white/55">
                Sem movimento de loyalty no período.
              </p>
            ) : null}{" "}
          </div>{" "}
        </section>
      ...
      {isSectionVisible("frontDesk") ? (
        <section className={cn(DASHBOARD_CARD, "space-y-3 p-4")}>
          ...
          <div className="flex items-center justify-between">
            {" "}
            <h2 className="text-sm font-semibold text-white">
              Fila receção por risco/valor
            </h2>{" "}
            <span className="text-[11px] text-white/50">
              {sortedFrontDeskQueue.length} contactos
            </span>{" "}
          </div>{" "}
          <div className="grid gap-2">
                ...
                <div className="flex flex-wrap items-start justify-between gap-2 text-[12px]">
                  ...
                  <div>
                    ...
                    <p className="text-white/55">
                      Prioridade {row.priorityScore} · Valor{" "}
                      {row.estimatedValueBand}
                    </p>{" "}
                  </div>{" "}
                  <div className="text-right text-white/60">
                    {" "}
                    <p>Churn {row.churnRiskScore}</p> <p>RFM {row.rfmScore}</p>{" "}
                    <p>Jogos 30d {row.matches30d}</p>{" "}
                  </div>{" "}
                </div>{" "}
                <p className="mt-2 text-[12px] text-[#BDF6FF]">
            ...
            {sortedFrontDeskQueue.length === 0 ? (
              <p className="text-[12px] text-white/55">
                Sem contactos priorizados no período.
              </p>
            ) : null}{" "}
          </div>{" "}
        </section>
