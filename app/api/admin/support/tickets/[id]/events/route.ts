...
import { addSupportTicketEvent, supportTicketEventCreateSchema } from "@/lib/domain/supportTickets";
...
async function _POST(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const parsed = supportTicketEventCreateSchema.safeParse(body ?? {});
    if (!parsed.success) {
      return fail(ctx, 422, parsed.error.issues[0]?.message ?? "VALIDATION_FAILED", "VALIDATION_FAILED");
    }

    const event = await addSupportTicketEvent({
      id,
    ...
    await auditAdminAction({
      action: "SUPPORT_TICKET_EVENT_ADD",
    ...
    console.error("[admin/support/tickets/:id/events][POST]", err);
