export type InventoryHoldSubjectType =
  | "TICKET_TYPE"
  | "STORE_PRODUCT"
  | "STORE_VARIANT";

export type InventoryHoldFingerprintInput = {
  orgId: number | string;
...
export function buildInventoryHoldFingerprintSeed(
  input: InventoryHoldFingerprintInput,
): string {
  const orgId = normalizePositiveInt(input.orgId);
  const subjectType = String(input.subjectType || "").trim().toUpperCase();
  const storeId = normalizePositiveInt(input.storeId);
  const eventId = normalizePositiveInt(input.eventId);
  const productId = normalizePositiveInt(input.productId);
  const variantId = normalizePositiveInt(input.variantId);
  const ticketTypeId = normalizePositiveInt(input.ticketTypeId);
...
export function buildInventoryHoldFingerprint(
  input: InventoryHoldFingerprintInput,
): string {
  return sha256Hex(buildInventoryHoldFingerprintSeed(input));
}
