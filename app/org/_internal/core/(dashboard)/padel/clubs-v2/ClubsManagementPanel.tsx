    ...
    <div className="space-y-4 transition-all duration-250 ease-out opacity-100 translate-y-0">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <p className="text-[12px] text-white/65">Criação de clubes e gestão de campos no mesmo fluxo.</p>
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {props.visibleClubs.map((club) => (
            <div
              ...
              } ${props.drawerClubId === club.id ? "ring-2 ring-cyan-400/40" : ""}`}
              role="button"
              tabIndex={0}
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="space-y-1">
                  ...
                  <p className="text-[12px] text-white/65">{props.compactAddress(club)}</p>
                  <p className="text-[12px] text-white/55">Campos ativos: {props.activeCourtsForClub(club)}</p>
                </div>
        ...
        <div className="space-y-4 rounded-2xl border border-white/12 bg-gradient-to-br from-white/6 via-[#0c1628]/65 to-[#050912]/85 p-4 shadow-[0_18px_60px_rgba(0,0,0,0.5)]">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2">
              <span className={props.badgeClass("slate")}>{props.selectedClub.name}</span>
              <button
                type="button"
          ...
          {props.loadingDrawer ? (
            <div className="space-y-3">
              ...
              <div className={`grid gap-3 ${props.showClubStaffPanel ? "lg:grid-cols-2" : ""}`}>
          ...
          <div className={`grid gap-4 ${props.showClubStaffPanel ? "lg:grid-cols-2" : ""}`}>
            <div className="space-y-3 rounded-xl border border-white/12 bg-gradient-to-br from-white/6 via-[#0c1628]/60 to-[#050912]/85 p-3 shadow-[0_14px_45px_rgba(0,0,0,0.45)]">
              <div className="flex items-center justify-between">
                ...
                <span className={props.badgeClass("slate")}>{props.courts.filter((court) => court.isActive).length} ativos</span>
              </div>
                  ...
                  <div className="inline-flex rounded-full border border-white/15 bg-black/40 p-1 text-[12px]">
                    {[
                      ...
                      <button
                        key={String(opt.key)}
                        type="button"
                        onClick={() => props.onCourtFormPatch({ indoor: opt.key })}
                        className={`rounded-full px-3 py-1 transition ${
                          props.courtForm.indoor === opt.key
                  ...
                  <div className="inline-flex rounded-full border border-white/15 bg-black/40 p-1 text-[12px]">
                    {[
                      ...
                      <button
                        key={String(opt.key)}
                        type="button"
                        onClick={() => props.onCourtFormPatch({ isActive: opt.key })}
                        className={`rounded-full px-3 py-1 transition ${
                          props.courtForm.isActive === opt.key
                            ? opt.key
              ...
              <div className="flex flex-wrap gap-2">
                ...
                >
                  {props.savingCourt ? "A guardar…" : props.courtForm.id ? "Atualizar campo" : "Guardar campo"}
                </button>
                {props.courtForm.id ? (
                  <button
                    type="button"
              ...
              {props.courtError || props.courtMessage ? (
                <span className="text-[12px] text-white/70">{props.courtError || props.courtMessage}</span>
              ) : null}

              <div className="space-y-2 rounded-lg border border-white/10 bg-white/5 p-2 text-[12px] text-white/80">
                {props.courts.length === 0 ? <p className="text-white/60">Sem campos ainda.</p> : null}
                  ...
                  <div
                    ...
                    className={`flex items-center justify-between gap-3 rounded-md px-3 py-2 transition ${
                      court.isActive
                        ...
                        : "border border-red-500/40 bg-red-500/8"
                    } ${props.draggingCourtId === court.id ? "opacity-60" : "opacity-100"}`}
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className={`text-[11px] ${court.isActive ? "text-emerald-100/80" : "text-red-100/80"}`}>
                          {court.indoor ? "Indoor" : "Outdoor"} · Ordem {court.displayOrder} · {court.isActive ? "Ativo" : "Inativo"}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
            ...
            {props.showClubStaffPanel ? (
              <div className="space-y-3 rounded-xl border border-white/12 bg-gradient-to-br from-white/6 via-[#0c1628]/60 to-[#050912]/85 p-3 shadow-[0_14px_45px_rgba(0,0,0,0.45)]">
                <div className="flex items-center justify-between">
                  <div className="space-y-1">
                    ...
                    <p className="text-[11px] text-white/60">
                      {props.staff.length} membros · {props.inheritedStaffCount} herdam para torneios
                    </p>
                  </div>
                  <span className={props.badgeClass("slate")}>Herdam: {props.inheritedStaffCount}</span>
                </div>
                      ...
                      >
                        ...
                        {props.staffOptions.map((member) => (
                          <option key={member.userId} value={member.userId}>
                            {(member.fullName || member.username || member.email || "Membro").trim()}
                  ...
                  <div className="inline-flex rounded-full border border-white/15 bg-black/40 p-1 text-[12px]">
                    {[
                      ...
                      <button
                        key={String(opt.key)}
                        type="button"
                        onClick={() => props.onStaffFormPatch({ inheritToEvents: opt.key })}
                        disabled={props.courtsPanelReadOnly}
                        className={`rounded-full px-3 py-1 transition ${
                          props.staffForm.inheritToEvents === opt.key
                            ? "bg-white text-black font-semibold shadow"
                            : "text-white/75 hover:bg-white/5"
                        } disabled:cursor-not-allowed disabled:opacity-60`}
                      >
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {props.staffError || props.staffMessage || props.staffInviteNotice ? (
                ...
                <div className="space-y-2 rounded-lg border border-white/12 bg-white/5 p-2 text-[12px] text-white/80">
                  {props.staff.length === 0 ? <p className="text-white/60">Sem staff.</p> : null}
                  {props.staff.map((member) => (
                    ...
                    >
                      <div className="space-y-0.5">
                        <p className="text-sm text-white">{member.user?.fullName || member.user?.username || member.userId}</p>
                        <div className="flex flex-wrap items-center gap-2 text-[11px] text-white/60">
                          <span className="rounded-full border border-white/20 bg-white/5 px-2 py-[2px]">{member.role}</span>
                          <span
                          ...
                          >
                            {member.inheritToEvents ? "Herdado" : "Só clube"}
                          </span>
