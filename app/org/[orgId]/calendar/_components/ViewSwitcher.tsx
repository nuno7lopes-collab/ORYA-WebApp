...
export function ViewSwitcher({ value, onChange, className }: ViewSwitcherProps) {
  return (
    <div className={cn("inline-flex items-center gap-2", className)}>
      <div className="hidden sm:inline-flex items-center rounded-full border border-white/20 bg-black/35 p-1">
        {OPTIONS.map((option) => (
          <button
      ...
      <label className="sm:hidden inline-flex items-center gap-2 rounded-full border border-white/20 bg-black/35 px-3 py-1.5 text-xs text-white/85">
