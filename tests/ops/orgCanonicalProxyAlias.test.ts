...
describe("proxy org canonical hard-cut", () => {
  it("returns 404 for legacy /organizacao/*", async () => {
  ...
  it("returns 404 for legacy /api/organizacao/*", async () => {
    const req = new NextRequest("http://localhost/api/organizacao/events/list?organizationId=42");
    const res = await proxy(req);
    expect(res.status).toBe(404);
  });

  it("returns 404 for removed /api/org/:orgId/payouts/* routes", async () => {
    const req = new NextRequest("http://localhost/api/org/42/payouts/status");
    const res = await proxy(req);
    expect(res.status).toBe(404);
  });

  it("returns 404 for removed PT legacy slugs under /org/:orgId", async () => {
    const legacyPaths = [
      "/org/42/financas",
      "/org/42/checkin",
      "/org/42/loja",
  ...
  it("returns 404 for removed bookings legacy route", async () => {
    const req = new NextRequest("http://localhost/org/42/bookings/services");
    const res = await proxy(req);
    expect(res.status).toBe(404);
  });

  it("returns 404 for removed legacy bookings query routes", async () => {
  ...
  it("returns 404 for removed finance/analytics legacy sub-routes", async () => {
    const req = new NextRequest("http://localhost/org/42/finance/subscriptions");
    const res = await proxy(req);
    expect(res.status).toBe(404);
  });

  it("returns 404 for removed finance/analytics legacy query keys", async () => {
    const req = new NextRequest("http://localhost/org/42/analytics?tab=analyze&section=vendas&analytics=conversion");
    const res = await proxy(req);
    expect(res.status).toBe(404);
  });

  it("returns 404 for shorthand /org/overview using organizationId context", async () => {
    const req = new NextRequest("http://localhost/org/overview?organizationId=42&section=ferramentas");
    const res = await proxy(req);
    expect(res.status).toBe(404);
  });

  it("returns 404 for shorthand /org/overview using organization cookie fallback", async () => {
  ...
  it("returns 404 for shorthand /org/treinadores", async () => {
