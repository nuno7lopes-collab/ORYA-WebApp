...
export default function PadelTournamentLifecyclePanel({
  eventId,
}: {
  eventId: number;
}) {
  ...
  const currentStatus = (lifecycle?.lifecycleStatus ??
    null) as PadelTournamentLifecycleStatus | null;
    ...
    () =>
      TOURNAMENT_LIFECYCLE_ORDER.map((status) => {
        ...
        const currentIndex = currentStatus
          ? TOURNAMENT_LIFECYCLE_ORDER.indexOf(currentStatus)
          : -1;
        const done =
          currentIndex >= 0 &&
          TOURNAMENT_LIFECYCLE_ORDER.indexOf(status) < currentIndex;
  ...
  return (
    <div className="rounded-2xl border border-white/12 bg-white/5 p-4">
      ...
      <div className="flex flex-wrap items-start justify-between gap-2">
        ...
        <div>
          ...
          <p className="text-[11px] uppercase tracking-[0.2em] text-white/60">
            Lifecycle do torneio
          </p>{" "}
          <p className="text-sm text-white/70">
            Define fases e bloqueios do torneio.
          </p>{" "}
        </div>{" "}
        <span className="rounded-full border border-white/20 bg-white/10 px-3 py-1 text-[11px] text-white/80">
          {" "}
          {currentStatus
            ? (TOURNAMENT_LIFECYCLE_LABELS[currentStatus] ?? currentStatus)
            : "—"}{" "}
        </span>{" "}
      </div>{" "}
      ...
      <div className="mt-4 grid gap-2 text-[11px] text-white/70 sm:grid-cols-2">
        ...
        <div>
          Atualizado: {formatLifecycleDate(lifecycle?.lifecycleUpdatedAt)}
        </div>{" "}
      </div>{" "}
      {error && (
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          {transitions.map((status) => (
            ...
            >
              ...
              {saving === status
                ? "A atualizar…"
                : `Mover para ${TOURNAMENT_LIFECYCLE_LABELS[status] ?? status}`}{" "}
            </button>
          ))}{" "}
        </div>
      )}{" "}
