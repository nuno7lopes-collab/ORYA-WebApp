...
export default function EventPageClient({
  ...
  const isPadel = checkoutUiVariant === "PADEL";
  const ticketWordTitle = isPadel ? "Inscrição" : "Bilhete";
  const ticketInvalidSuffix = isPadel ? "a" : "o";
  ...
  useEffect(() => {
    ...
    const handleInvite = async () => {
      try {
        ...
        if (!ticketId) {
          throw new Error(`${ticketWordTitle} inválid${ticketInvalidSuffix} para este convite.`);
        }
        ...
        const ticketName =
          ...
          (fallbackTicket?.name || (isPadel ? "Inscrição Padel" : "Bilhete Padel"));
