import { NextRequest } from "next/server";
import { createSupabaseServer } from "@/lib/supabaseServer";
import { prisma } from "@/lib/prisma";
import { resolveGroupMemberForOrg } from "@/lib/organizationGroupAccess";
import { ensureOrganizationEmailVerified } from "@/lib/organizationWriteAccess";
import { recordOrganizationAuditSafe } from "@/lib/organizationAudit";
import {
  getOrganizationSuspensionSnapshot,
  normalizeOrganizationDangerReasonCode,
  ORGANIZATION_SUSPENSION_WINDOW_DAYS,
} from "@/lib/organizationSuspension";
import { requireOrganizationStepUp } from "@/lib/organizationStepUp";
import { getRequestContext } from "@/lib/http/requestContext";
import { respondError, respondOk } from "@/lib/http/envelope";
import { withApiEnvelope } from "@/lib/http/withApiEnvelope";

import { getUserWithPolicy } from "@/lib/auth/getUserWithPolicy";
function errorCodeForStatus(status: number) {
  if (status === 401) return "UNAUTHENTICATED";
  if (status === 403) return "FORBIDDEN";
  if (status === 404) return "NOT_FOUND";
  if (status === 409) return "CONFLICT";
  if (status === 410) return "GONE";
  if (status === 413) return "PAYLOAD_TOO_LARGE";
  if (status === 422) return "VALIDATION_FAILED";
  if (status === 400) return "BAD_REQUEST";
  return "INTERNAL_ERROR";
}

async function _POST(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  const ctx = getRequestContext(req);
  const fail = (
    status: number,
    message: string,
    errorCode = errorCodeForStatus(status),
    retryable = status >= 500,
  ) => {
    const resolvedMessage = typeof message === "string" ? message : String(message);
    const resolvedCode = /^[A-Z0-9_]+$/.test(resolvedMessage) ? resolvedMessage : errorCode;
    return respondError(ctx, { errorCode: resolvedCode, message: resolvedMessage, retryable }, { status });
  };

  try {
    const supabase = await createSupabaseServer();
    const {
      data: { user },
      error,
    } = await getUserWithPolicy("required_verified", { supabaseOverride: supabase });

    if (error || !user) {
      return fail(401, "UNAUTHENTICATED");
    }

    const { id } = await context.params;
    const organizationId = Number(id);
    if (!organizationId || Number.isNaN(organizationId)) {
      return fail(400, "INVALID_ORGANIZATION_ID");
    }

    const membership = await resolveGroupMemberForOrg({ organizationId, userId: user.id });
    if (!membership || membership.role !== "OWNER") {
      return fail(403, "ONLY_OWNER_CAN_SUSPEND");
    }

    const payload = (await req.json().catch(() => null)) as
      | {
          reasonCode?: unknown;
          stepUpChallengeId?: unknown;
          stepUpCode?: unknown;
        }
      | null;
    const reasonCode = normalizeOrganizationDangerReasonCode(payload?.reasonCode, "OWNER_REQUEST");

    const organization = await prisma.organization.findUnique({
      where: { id: organizationId },
      select: {
        id: true,
        status: true,
        updatedAt: true,
        officialEmail: true,
        officialEmailVerifiedAt: true,
      },
    });
    if (!organization) {
      return fail(404, "ORGANIZATION_NOT_FOUND");
    }

    const emailGate = ensureOrganizationEmailVerified(organization, {
      reasonCode: "ORG_SUSPEND",
      organizationId,
    });
    if (!emailGate.ok) {
      const message =
        "message" in emailGate && typeof emailGate.message === "string"
          ? emailGate.message
          : emailGate.errorCode ?? "Sem permissões.";
      return respondError(
        ctx,
        { errorCode: emailGate.errorCode ?? "FORBIDDEN", message, retryable: false, details: emailGate },
        { status: 403 },
      );
    }

    const stepUp = await requireOrganizationStepUp({
      organizationId,
      userId: user.id,
      userEmail: user.email ?? null,
      action: "ORG_SUSPEND",
      challengeId: payload?.stepUpChallengeId,
      code: payload?.stepUpCode,
    });
    if (!stepUp.ok) {
      return respondError(
        ctx,
        {
          errorCode: stepUp.errorCode,
          message: stepUp.message,
          retryable: false,
          details: stepUp.details,
        },
        { status: stepUp.status },
      );
    }

    let effectiveStatus = organization.status;
    let effectiveUpdatedAt = organization.updatedAt ?? null;
    if (organization.status !== "SUSPENDED") {
      const updated = await prisma.organization.update({
        where: { id: organizationId },
        data: { status: "SUSPENDED" },
        select: { status: true, updatedAt: true },
      });
      effectiveStatus = updated.status;
      effectiveUpdatedAt = updated.updatedAt;
      await recordOrganizationAuditSafe({
        organizationId,
        actorUserId: user.id,
        action: "ORGANIZATION_SUSPENDED",
        metadata: {
          source: "settings_danger_zone",
          reasonCode,
        },
      });
    }
    const suspension = await getOrganizationSuspensionSnapshot({
      organizationId,
      status: effectiveStatus,
      updatedAt: effectiveUpdatedAt,
    });

    return respondOk(
      ctx,
      {
        organizationId,
        status: "SUSPENDED",
        graceWindowDays: ORGANIZATION_SUSPENSION_WINDOW_DAYS,
        suspension,
      },
      { status: 200 },
    );
  } catch (err) {
    console.error("[organization/suspend]", err);
    return fail(500, "INTERNAL_ERROR");
  }
}

async function _DELETE(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  const ctx = getRequestContext(req);
  const fail = (
    status: number,
    message: string,
    errorCode = errorCodeForStatus(status),
    retryable = status >= 500,
    details?: Record<string, unknown>,
  ) => {
    const resolvedMessage = typeof message === "string" ? message : String(message);
    const resolvedCode = /^[A-Z0-9_]+$/.test(resolvedMessage) ? resolvedMessage : errorCode;
    return respondError(ctx, { errorCode: resolvedCode, message: resolvedMessage, retryable, details }, { status });
  };

  try {
    const supabase = await createSupabaseServer();
    const {
      data: { user },
      error,
    } = await getUserWithPolicy("required_verified", { supabaseOverride: supabase });

    if (error || !user) {
      return fail(401, "UNAUTHENTICATED");
    }

    const { id } = await context.params;
    const organizationId = Number(id);
    if (!organizationId || Number.isNaN(organizationId)) {
      return fail(400, "INVALID_ORGANIZATION_ID");
    }

    const membership = await resolveGroupMemberForOrg({ organizationId, userId: user.id });
    if (!membership || membership.role !== "OWNER") {
      return fail(403, "ONLY_OWNER_CAN_REACTIVATE");
    }

    const payload = (await req.json().catch(() => null)) as
      | {
          reasonCode?: unknown;
          stepUpChallengeId?: unknown;
          stepUpCode?: unknown;
        }
      | null;
    const reasonCode = normalizeOrganizationDangerReasonCode(payload?.reasonCode, "OWNER_REQUEST");

    const organization = await prisma.organization.findUnique({
      where: { id: organizationId },
      select: {
        id: true,
        status: true,
        updatedAt: true,
      },
    });
    if (!organization) {
      return fail(404, "ORGANIZATION_NOT_FOUND");
    }
    if (organization.status !== "SUSPENDED") {
      return fail(409, "ORGANIZATION_NOT_SUSPENDED");
    }

    const suspension = await getOrganizationSuspensionSnapshot({
      organizationId,
      status: organization.status,
      updatedAt: organization.updatedAt ?? null,
    });
    if (!suspension.reactivationWindowOpen) {
      return fail(
        409,
        "REACTIVATION_WINDOW_EXPIRED",
        "CONFLICT",
        false,
        {
          suspension,
          graceWindowDays: ORGANIZATION_SUSPENSION_WINDOW_DAYS,
        },
      );
    }

    const stepUp = await requireOrganizationStepUp({
      organizationId,
      userId: user.id,
      userEmail: user.email ?? null,
      action: "ORG_REACTIVATE",
      challengeId: payload?.stepUpChallengeId,
      code: payload?.stepUpCode,
    });
    if (!stepUp.ok) {
      return respondError(
        ctx,
        {
          errorCode: stepUp.errorCode,
          message: stepUp.message,
          retryable: false,
          details: stepUp.details,
        },
        { status: stepUp.status },
      );
    }

    const updated = await prisma.organization.update({
      where: { id: organizationId },
      data: { status: "ACTIVE" },
      select: { status: true, updatedAt: true },
    });

    await recordOrganizationAuditSafe({
      organizationId,
      actorUserId: user.id,
      action: "ORGANIZATION_REACTIVATED",
      metadata: {
        source: "suspended_dashboard",
        reasonCode,
        suspendedAt: suspension.suspendedAt?.toISOString() ?? null,
        reactivationWindowDays: ORGANIZATION_SUSPENSION_WINDOW_DAYS,
      },
    });

    return respondOk(
      ctx,
      {
        organizationId,
        status: updated.status,
        suspension: await getOrganizationSuspensionSnapshot({
          organizationId,
          status: updated.status,
          updatedAt: updated.updatedAt ?? null,
        }),
      },
      { status: 200 },
    );
  } catch (err) {
    console.error("[organization/reactivate]", err);
    return fail(500, "INTERNAL_ERROR");
  }
}

async function _GET(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  const ctx = getRequestContext(req);
  const fail = (
    status: number,
    message: string,
    errorCode = errorCodeForStatus(status),
    retryable = status >= 500,
  ) => {
    const resolvedMessage = typeof message === "string" ? message : String(message);
    const resolvedCode = /^[A-Z0-9_]+$/.test(resolvedMessage) ? resolvedMessage : errorCode;
    return respondError(ctx, { errorCode: resolvedCode, message: resolvedMessage, retryable }, { status });
  };

  try {
    const supabase = await createSupabaseServer();
    const {
      data: { user },
      error,
    } = await getUserWithPolicy("required_verified", { supabaseOverride: supabase });

    if (error || !user) {
      return fail(401, "UNAUTHENTICATED");
    }

    const { id } = await context.params;
    const organizationId = Number(id);
    if (!organizationId || Number.isNaN(organizationId)) {
      return fail(400, "INVALID_ORGANIZATION_ID");
    }

    const membership = await resolveGroupMemberForOrg({ organizationId, userId: user.id });
    if (!membership) {
      return fail(403, "FORBIDDEN");
    }

    const organization = await prisma.organization.findUnique({
      where: { id: organizationId },
      select: {
        id: true,
        status: true,
        updatedAt: true,
      },
    });
    if (!organization) {
      return fail(404, "ORGANIZATION_NOT_FOUND");
    }

    const suspension = await getOrganizationSuspensionSnapshot({
      organizationId,
      status: organization.status,
      updatedAt: organization.updatedAt ?? null,
    });

    return respondOk(
      ctx,
      {
        organizationId,
        status: organization.status,
        suspension,
        canReactivate: membership.role === "OWNER" && suspension.reactivationWindowOpen,
      },
      { status: 200 },
    );
  } catch (err) {
    console.error("[organization/suspend-state]", err);
    return fail(500, "INTERNAL_ERROR");
  }
}

export const POST = withApiEnvelope(_POST);
export const DELETE = withApiEnvelope(_DELETE);
export const GET = withApiEnvelope(_GET);
