...
import {
  isAnalyticsAllowedView,
  type AnalyticsAllowedView,
} from "@/lib/domainBoundaries";
...
function parseTelemetrySource(raw: string | null | undefined) {
  ...
  if (
    ["WEB", "MOBILE", "API", "WORKER", "CRON", "INTERNAL"].includes(normalized)
  )
    return normalized;
  return "";
}
function parseTelemetrySeverity(raw: string | null | undefined) {
  if (!raw) return "";
  const normalized = raw.trim().toUpperCase();
  if (["INFO", "WARN", "ERROR", "CRITICAL"].includes(normalized))
    return normalized;
  return "";
}
function parseTelemetryIncidentStatuses(raw: string | null | undefined) {
  ...
  const valid = normalized.filter((token) =>
    ["OPEN", "ACKNOWLEDGED", "RESOLVED"].includes(token),
  );
...
function buildTelemetryIncidentSlaHint(
  ...
  if (incident.status === "RESOLVED" && incident.resolvedAt) {
    const resolvedMinutes = minutesBetween(
...
export default function AnalyticsToolClient({
  orgId,
  initialView,
}: AnalyticsToolClientProps) {
  ...
  const dimensionKey = parseDimension(
    searchParams?.get("dimensionKey") ?? null,
  );
  ...
  const telemetryHours = parseTelemetryHours(
    searchParams?.get("telemetryHours") ?? null,
  );
  const telemetrySource = parseTelemetrySource(
    searchParams?.get("telemetrySource") ?? null,
  );
  const telemetrySeverity = parseTelemetrySeverity(
    searchParams?.get("telemetrySeverity") ?? null,
  );
  ...
  const {
    ...
    mutate: mutateEvents,
  } = useSWR<AnalyticsEventsResponse>(eventsKey, apiFetcher, swrOptions);
  const effectiveEventId =
    selectedEventId ?? eventsData?.items?.[0]?.id ?? null;
  const buyersKey =
  ...
  const telemetryEventsKey =
    view === "telemetry"
      ? `${orgApiBase}/telemetry/events?${buildQueryString({ take: 80, sourceType: telemetrySource || null, severity: telemetrySeverity || null })}`
      : null;
  const telemetryIncidentsKey =
    view === "telemetry"
      ? `${orgApiBase}/telemetry/incidents?${buildQueryString({ take: 120, statuses: telemetryIncidentStatuses === "ALL" ? null : telemetryIncidentStatuses, severities: telemetryIncidentSeverity || null, q: telemetryIncidentQuery || null, sort: telemetryIncidentSort })}`
      : null;
  const telemetryFunnelsKey =
    view === "telemetry"
      ? `${orgApiBase}/telemetry/funnels?${buildQueryString({ includeGlobal: "true", activeOnly: "false" })}`
      : null;
  const telemetryFunnelResultsKey =
    view === "telemetry"
      ? `${orgApiBase}/telemetry/funnels/results?${buildQueryString({ take: 120, bucketUnit: "HOUR" })}`
      : null;
  const telemetryCatalogKey =
    view === "telemetry" ? `${orgApiBase}/telemetry/catalog` : null;
  const {
    data: overview,
    error: overviewError,
    isLoading: overviewLoading,
    mutate: mutateOverview,
  } = useSWR<AnalyticsOverviewResponse>(overviewKey, apiFetcher, swrOptions);
  const {
    data: conversion,
    error: conversionError,
    isLoading: conversionLoading,
    mutate: mutateConversion,
  } = useSWR<AnalyticsConversionResponse>(
    conversionKey,
    apiFetcher,
    swrOptions,
  );
  ...
  } = useSWR<AnalyticsCohortsResponse>(cohortsKey, apiFetcher, swrOptions);
  const {
    data: buyers,
    error: buyersError,
    isLoading: buyersLoading,
    mutate: mutateBuyers,
  } = useSWR<AnalyticsBuyersResponse>(buyersKey, apiFetcher, swrOptions);
  const {
    data: series,
    error: seriesError,
    isLoading: seriesLoading,
    mutate: mutateSeries,
  } = useSWR<AnalyticsTimeSeriesResponse>(seriesKey, apiFetcher, swrOptions);
  const {
    data: dimensions,
    error: dimensionsError,
    isLoading: dimensionsLoading,
    mutate: mutateDimensions,
  } = useSWR<AnalyticsDimensionsResponse>(
    dimensionsKey,
    apiFetcher,
    swrOptions,
  );
  ...
  } = useSWR<TelemetryOverviewResponse>(
    telemetryOverviewKey,
    apiFetcher,
    swrOptions,
  );
  const {
  ...
  } = useSWR<TelemetryEventsResponse>(
    telemetryEventsKey,
    apiFetcher,
    swrOptions,
  );
  ...
  } = useSWR<TelemetryFunnelListResponse>(
    telemetryFunnelsKey,
    apiFetcher,
    swrOptions,
  );
  ...
  } = useSWR<TelemetryFunnelResultsResponse>(
    telemetryFunnelResultsKey,
    apiFetcher,
    swrOptions,
  );
  const {
  ...
  } = useSWR<TelemetryCatalogResponse>(
    telemetryCatalogKey,
    apiFetcher,
    swrOptions,
  );
  const [telemetryActionBusyKey, setTelemetryActionBusyKey] = useState<
    string | null
  >(null);
  const [telemetryActionError, setTelemetryActionError] = useState<
    string | null
  >(null);
  const [telemetryActionInfo, setTelemetryActionInfo] = useState<string | null>(
    null,
  );
  ...
  const [telemetryFunnelBusyKey, setTelemetryFunnelBusyKey] = useState<
    string | null
  >(null);
  const [telemetryFunnelSaveBusy, setTelemetryFunnelSaveBusy] = useState(false);
  const [telemetryFunnelEditingId, setTelemetryFunnelEditingId] = useState<
    string | null
  >(null);
  const [telemetryFunnelName, setTelemetryFunnelName] = useState("");
  const [telemetryFunnelDescription, setTelemetryFunnelDescription] =
    useState("");
  const [telemetryFunnelIsActive, setTelemetryFunnelIsActive] = useState(true);
  const [telemetryFunnelSteps, setTelemetryFunnelSteps] = useState<
    TelemetryFunnelDraftStep[]
  >(() => defaultTelemetryFunnelDraftSteps());
  const [telemetryLastEvaluation, setTelemetryLastEvaluation] =
    useState<TelemetryEvaluationResult | null>(null);
  const [telemetryLastRecompute, setTelemetryLastRecompute] =
    useState<TelemetryRecomputeResult | null>(null);
  const [telemetryExportDataset, setTelemetryExportDataset] =
    useState<TelemetryExportDataset>("events");
  const [telemetryExportFormat, setTelemetryExportFormat] =
    useState<TelemetryExportFormat>("csv");
  const [telemetryExportPreviewBusy, setTelemetryExportPreviewBusy] =
    useState(false);
  const [telemetryExportPreview, setTelemetryExportPreview] =
    useState<TelemetryExportPreviewPayload | null>(null);
  const [telemetryIncidentRows, setTelemetryIncidentRows] = useState<
    TelemetryIncidentResponse[]
  >([]);
  const [telemetryIncidentHasMore, setTelemetryIncidentHasMore] =
    useState(false);
  const [telemetryIncidentNextCursor, setTelemetryIncidentNextCursor] =
    useState<string | null>(null);
  const [telemetryIncidentLoadMoreBusy, setTelemetryIncidentLoadMoreBusy] =
    useState(false);
  useEffect(() => {
    if (view !== "telemetry") return;
    const items = telemetryIncidents?.items ?? [];
    setTelemetryIncidentRows(items);
    setTelemetryIncidentHasMore(
      Boolean(telemetryIncidents?.pagination?.hasMore),
    );
    setTelemetryIncidentNextCursor(
      telemetryIncidents?.pagination?.nextCursor ?? null,
    );
  }, [
    telemetryIncidents?.items,
    telemetryIncidents?.pagination?.hasMore,
    telemetryIncidents?.pagination?.nextCursor,
    view,
  ]);
    ...
    async (incidentId: string, action: "ACK" | "RESOLVE") => {
      ...
      try {
        const res = await fetch(
          `${orgApiBase}/telemetry/incidents/${incidentId}`,
          {
            method: "PATCH",
        ...
        setTelemetryActionInfo(
          action === "ACK" ? "Incidente reconhecido." : "Incidente resolvido.",
        );
        await Promise.all([
        ...
        setTelemetryActionBusyKey((current) =>
  ...
  const runTelemetryEvaluate = useCallback(async () => {
    ...
    try {
      ...
      const result =
        ...
        (payload && typeof payload === "object"
          ? ((payload as Record<string, unknown>).result as
  ...
  const runTelemetryRecompute = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${orgApiBase}/telemetry/recompute?${params.toString()}`,
        { method: "POST" },
      );
      const payload = await res.json().catch(() => null);
      const body = unwrapEnvelope(payload) as Record<string, unknown> | null;
      const result =
        ...
        (payload && typeof payload === "object"
          ? (payload as Record<
              string,
              unknown
            > as unknown as TelemetryRecomputeResult)
          : null);
      const ok = res.ok && body?.ok !== false;
      if (!ok || !result?.rollup) {
        const errorCode =
  ...
  const runTelemetryExport = useCallback(() => {
    ...
    if (telemetryExportDataset === "events") {
      ...
      params.set(
        "statuses",
        telemetryIncidentStatuses === "ALL" ? "ALL" : telemetryIncidentStatuses,
      );
      params.set("sort", telemetryIncidentSort);
      if (telemetryIncidentSeverity)
        params.set("severity", telemetryIncidentSeverity);
      if (telemetryIncidentQuery) params.set("q", telemetryIncidentQuery);
    } else if (
      telemetryExportDataset === "rules" ||
      telemetryExportDataset === "funnels"
    ) {
    ...
    window.open(
      `${orgApiBase}/telemetry/export?${params.toString()}`,
      "_blank",
      "noopener,noreferrer",
    );
    setTelemetryActionInfo(
      telemetryExportFormat === "pdf"
        ? "Exportação PDF iniciada."
  ...
  const runTelemetryExportPreview = useCallback(async () => {
    ...
    try {
      ...
      const payload = await apiFetcher<{
        preview: TelemetryExportPreviewPayload;
  ...
  const loadMoreTelemetryIncidents = useCallback(async () => {
    ...
    try {
      ...
      params.set("sort", telemetryIncidentSort);
      if (telemetryIncidentStatuses !== "ALL")
        params.set("statuses", telemetryIncidentStatuses);
      if (telemetryIncidentSeverity)
        params.set("severities", telemetryIncidentSeverity);
      if (telemetryIncidentQuery) params.set("q", telemetryIncidentQuery);
  ...
  const startEditTelemetryFunnel = useCallback(
    (funnel: TelemetryFunnelDefinitionResponse) => {
  ...
  const saveTelemetryFunnel = useCallback(async () => {
    ...
    for (const step of telemetryFunnelSteps) {
      ...
      const withinMinutes = parseTelemetryFunnelWithinMinutes(
        step.withinMinutes,
      );
      ...
      normalizedSteps.push({ key, eventName, required: true, withinMinutes });
    }
  ...
  const toggleTelemetryFunnelActive = useCallback(
    async (funnel: TelemetryFunnelDefinitionResponse) => {
      ...
      try {
        const res = await fetch(
          `${orgApiBase}/telemetry/funnels/${funnel.id}`,
          {
            method: "PATCH",
            headers: { "content-type": "application/json" },
        ...
        setTelemetryActionInfo(
          funnel.isActive ? "Funil desativado." : "Funil ativado.",
        );
        await Promise.all([
          mutateTelemetryFunnels(),
          mutateTelemetryFunnelResults(),
        ...
        setTelemetryFunnelBusyKey((current) =>
          current === busyKey ? null : current,
        );
      }
    },
  ...
  const refreshCurrentView = useCallback(async () => {
    if (view === "overview")
      await Promise.all([mutateOverview(), mutateSeries()]);
  ...
  const effectiveMetricKey = dimensionsMetricOptions.includes(metricKey)
    ? metricKey
    : dimensionsMetricOptions[0];
  ...
  const cohortsHeatmapRows = useMemo(
  ...
  const visibleCohortOffsets = useMemo(
    () =>
      Array.from(
        { length: Math.min(8, Math.max(1, months)) },
        (_, index) => index,
      ),
    [months],
  );
  const buyersSortedByPaid = useMemo(
    () =>
      [...(buyers?.items ?? [])].sort(
        (a, b) => (b.totalPaidCents ?? 0) - (a.totalPaidCents ?? 0),
      ),
    [buyers?.items],
  );
  const buyersTotalPaidCents = useMemo(
    () =>
      buyersSortedByPaid.reduce(
        (sum, item) => sum + (item.totalPaidCents ?? 0),
        0,
      ),
    [buyersSortedByPaid],
  );
  ...
  const telemetryFunnelRows = useMemo(
    () =>
      (telemetryFunnelResults?.items ?? [])
        .slice(0, 40)
        .map((item) => ({
          ...item,
          funnelName:
            telemetryFunnelsById.get(item.funnelId)?.name ?? item.funnelId,
        })),
  ...
  const activeFilters = useMemo(() => {
    ...
    if (view === "cohorts")
      chips.push({ id: "months", label: `Janela: ${months}m` });
    if (view === "dimensions") {
      chips.push({
        id: "dimension",
        label: `Dimensão: ${prettyDimensionKey(dimensionKey)}`,
      });
      chips.push({
        id: "metric",
        label: `Métrica: ${prettyMetricKey(effectiveMetricKey)}`,
      });
    }
    if (view === "buyers" && effectiveEventId)
      chips.push({ id: "event", label: `Evento #${effectiveEventId}` });
    if (view === "telemetry") {
      chips.push({
        id: "telemetry-hours",
        label: `Telemetria: ${telemetryHours}h`,
      });
      if (telemetrySource)
        chips.push({
          id: "telemetry-source",
          label: `Source: ${telemetrySource}`,
        });
      if (telemetrySeverity)
        chips.push({
          id: "telemetry-severity",
          label: `Sev: ${telemetrySeverity}`,
        });
      if (telemetryIncidentStatuses !== "OPEN,ACKNOWLEDGED") {
        chips.push({
          id: "telemetry-incident-statuses",
          label: `Incidentes: ${telemetryIncidentStatuses}`,
        });
      }
      if (telemetryIncidentSeverity) {
        chips.push({
          id: "telemetry-incident-severity",
          label: `Sev incidente: ${telemetryIncidentSeverity}`,
        });
      }
      if (telemetryIncidentQuery) {
        chips.push({
          id: "telemetry-incident-query",
          label: `Pesquisa incidente: ${telemetryIncidentQuery}`,
        });
      }
      if (telemetryIncidentSort !== "TRIGGERED_DESC") {
        chips.push({
          id: "telemetry-incident-sort",
          label: "Ordenação incidente: impacto SLA",
        });
      }
    }
    return chips;
  ...
  return (
    <section className="space-y-5 text-white sm:space-y-6">
      {" "}
      <div className="rounded-3xl border border-white/16 bg-white/[0.04] px-4 py-4 sm:px-6 sm:py-5">
        {" "}
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-white/70">
              Analytics focado em BI/performance monetária, sem CRM.
            </p>{" "}
          </div>{" "}
          <div className="rounded-xl border border-emerald-300/45 bg-emerald-300/12 px-3 py-2 text-xs text-emerald-100">
            {" "}
            Domínio de dados:{" "}
            <span className="font-semibold">BI financeiro</span>{" "}
          </div>{" "}
        </div>{" "}
      </div>{" "}
      <div className="rounded-2xl border border-white/12 bg-[#141414]/88 p-4">
        {" "}
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-6">
          ...
          {view === "dimensions" && (
            <FilterSelect
              ...
              options={dimensionsMetricOptions.map((option) => ({
                label: prettyMetricKey(option),
                value: option,
              }))}
            />
          )}{" "}
          {view === "buyers" && (
            <FilterSelect
              ...
              placeholder={
                eventsLoading ? "A carregar eventos..." : "Sem eventos"
              }
            />
          )}{" "}
          {view === "telemetry" && (
            <FilterSelect
              ...
              onChange={(value) =>
                updateQuery({ telemetryHours: Number(value) })
              }
              options={[
          ...
          {view === "telemetry" && (
            <FilterSelect
              ...
              onChange={(value) =>
                updateQuery({ telemetrySource: value || null })
              }
              options={[
                { label: "Todos", value: "" },
                { label: "WEB", value: "WEB" },
          ...
          {view === "telemetry" && (
            <FilterSelect
              ...
              onChange={(value) =>
                updateQuery({ telemetrySeverity: value || null })
              }
              options={[
                { label: "Todas", value: "" },
          ...
          {view === "telemetry" && (
            <FilterSelect
              ...
              onChange={(value) =>
                updateQuery({ telemetryIncidentSeverity: value || null })
              }
              options={[
                { label: "Todas", value: "" },
                { label: "INFO", value: "INFO" },
          ...
          {view === "telemetry" && (
            <label className="flex flex-col gap-1">
              ...
              <input
                ...
                onBlur={(event) =>
                  updateQuery({
                    telemetryIncidentQuery: event.target.value.trim() || null,
                  })
                }
              />{" "}
            </label>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          ...
          {activeFilters.map((filter) => (
            <span
              key={filter.id}
              className="rounded-md border border-white/24 bg-white/[0.08] px-2 py-1 text-xs text-white/85"
            >
              {" "}
              {filter.label}{" "}
            </span>
        ...
        >
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            ...
            <MetricCard
              label={`Líquido (${range})`}
              value={toCurrency(
                overview?.netRevenueCents,
                overview?.currency ?? "EUR",
              )}
            />{" "}
            <MetricCard
              label="Eventos com vendas"
              value={String(overview?.eventsWithSalesCount ?? 0)}
            />{" "}
            <MetricCard
          ...
          <Panel
            ...
            subtitle="Bruto, taxas e líquido no tempo"
          >
            {" "}
                ...
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <Tooltip
                      ...
                      formatter={(value, key) => [
                        toEuroChartLabel(value),
                        String(key ?? ""),
                      ]}
                    />{" "}
                    <Legend
                      formatter={formatRechartsLegendLabel}
                      wrapperStyle={RECHARTS_LEGEND_WRAPPER_STYLE}
                    />{" "}
                    <Area
                      type="monotone"
                      dataKey="gross"
        ...
        >
          ...
          <div className="grid gap-3 md:grid-cols-3">
            ...
            <MetricCard
              ...
              value={String(conversion?.startedCount ?? 0)}
            />{" "}
            <MetricCard
              label="Pagamentos concluídos"
              value={String(conversion?.succeededCount ?? 0)}
            />{" "}
            <MetricCard
              label="Taxa de conversão"
              value={toPctFromBps(conversion?.conversionRateBps)}
            />{" "}
          </div>{" "}
          <Panel
            title="Funil por tipo de origem"
      ...
      {view === "cohorts" && (
        ...
        >
          {" "}
          <Panel
            title="Heatmap de retenção"
            subtitle={`Janela M+0..M+${Math.max(0, months - 1)}`}
          >
            {" "}
            <div className="space-y-2 overflow-auto">
              ...
              <p className="text-[11px] text-white/55">
                {" "}
                Deslize horizontalmente no mobile para ver todos os meses
                visíveis.{" "}
              </p>{" "}
              <div
                className="grid min-w-[760px] gap-1 text-[11px] uppercase tracking-wide text-white/60"
                style={{
                  gridTemplateColumns: `140px repeat(${visibleCohortOffsets.length}, minmax(86px, 1fr))`,
                }}
              >
                {" "}
                <div>Coorte</div>{" "}
              ...
              {cohortsHeatmapRows.map((cohort) => (
                ...
                >
                  ...
                  {visibleCohortOffsets.map((offset) => {
                    ...
                    return (
                      <div
                        key={`${cohort.cohortMonth}-${row.monthOffset}`}
                        className="rounded-md border border-white/10 px-2 py-2"
                        style={{ background: bg }}
                      >
                        {" "}
                        <div className="font-semibold text-white">
                          {toPctFromBps(row.retentionRateBps)}
                        </div>{" "}
                        <div className="text-[11px] text-white/75">
                          {row.retainedBuyers} compradores
                        </div>{" "}
                        <div className="text-[11px] text-white/75">
                          {toCurrency(row.revenueCents)}
                        </div>{" "}
                      </div>
                    );
                  })}{" "}
      ...
      {view === "buyers" && (
        ...
        >
          ...
          <div className="grid gap-3 md:grid-cols-3">
            ...
            <MetricCard
              label="Compradores"
              value={String(buyersSortedByPaid.length)}
            />{" "}
            <MetricCard
              label="Total pago"
              value={toCurrency(buyersTotalPaidCents)}
            />{" "}
            <MetricCard
              label="Estados ativos"
              value={String(buyersStatusChartData.length)}
            />{" "}
          </div>{" "}
          <div className="grid gap-3 lg:grid-cols-[340px_1fr]">
            ...
            <Panel
              ...
              subtitle={`Evento #${effectiveEventId ?? "—"}`}
            >
              {" "}
                    ...
                    <PieChart>
                      ...
                      <Tooltip
                        ...
                        formatter={(value, key, item) => {
                          ...
                          const amountCents =
                            Number(payload?.amount ?? 0) * 100;
                          return [
                            `${count} compras · ${toCurrency(amountCents)}`,
                            String(key ?? "estado"),
                          ];
                        }}
                      />{" "}
            ...
            <Panel
              ...
              subtitle="Ordenada por valor pago"
            >
              {" "}
                ...
                {buyersSortedByPaid.map((item) => (
                  ...
                  >
                    ...
                    <div className="mt-2 flex items-center justify-between text-xs">
                      {" "}
                      <span className="rounded-md border border-white/15 bg-white/[0.08] px-2 py-1 text-white/85">
                        {item.status}
                      </span>{" "}
                      <span className="font-semibold text-emerald-200">
                        {toCurrency(item.totalPaidCents)}
                      </span>{" "}
                    </div>{" "}
                  </div>
                ))}{" "}
      ...
      {view === "time-series" && (
        ...
        >
          ...
          <Panel
            ...
            subtitle="Leitura diária de bruto, taxas e líquido"
          >
            {" "}
          ...
          <Panel
            ...
            subtitle={
              dimensions?.bucketDate
                ? `Agregado: ${dimensions.bucketDate}`
                : "Sem agregado definido"
            }
          >
            {" "}
                ...
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <Tooltip
                      ...
                      formatter={(value) => [
                        toEuroChartLabel(value),
                        prettyMetricKey(effectiveMetricKey),
                      ]}
                    />{" "}
                    <Bar
                      dataKey="value"
      ...
      {view === "telemetry" && (
        ...
        >
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-6">
            ...
            <MetricCard
              label="Eventos"
              value={String(telemetryOverview?.totals.totalEvents ?? 0)}
            />{" "}
            <MetricCard
              label="Erros"
              value={String(telemetryOverview?.totals.errorEvents ?? 0)}
            />{" "}
            <MetricCard
              label="Taxa de erro"
            ...
            <MetricCard
              ...
              value={String(telemetryOpenIncidents.length)}
            />{" "}
            <MetricCard
              label="Funis activos"
              value={String(telemetryActiveFunnels.length)}
            />{" "}
          </div>{" "}
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            {" "}
            <MetricCard
              label="MTTA"
              value={formatTelemetryMinutes(telemetryIncidentKpis?.mttaMinutes)}
            />{" "}
            <MetricCard
              label="MTTR"
              value={formatTelemetryMinutes(telemetryIncidentKpis?.mttrMinutes)}
            />{" "}
            <MetricCard
          ...
          <div className="grid gap-3 lg:grid-cols-[1.2fr_1fr]">
            ...
            <Panel
              title="Incidentes"
              subtitle="Acompanhar, filtrar e fechar alertas desta organização"
            >
              {" "}
              {telemetryActionInfo ? (
              ...
              <div className="space-y-2">
                ...
                {telemetryIncidentItems.map((incident) => {
                  ...
                  const ackBusy =
                    telemetryActionBusyKey === `${incident.id}:ACK`;
                  const resolveBusy =
                    telemetryActionBusyKey === `${incident.id}:RESOLVE`;
                  const slaHint = buildTelemetryIncidentSlaHint(
                    incident,
                    telemetryIncidentKpis,
                  );
                  return (
                    <div
                      key={incident.id}
                    ...
                    >
                      ...
                      <p className="mt-1 text-xs text-white/70">
                        {" "}
                        {formatTelemetryMetricKey(incident.metricKey)} ·
                        Observado {incident.observedValue ?? 0} / Limite{" "}
                        {incident.thresholdValue ?? 0}{" "}
                      </p>{" "}
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        ...
                        {canAck ? (
                          <button
                            ...
                            disabled={
                              ackBusy || Boolean(telemetryActionBusyKey)
                            }
                            onClick={() =>
                              void applyTelemetryIncidentAction(
                                incident.id,
                                "ACK",
                              )
                            }
                        ...
                        {canResolve ? (
                          <button
                            ...
                            disabled={
                              resolveBusy || Boolean(telemetryActionBusyKey)
                            }
                            onClick={() =>
                              void applyTelemetryIncidentAction(
                                incident.id,
            ...
            <Panel
              title="Regras e avaliação"
              subtitle="Regras activas e avaliação manual imediata"
            >
              {" "}
              <div className="mb-3 flex flex-wrap gap-2">
                ...
                >
                  {" "}
                  {telemetryEvaluateBusy
                    ? "A avaliar..."
                    : "Avaliar alertas agora"}{" "}
                </button>{" "}
                <button
                  type="button"
                ...
                <select
                  ...
                  onChange={(event) =>
                    setTelemetryExportDataset(
                      event.target.value as TelemetryExportDataset,
                    )
                  }
                >
                  {" "}
                  ...
                  <option value="funnel_results">
                    Exportar: resultados funil
                  </option>{" "}
                </select>{" "}
                <select
                  className="rounded-md border border-white/20 bg-black/25 px-2 py-1.5 text-xs text-white outline-none transition focus:border-cyan-300/80"
                  ...
                  onChange={(event) =>
                    setTelemetryExportFormat(
                      event.target.value as TelemetryExportFormat,
                    )
                  }
                >
                  {" "}
              ...
              {telemetryLastEvaluation ? (
                <div className="mb-3 rounded-lg border border-white/15 bg-black/20 px-3 py-2 text-[11px] text-white/75">
                  {" "}
                  {telemetryLastEvaluation.evaluatedRules} regras ·{" "}
                  {telemetryLastEvaluation.openedIncidents} novos incidentes ·
                  {""} {telemetryLastEvaluation.resolvedIncidents}{" "}
                  resolvidos{" "}
                </div>
              ) : null}{" "}
              {telemetryLastRecompute ? (
                <div className="mb-3 rounded-lg border border-emerald-300/30 bg-emerald-400/10 px-3 py-2 text-[11px] text-emerald-100">
                  {" "}
                  Recompute: {telemetryLastRecompute.rollup.written} linhas
                  rollup ·{""}{" "}
                  {telemetryLastRecompute.funnels?.rowsWritten ?? 0} linhas de
                  funil.{" "}
                </div>
              ) : null}{" "}
                ...
                <div className="mb-3 rounded-xl border border-white/14 bg-black/20 p-3">
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    ...
                    <p className="text-[11px] text-white/60">
                      ...
                      {telemetryExportPreview.truncated
                        ? ` · amostra ${telemetryExportPreview.sampleSize}`
                        : ""}{" "}
                    </p>{" "}
                  </div>{" "}
                    ...
                    <table className="min-w-full text-[11px]">
                      ...
                      <thead className="bg-white/5 text-left uppercase tracking-[0.12em] text-white/50">
                        ...
                        <tr>
                          ...
                          {telemetryExportPreview.headers.map((header) => (
                            <th
                              key={`telemetry-preview-head-${header}`}
                      ...
                      <tbody className="divide-y divide-white/10">
                        ...
                        {telemetryExportPreview.rows.map((row, rowIndex) => (
                          <tr
                            key={`telemetry-preview-row-${rowIndex}`}
                            className="bg-black/15"
                          >
                            {" "}
                            {telemetryExportPreview.headers.map(
                              (header, colIndex) => (
                                <td
              ...
              <div className="space-y-2">
                ...
                {telemetryActiveRules.map((rule) => (
                  <div
                    key={rule.id}
                    className="rounded-xl border border-white/12 bg-black/20 px-3 py-2 text-sm"
                  >
                    {" "}
                    ...
                    <p className="mt-1 text-xs text-white/65">
                      {" "}
                      {formatTelemetryMetricKey(rule.metricKey)} ·{" "}
                      {rule.comparisonOperator} {rule.threshold} · janela{" "}
          ...
          <div className="grid gap-3 lg:grid-cols-2">
            ...
            <Panel
              title="Funis activos"
              subtitle="Definições activas para análise desta organização"
            >
              {" "}
              <div className="space-y-2">
                {" "}
                      ...
                      {funnel.organizationId === orgId ? (
                        <>
                          ...
                          <button
                            ...
                            onClick={() =>
                              void toggleTelemetryFunnelActive(funnel)
                            }
                            disabled={
                              telemetryFunnelBusyKey ===
                                `funnel:${funnel.id}` || telemetryFunnelSaveBusy
                            }
                          >
                            {" "}
            ...
            <Panel
              title="Resultados de funil"
              subtitle="Conversões recentes por passo (hora)"
            >
              {" "}
              <div className="space-y-2">
                {" "}
                {telemetryFunnelRows.map((item) => (
                  ...
                  >
                    ...
                    <p className="mt-1 text-xs text-white/65">
                      ...
                      · entraram {item.enteredCount} · converteram{" "}
                      {item.convertedCount} · taxa{""}{" "}
          ...
          >
            ...
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <label className="flex flex-col gap-1">
                ...
                <input
                  ...
                  onChange={(event) =>
                    setTelemetryFunnelName(event.target.value)
                  }
              ...
              <label className="flex flex-col gap-1">
                ...
                <select
                  ...
                  onChange={(event) =>
                    setTelemetryFunnelIsActive(event.target.value === "true")
                  }
                >
                  {" "}
            ...
            <label className="mt-3 flex flex-col gap-1">
              ...
              <textarea
                ...
                onChange={(event) =>
                  setTelemetryFunnelDescription(event.target.value)
                }
            ...
            <div className="mt-3 rounded-xl border border-white/12 bg-black/20 px-3 py-2 text-[11px] text-white/70">
              {" "}
              Catálogo ORYA disponível:{" "}
              {telemetryCatalog?.total ??
                telemetryCatalog?.items?.length ??
                0}{" "}
              eventos canónicos.{" "}
              {telemetryDraftUnknownEvents.length > 0 ? (
                <p className="mt-1 text-amber-200">
                  {" "}
                  Eventos fora de catálogo no draft:{" "}
                  {telemetryDraftUnknownEvents.join(",")}{" "}
                </p>
              ) : null}{" "}
            </div>{" "}
                ...
                >
                  ...
                  <div className="grid gap-2 md:grid-cols-[1fr_2fr_1fr_auto]">
                    ...
                    <input
                      ...
                      value={step.key}
                      onChange={(event) =>
                        updateTelemetryFunnelStep(index, {
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        updateTelemetryFunnelStep(index, {
                          withinMinutes: event.target.value,
                        })
                      }
                      inputMode="numeric"
          ...
          <Panel
            title="Timeline de telemetria"
            subtitle="Eventos totais vs erros"
          >
            {" "}
          ...
          <div className="grid gap-3 lg:grid-cols-2">
            ...
            <Panel
              title="Top eventos"
              subtitle="Mais frequentes na janela selecionada"
            >
              {" "}
              <div className="space-y-2">
                {" "}
                {telemetryTopEvents.map((item) => (
                  <div
                    key={item.eventName}
                    className="flex items-center justify-between rounded-xl border border-white/12 bg-black/20 px-3 py-2 text-sm"
                  >
                    {" "}
                ...
                {telemetryTopEvents.length === 0 && (
                  <EmptyState label="Sem eventos agregados." />
                )}{" "}
              </div>{" "}
            </Panel>{" "}
            <Panel
              title="Breakdown por source"
              subtitle="Distribuição da origem de eventos"
            >
              {" "}
              <div className="space-y-2">
                {" "}
                ...
                {telemetrySourceBreakdown.length === 0 && (
                  <EmptyState label="Sem breakdown por source." />
                )}{" "}
              </div>{" "}
            </Panel>{" "}
          </div>{" "}
          <Panel
            ...
            subtitle="Feed operativo de telemetria"
          >
            {" "}
            <div className="space-y-2 md:hidden">
              {" "}
              {(telemetryEvents?.items ?? []).map((item) => (
                ...
                >
                  ...
                  <div className="mt-2 flex items-center gap-2 text-[11px]">
                    {" "}
                    <span className="rounded-md border border-white/15 bg-white/[0.08] px-2 py-1">
                      {item.sourceType}
                    </span>{" "}
                    <span className="rounded-md border border-white/15 bg-white/[0.08] px-2 py-1">
                      {item.severity}
...
function ViewSection({
  ...
  if (loading) {
    return (
      <div className="rounded-2xl border border-white/12 bg-white/[0.04] p-4">
        {" "}
        <p className="mb-1 text-xs uppercase tracking-[0.14em] text-white/55">
          A carregar dados da vista
        </p>{" "}
        <p className="mb-3 text-[12px] text-white/65">
          A sincronizar métricas e séries em tempo real.
        </p>{" "}
  ...
  if (error) {
    return (
      <div className="rounded-2xl border border-rose-300/50 bg-white/[0.04] p-4 text-sm text-rose-100">
        {" "}
        <p className="font-semibold">
          Falha ao carregar dados desta vista.
        </p>{" "}
        <p className="mt-1 rounded-md border border-rose-200/30 bg-black/15 px-2 py-1 text-rose-100/85">
          {" "}
          {formatAnalyticsError(error)}{" "}
        </p>{" "}
        <p className="mt-1 text-xs text-rose-100/65">
          Se persistir, valide filtros e permissoes de ferramenta.
        </p>{" "}
        <button
          type="button"
          onClick={onRetry}
