...
export default function EventAttendeesPanel({
    ...
    async (cursor: string | null, mode: "reset" | "append") => {
      ...
      try {
        ...
        const data = (await res
          .json()
          .catch(() => null)) as AttendeesResponse | null;
        ...
        setItems((prev) =>
          mode === "reset" ? nextItems : [...prev, ...nextItems],
        );
  ...
  const handleRefund = async (
    ...
    try {
      const res = await fetch(
        resolveCanonicalOrgApiPath(`/api/org/[orgId]/events/${eventId}/refund`),
        {
          method: "POST",
  ...
  return (
    <section className="rounded-2xl border border-white/12 bg-black/40 p-5 space-y-4">
      {" "}
            ...
            return (
              ...
              >
                ...
                <div className="space-y-1">
                  ...
                  {holderEmail && (
                    <p className="text-[11px] text-white/60">{holderEmail}</p>
                  )}{" "}
                  <p className="text-[10px] text-white/40">
                    {" "}
                    Compra {formatPurchaseId(item.purchaseId)} • Entitlement{" "}
                    {item.entitlementId.slice(0, 6)}{" "}
                  </p>{" "}
                </div>{" "}
                <div className="flex flex-wrap items-center gap-2 text-[11px] text-white/70">
                  ...
                  {checkedInLabel && (
                    <span className="text-[10px] text-white/55">
                      Check-in {checkedInLabel}
                    </span>
                  )}{" "}
                  {refundedLabel && (
                    <span className="text-[10px] text-white/55">
                      Reembolso {refundedLabel}
                    </span>
                  )}{" "}
                </div>{" "}
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    disabled={
                      refundDisabled ||
                      Boolean(item.purchaseId && refundBusy[item.purchaseId])
                    }
                    className={cn(
                  ...
                  >
                    {" "}
                    {item.purchaseId && refundBusy[item.purchaseId]
                      ? "A reembolsar..."
                      : "Reembolsar"}{" "}
                  </button>{" "}
                </div>{" "}
