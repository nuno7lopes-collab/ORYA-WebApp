...
"[project]/app/api/organizador/promo/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function requireOrganizer(req) {
    ...
    if (!membership || !organizer || !(0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$organizerPermissions$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["isOrgAdminOrAbove"])(membership.role)) {
        return {
...
async function GET(req) {
    try {
        ...
        if ("error" in ctx) {
            const status = ctx.error === "UNAUTHENTICATED" ? 401 : ctx.error === "PROFILE_NOT_FOUND" ? 404 : 403;
        ...
        if (!promoRepo) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                ok: false,
                error: "Promo codes indisponíveis nesta instância do Prisma."
            }, {
                status: 500
            });
        }
        const organizerEvents = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"].event.findMany({
            where: {
        ...
        const promoCodes = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"].promoCode.findMany({
            where: {
                OR: [
                    {
                        organizerId: ctx.organizer.id
        ...
        const validRedemptionSummaryIds = new Set();
        promoCodes.forEach((p)=>{
            p.redemptions.forEach((r)=>{
                if (r.saleSummaryId) validRedemptionSummaryIds.add(r.saleSummaryId);
            });
        });
        const lines = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"].saleLine.findMany({
            where: {
                eventId: {
                    in: eventIds
                },
                ...
                ...validRedemptionSummaryIds.size > 0 ? {
                    saleSummaryId: {
                        in: Array.from(validRedemptionSummaryIds)
                    }
                } : {}
            },
            select: {
        ...
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            ...
            promoCodes: promoCodes.map((p)=>({
                    ...p,
                    status: !p.active ? "INACTIVE" : p.validUntil && new Date(p.validUntil) < new Date() ? "EXPIRED" : "ACTIVE",
...
async function POST(req) {
    try {
        ...
        const { code, type, value, maxUses, perUserLimit, validFrom, validUntil, eventId, active, autoApply, minQuantity, minTotalCents, name, description, minCartValueCents } = body;
        const cleanCode = (code || "").trim();
        const auto = Boolean(autoApply);
        // Se autoApply e sem código, gerar um placeholder interno
        const finalCode = auto && !cleanCode ? `AUTO-${Math.random().toString(36).slice(2, 8).toUpperCase()}` : cleanCode;
        if (!finalCode) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                ok: false,
        ...
        if (eventId !== null && eventId !== undefined) {
            const exists = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"].event.findFirst({
                where: {
                    id: Number(eventId),
                    organizerId: ctx.organizer.id
                },
            ...
            if (!exists) {
                return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                    ok: false,
                    error: "Evento inválido ou não pertence ao organizador."
                }, {
                    status: 400
                });
            }
        ...
        const created = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"].promoCode.create({
            data: {
                code: finalCode,
                type,
...
async function PATCH(req) {
    try {
        ...
        const promo = await promoRepo.findUnique({
            where: {
                id: body.id
            }
        });
        ...
        if (!promo.organizerId && promo.eventId) {
            const evt = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"].event.findFirst({
                where: {
                    id: promo.eventId,
                    organizerId: ctx.organizer.id
                },
        ...
        const { active, autoApply, code, type, value, maxUses, perUserLimit, validFrom, validUntil, eventId, minQuantity, minTotalCents, minCartValueCents, name, description } = body;
        ...
        if (targetEventId !== undefined) {
            dataUpdate.event = targetEventId === null ? {
                disconnect: true
            } : {
                connect: {
                    id: targetEventId
        ...
        const updated = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"].promoCode.update({
            where: {
                id: promo.id
            },
            data: dataUpdate
...
async function DELETE(req) {
    try {
        ...
        const promo = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"].promoCode.findUnique({
            where: {
                id: body.id
            }
        });
        ...
        await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"].promoCode.delete({
            where: {
                id: promo.id
            }
        });
...
//# sourceMappingURL=%5Broot-of-the-server%5D__b9094504._.js.map