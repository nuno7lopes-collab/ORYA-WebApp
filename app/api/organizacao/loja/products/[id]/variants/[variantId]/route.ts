...
async function _PATCH(
  ...
  try {
    ...
    if (!variantId.ok) {
      return jsonWrap({ ok: false, error: variantId.error }, { status: 400 });
    }
    ...
    if (!existing) {
      return jsonWrap({ ok: false, error: "Variante nao encontrada." }, { status: 404 });
    }
    ...
    return jsonWrap({ ok: false, error: "Erro ao atualizar variante." }, { status: 500 });
  }
}
...
async function _DELETE(
  ...
  try {
    ...
    return jsonWrap({ ok: false, error: "Erro ao remover variante." }, { status: 500 });
  }
}
