...
export default function ServicoDetalhePage() {
  ...
  const availabilityKey = serviceId ? `/api/organizacao/servicos/${serviceId}/disponibilidade` : null;
  ...
  const createAvailability = async () => {
    ...
    try {
      const res = await fetch(`/api/organizacao/servicos/${serviceId}/disponibilidade`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          startsAt: startsAt ? new Date(startsAt).toISOString() : null,
          durationMinutes: durationMinutes ? Number(durationMinutes) : service?.durationMinutes,
          capacity: Number.isFinite(Number(capacity)) ? Math.floor(Number(capacity)) : 0,
        }),
      });
  ...
  const cancelAvailability = async (availabilityId: number) => {
    if (!serviceId) return;
    try {
      await fetch(`/api/organizacao/servicos/${serviceId}/disponibilidade/${availabilityId}`, {
        method: "DELETE",
      });
      mutateAvailability();
    } catch {
  ...
  const toggleService = async () => {
    ...
    await fetch(`/api/organizacao/servicos/${serviceId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
