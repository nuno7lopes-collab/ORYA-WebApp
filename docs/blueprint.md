...
**Autoridade (SSOT):** Este documento é normativo/arquitetural e substitui **todas** as versões anteriores (incl. v7/v8 e drafts). A autoridade final dos contratos SSOT é o `docs/ssot_registry.md`.  
**Regra de conflito:** em caso de divergência, prevalece o `docs/ssot_registry.md`; depois as secções marcadas como **FECHADO** (e, se persistir, prevalece a secção mais específica do domínio).
...
- Split de receitas por sourceType (TICKET_ORDER / BOOKING / PADEL_REGISTRATION / STORE_ORDER), com settlement automático via Stripe Connect (destination charges). Controlo interno de payouts está desativado.
