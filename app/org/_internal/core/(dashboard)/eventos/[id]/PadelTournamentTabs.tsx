...
type GenerationPlanAlternative = { type?: string; summary?: string };
...
const isMatchUnscheduled = (match: Match) =>
  !match.plannedStartAt && !match.startTime;
const doesMatchPassLiveOpsFilter = (match: Match, filter: LiveOpsFilter) => {
  switch (filter) {
    case "ACTION_REQUIRED":
      return (
        LIVE_ACTION_STATUSES.has(match.status) || isMatchDisputeOpen(match)
      );
...
const resolvePairingStatusLabel = (pairing: Pairing, locale: string) => {
  if (pairing.pairingStatus === "CANCELLED")
    return t("pairingStatusCancelled", locale);
  if (pairing.lifecycleStatus === "CANCELLED_INCOMPLETE")
    return t("pairingStatusExpired", locale);
  const slots = pairing.slots || [];
  const allFilled =
    slots.length > 0 && slots.every((slot) => slot.slotStatus === "FILLED");
  const allPaid =
    slots.length > 0 && slots.every((slot) => slot.paymentStatus === "PAID");
  if (allFilled && allPaid) return t("pairingStatusConfirmed", locale);
  if (pairing.pairingJoinMode === "LOOKING_FOR_PARTNER")
    return t("pairingStatusMatchmaking", locale);
...
const resolveCategoryTeamsForPlanning = (
  ...
  const fallback = Math.max(
    0,
    Math.floor(Number.isFinite(fallbackTeams) ? fallbackTeams : 0),
  );
...
const isAmMxFormatValue = (format: string | null | undefined) =>
  format === "AMERICANO" || format === "MEXICANO";
const isNonStopFormatValue = (format: string | null | undefined) =>
  format === "NON_STOP";
export default function PadelTournamentTabs({
  ...
  const orgApi = (suffix: string, explicitOrgId?: number | null) =>
    resolveCanonicalOrgApiPath(
      ...
      explicitOrgId ?? resolvedOrganizationId,
    );
  const searchParams = useSearchParams();
  const locale = resolveLocale(searchParams?.get("lang"));
  const [tab, setTab] = useState<"duplas" | "grupos" | "eliminatorias">(
    "duplas",
  );
  ...
  const [importMode, setImportMode] = useState<"preview" | "import" | null>(
    null,
  );
  ...
  >(null);
  const [generationBusy, setGenerationBusy] = useState<
    "GROUPS" | "KNOCKOUT" | null
  >(null);
  ...
  const [generationPlanDetails, setGenerationPlanDetails] =
    useState<GenerationPlanDetails | null>(null);
  const [planningMode, setPlanningMode] = useState<"runtime" | "capacity">(
    "runtime",
  );
  const [planningPreview, setPlanningPreview] =
    useState<GenerationPlanDetails | null>(null);
  ...
  const [koEdits, setKoEdits] = useState<
    Record<number, { pairingAId: number | null; pairingBId: number | null }>
  >({});
    ...
    async (nextCoverUrl: string | null) => {
      ...
      try {
        const res = await fetch(`/api/org/${organizationId}/events/update`, {
          ...
          body: JSON.stringify({ eventId, coverImageUrl: nextCoverUrl }),
        });
  ...
  const categoryOptions = useMemo(() => {
    ...
    (categoriesMeta || [])
      ...
      .forEach((c) => {
        ...
        byId.set(id, { id, label: c.name || `Categoria ${id}` });
      });
  ...
  useEffect(() => {
    ...
    if (
      selectedCategoryId &&
      categoryOptions.some((c) => c.id === selectedCategoryId)
    )
      return;
  ...
  const { data: waitlistRes, mutate: mutateWaitlist } = useSWR(
    eventId
      ? orgApi(`/padel/waitlist?eventId=${eventId}${categoryParam}`)
      : null,
    fetcher,
  );
  ...
  const orgIdForMe =
    typeof configRes?.config?.organizationId === "number"
      ? configRes.config.organizationId
      : null;
  ...
  const { data: auditRes } = useSWR(
    eventId
      ? orgApi(`/padel/audit?eventId=${eventId}&limit=25&actionPrefix=PADEL_`)
      : null,
    fetcher,
  );
  const pairings: Pairing[] = pairingsRes?.pairings ?? [];
  const matches: Match[] = Array.isArray(matchesRes?.items)
    ? (matchesRes.items as Match[])
    : emptyMatches;
  const standings: Standings =
    standingsRes?.groups ?? standingsRes?.standings ?? {};
  ...
  const pairingCountByCategory = useMemo(() => {
    ...
    pairings.forEach((pairing) => {
      const categoryId =
        typeof pairing.categoryId === "number" &&
        Number.isFinite(pairing.categoryId)
          ? pairing.categoryId
          : null;
  ...
  const planningStrategy =
    planningMode === "capacity" ? "capacity-first" : "runtime-first";
  const pairingsById = useMemo(
    () => new Map(pairings.map((pairing) => [pairing.id, pairing])),
    [pairings],
  );
  const swapCandidates = useMemo(
    () =>
      pairings.filter((pairing) => {
        ...
        const partnerSlot = pairing.slots.find(
          (slot) => slot.slotRole === "PARTNER",
        );
  ...
  const standingsGroups = useMemo(() => {
    ...
    return entries.sort((a, b) =>
      a[0].localeCompare(b[0], "pt-PT", { numeric: true }),
    );
  }, [standings]);
  ...
  const ruleSets = Array.isArray(ruleSetsRes?.items)
    ...
    : [];
  const activeRuleSet = configRes?.config?.ruleSet as
    | PadelRuleSetSummary
    | undefined;
  const auditItems: AuditItem[] =
    auditRes && auditRes.ok && Array.isArray(auditRes.items)
      ? (auditRes.items as AuditItem[])
      : [];
  const memberRole = orgMeRes?.membershipRole ?? null;
  const normalizedRole =
    typeof memberRole === "string" ? memberRole.toUpperCase() : null;
  const isAdminRole =
    normalizedRole === "OWNER" ||
    normalizedRole === "CO_OWNER" ||
    normalizedRole === "ADMIN";
  const isOwnerRole =
    normalizedRole === "OWNER" || normalizedRole === "CO_OWNER";
  ...
  const autoGeneratedAt = configRes?.tournament?.generatedAt as
    | string
    | undefined;
  const autoGeneratedBy = configRes?.tournament?.generatedByUserId as
    | string
    | null
    | undefined;
  const waitlistEnabled = advanced.waitlistEnabled === true;
  const registrationStartsAt =
    typeof advanced.registrationStartsAt === "string"
      ? advanced.registrationStartsAt
      : null;
  const registrationEndsAt =
    typeof advanced.registrationEndsAt === "string"
      ? advanced.registrationEndsAt
      : null;
  const allowSecondCategory = advanced.allowSecondCategory !== false;
  const maxEntriesTotal =
    typeof advanced.maxEntriesTotal === "number" &&
    Number.isFinite(advanced.maxEntriesTotal)
      ? Math.floor(advanced.maxEntriesTotal)
      : null;
  const tvMonitor =
    (advanced.tvMonitor as
      | { footerText?: string | null; sponsors?: string[] }
      | undefined) ?? {};
  const [tvFooterText, setTvFooterText] = useState(tvMonitor.footerText ?? "");
  const [tvSponsors, setTvSponsors] = useState(
    (tvMonitor.sponsors ?? []).join("\n"),
  );
  ...
  const koManualAt =
    typeof advanced.koManualAt === "string" ? advanced.koManualAt : null;
  const competitionState =
    typeof advanced.competitionState === "string"
      ? advanced.competitionState
      : null;
  const seedRanks =
    (advanced.seedRanks as Record<string, number> | undefined) ?? {};
  const scoreRulesByCategoryRaw =
    advanced.scoreRulesByCategory &&
    typeof advanced.scoreRulesByCategory === "object"
      ? (advanced.scoreRulesByCategory as Record<string, unknown>)
      : {};
  const scoreRulesByCategory = useMemo(
    () =>
      Object.entries(scoreRulesByCategoryRaw).reduce<
        Record<string, PadelScoreRules>
      >((acc, [key, value]) => {
  ...
  const selectedScoreRulesOverride = selectedCategoryId
    ? (scoreRulesByCategory[String(selectedCategoryId)] ?? null)
    : null;
  ...
  const scoreRulesPreset = useMemo(
    () => resolveScoreRulesPresetId(scoreRules),
    [scoreRules],
  );
  const activeScorePreset =
    scoreRulesPreset === "CUSTOM"
      ? null
      : (PADEL_SCORE_RULE_PRESETS.find(
          (preset) => preset.id === scoreRulesPreset,
        ) ?? null);
  const resultValidationMode =
    configRes?.config?.resultValidationMode ===
  ...
  const pendingConfirmationWindowMinutes =
    ...
    Number.isFinite(configRes.config.pendingConfirmationWindowMinutes)
      ? Math.max(
          1,
          Math.floor(configRes.config.pendingConfirmationWindowMinutes),
        )
      : 15;
  const playerResultSubmissionEnabled =
    configRes?.config?.playerResultSubmissionEnabled === true;
  ...
  const selectedCategoryLabel = selectedCategoryId
    ? (categoryOptions.find((item) => item.id === selectedCategoryId)?.label ??
      `Categoria #${selectedCategoryId}`)
    : "Global";
  const runtimeCategoryKey = selectedCategoryId
    ? String(selectedCategoryId)
    : "global";
  const formatProfilesByCategoryRaw =
    advanced.formatProfilesByCategory &&
    typeof advanced.formatProfilesByCategory === "object"
      ? (advanced.formatProfilesByCategory as Record<string, unknown>)
      : {};
  const formatProfilesByCategory = useMemo(
    () =>
      Object.entries(formatProfilesByCategoryRaw).reduce<
  ...
  const selectedCategoryProfileOwn =
    formatProfilesByCategory[runtimeCategoryKey] ?? null;
  ...
  const generationFormatBaseRaw =
    formatRequested ||
    formatEffective ||
    configRes?.config?.format ||
    DEFAULT_PADEL_FORMAT_FALLBACK;
  const generationFormatBase =
    parsePadelFormat(generationFormatBaseRaw) ?? DEFAULT_PADEL_FORMAT_FALLBACK;
  const generationFormatProfile = parsePadelFormat(
  ...
  const selectedNonStopRounds =
    toPositiveInt(selectedCategoryProfile?.nonStopRounds) ??
    toPositiveInt(selectedCategoryProfile?.roundsHint) ??
    6;
  const supportsGroups = generationFormat === "GRUPOS_ELIMINATORIAS";
  const supportsKnockout = [
    "GRUPOS_ELIMINATORIAS",
    "QUADRO_ELIMINATORIO",
    "QUADRO_AB",
    "DUPLA_ELIMINACAO",
  ].includes(generationFormat);
  const isKnockoutOnlyFormat = [
    "QUADRO_ELIMINATORIO",
    "QUADRO_AB",
    "DUPLA_ELIMINACAO",
  ].includes(generationFormat);
  const isLeagueFormat = ["TODOS_CONTRA_TODOS", "CAMPEONATO_LIGA"].includes(
    generationFormat,
  );
  const isNonStopFormat = generationFormat === "NON_STOP";
  const isAmMxFormat =
    generationFormat === "AMERICANO" || generationFormat === "MEXICANO";
  const scheduledMatchesCount = matches.filter((match) =>
    Boolean(match.plannedStartAt || match.startTime),
  ).length;
  const unscheduledMatchesCount = Math.max(
    0,
    matches.length - scheduledMatchesCount,
  );
  const autoScheduleBaseHref = resolvedOrganizationId
  ...
  const primaryRoundMatches = useMemo(
    () => matches.filter((match) => match.roundType === "GROUPS"),
    [matches],
  );
  const nonStopRuntimeByCategory =
    advanced.nonStopRuntimeByCategory &&
    typeof advanced.nonStopRuntimeByCategory === "object"
      ? (advanced.nonStopRuntimeByCategory as Record<string, unknown>)
      : {};
  const amMxRuntimeByCategory =
    advanced.amMxRuntimeByCategory &&
    typeof advanced.amMxRuntimeByCategory === "object"
      ? (advanced.amMxRuntimeByCategory as Record<string, unknown>)
      : {};
  const selectedNonStopRuntime =
    nonStopRuntimeByCategory[runtimeCategoryKey] &&
    typeof nonStopRuntimeByCategory[runtimeCategoryKey] === "object"
      ? (nonStopRuntimeByCategory[runtimeCategoryKey] as Record<
          string,
          unknown
        >)
      : nonStopRuntimeByCategory.global &&
          typeof nonStopRuntimeByCategory.global === "object"
        ? (nonStopRuntimeByCategory.global as Record<string, unknown>)
        : null;
  const selectedAmMxRuntime =
    amMxRuntimeByCategory[runtimeCategoryKey] &&
    typeof amMxRuntimeByCategory[runtimeCategoryKey] === "object"
      ? (amMxRuntimeByCategory[runtimeCategoryKey] as Record<string, unknown>)
      : amMxRuntimeByCategory.global &&
          typeof amMxRuntimeByCategory.global === "object"
        ? (amMxRuntimeByCategory.global as Record<string, unknown>)
        : null;
  const nonStopRuntimeQueue = useMemo(() => {
    if (!selectedNonStopRuntime || !Array.isArray(selectedNonStopRuntime.queue))
      return [];
  ...
  const amMxRoundsGenerated = toPositiveInt(
  ...
  const amMxProgressionMode =
    selectedCategoryProfile?.amMxProgressionMode === "ROUND_BY_ROUND"
      ? "ROUND_BY_ROUND"
      : null;
  ...
  const filteredPrimaryRoundMatches = useMemo(
    () =>
      primaryRoundMatches.filter((match) =>
        doesMatchPassLiveOpsFilter(match, liveOpsFilter),
      ),
  ...
  const liveOpsCounters = useMemo(() => {
    ...
    matches.forEach((match) => {
      if (doesMatchPassLiveOpsFilter(match, "ACTION_REQUIRED"))
        actionRequired += 1;
      if (doesMatchPassLiveOpsFilter(match, "PENDING_CONFIRMATION"))
        pendingConfirmation += 1;
      if (doesMatchPassLiveOpsFilter(match, "PENDING_REVIEW_EXPIRED"))
        pendingReviewExpired += 1;
  ...
  const generationPlanAlternatives = useMemo(() => {
    if (
      !generationPlanDetails?.alternatives ||
      generationPlanDetails.alternatives.length === 0
    )
      return [];
  ...
  const planningPreviewAlternatives = useMemo(() => {
    if (
      !planningPreview?.alternatives ||
      planningPreview.alternatives.length === 0
    )
      return [];
  ...
  useEffect(() => {
    ...
    const targetCategoryIds = selectedCategoryId
      ? [selectedCategoryId]
    ...
    const categoriesPayload = targetCategoryIds
      .map((categoryId) => {
        ...
        const profileFormat =
          ...
          PADEL_FORMAT_PROFILE_OPTIONS.includes(
            profile.format as (typeof PADEL_FORMAT_PROFILE_OPTIONS)[number],
          )
            ? profile.format
            : null;
        const linkFormat =
          ...
          PADEL_FORMAT_PROFILE_OPTIONS.includes(
            categoryLink.format as (typeof PADEL_FORMAT_PROFILE_OPTIONS)[number],
          )
            ? categoryLink.format
            : null;
        ...
        const nonStopMode =
          profile?.nonStopMode === "HARD_CAP_WAITLIST"
            ...
            : "ACTIVE_QUEUE";
        const nonStopRounds =
          toPositiveInt(profile?.nonStopRounds) ??
          toPositiveInt(profile?.roundsHint) ??
          6;
        return {
          categoryId,
          label:
            categoryLabelById.get(String(categoryId)) ??
            categoryLink?.category?.label ??
            `Categoria #${categoryId}`,
          teams,
          format: formatValue,
          amMxMode: isAmMxFormatValue(formatValue) ? amMxMode : undefined,
          amMxProgressionMode: isAmMxFormatValue(formatValue)
            ? amMxProgressionMode
            : undefined,
          nonStopMode: isNonStopFormatValue(formatValue)
            ? nonStopMode
            : undefined,
          nonStopRounds: isNonStopFormatValue(formatValue)
            ? nonStopRounds
            : undefined,
        };
      })
    ...
    const timeout = window.setTimeout(async () => {
      ...
      try {
        const res = await fetch("/api/padel/formats/plan", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ eventId, categories: categoriesPayload }),
        ...
        if (!res.ok || json?.ok === false) {
          if (!controller.signal.aborted) {
            ...
            setPlanningPreviewError(
              sanitizeUiErrorMessage(
                json?.error,
                "Planner de formatos indisponível.",
              ),
            );
          }
          return;
        ...
        setPlanningPreview(null);
        setPlanningPreviewError(
          "Erro ao calcular pré-viabilidade por formato.",
        );
      } finally {
  ...
  const confirmedPairings = filteredPairings.filter(
    (p) =>
      p.pairingStatus === "COMPLETE" &&
      (p.lifecycleStatus === "CONFIRMED_BOTH_PAID" ||
        p.lifecycleStatus === "CONFIRMED_CAPTAIN_FULL"),
  );
  ...
  const koRounds = useMemo(() => {
    const winnerFromScore = (match: Match): "A" | "B" | null => {
      ...
      const score = (match.score || {}) as Record<string, unknown>;
      if (score.winnerSide === "A" || score.winnerSide === "B")
    ...
    matches
      ...
      .forEach((m) => {
        ...
        rounds.get(key)!.push({
          ...
          teamB: m.pairingB
            ? (pairingNameById.get(m.pairingB?.id ?? 0) ?? "—")
            : "BYE",
          status: m.status,
          score,
    ...
    const roundSortKey = (label: string) => {
      const prefix = label.startsWith("A")
        ? "A"
        : label.startsWith("B")
          ? "B"
          : "";
    ...
    return Array.from(rounds.entries()).sort((a, b) => {
      const aMeta = roundSortKey(a[0]);
      const bMeta = roundSortKey(b[0]);
      const prefixOrder = (value: string) =>
        value === "A" ? 0 : value === "B" ? 1 : 0;
      if (prefixOrder(aMeta.prefix) !== prefixOrder(bMeta.prefix)) {
        return prefixOrder(aMeta.prefix) - prefixOrder(bMeta.prefix);
      }
      const aOrder =
        aMeta.order ??
        (aMeta.size !== null ? -aMeta.size : Number.MAX_SAFE_INTEGER - 1);
      const bOrder =
        bMeta.order ??
        (bMeta.size !== null ? -bMeta.size : Number.MAX_SAFE_INTEGER - 1);
  ...
  const koVisibleMatchesCount = useMemo(() => {
    ...
    return koRounds.reduce((total, [, games]) => {
      const visible = games.filter((game) => {
        ...
        return fullMatch
          ? doesMatchPassLiveOpsFilter(fullMatch, liveOpsFilter)
          : false;
      });
      return total + visible.length;
    }, 0);
  }, [koRounds, liveOpsFilter, matchById]);
  const groupMatchesCount = matches.filter(
    (m) => m.roundType === "GROUPS",
  ).length;
  const groupMatchesDone = matches.filter(
    (m) =>
      m.roundType === "GROUPS" &&
      ["OFFICIAL", "WALKOVER", "RETIRED"].includes(m.status),
  ).length;
  const groupMissing = Math.max(0, groupMatchesCount - groupMatchesDone);
  const canGenerateGroups = isAdminRole && supportsGroups;
  const canGeneratePrimaryRound =
    isAdminRole &&
    (supportsGroups || isLeagueFormat || isNonStopFormat || isAmMxFormat);
  const canGenerateKnockout =
    isAdminRole && supportsKnockout && (groupMissing === 0 || isOwnerRole);
  const [resultDrafts, setResultDrafts] = useState<
    ...
    >
  >({});
  const [manualFallbackEnabled, setManualFallbackEnabled] = useState<
  ...
  const getTimedGames = (m: Match) => {
    ...
    return {
      gamesA:
        typeof gamesA === "number" || typeof gamesA === "string"
          ? String(gamesA)
          : "",
      gamesB:
        typeof gamesB === "number" || typeof gamesB === "string"
          ? String(gamesB)
          : "",
      allowTimedDraw:
        typeof score.allowDraw === "boolean" ? score.allowDraw : true,
  ...
  useEffect(() => {
    setResultDrafts((prev) => {
      ...
      matches.forEach((m) => {
        ...
        const shouldRefresh =
          !next[m.id] || (!next[m.id].saving && !next[m.id].uploading);
        if (shouldRefresh) {
  ...
  useEffect(() => {
    ...
    setManualFallbackEnabled((prev) => {
      ...
      if (
        ...
        prevKeys.every(
          (key, idx) =>
            key === nextKeys[idx] && prev[Number(key)] === next[Number(key)],
        )
      ) {
        return prev;
      }
  ...
  const resolvedGroupCount = useMemo(() => {
    ...
    if (Number.isFinite(groupCountRaw) && groupCountRaw > 0)
      return Math.floor(groupCountRaw);
  ...
  const groupLabels = useMemo(
    () =>
      Array.from({ length: Math.min(26, resolvedGroupCount) }, (_, idx) =>
  ...
  const getDisputeInfo = (match: Match) => {
    const score = (match.score || {}) as Record<string, unknown>;
    const rawStatus =
      typeof score.disputeStatus === "string" ? score.disputeStatus : null;
    const status =
      rawStatus === "OPEN" || rawStatus === "RESOLVED" ? rawStatus : null;
    const reason =
      typeof score.disputeReason === "string" ? score.disputeReason : null;
    const resolutionNote =
      typeof score.disputeResolutionNote === "string"
        ? score.disputeResolutionNote
        : null;
  ...
  const getLiveWorkflowInfo = (match: Match) => {
    ...
    const workflow =
      score.liveWorkflow &&
      typeof score.liveWorkflow === "object" &&
      !Array.isArray(score.liveWorkflow)
        ? (score.liveWorkflow as Record<string, unknown>)
        : null;
    const pendingReviewExpiredAt =
      workflow && typeof workflow.pendingReviewExpiredAt === "string"
        ? workflow.pendingReviewExpiredAt
        : null;
    const pendingConfirmationExpiresAt =
      workflow && typeof workflow.pendingConfirmationExpiresAt === "string"
        ...
        : null;
    return { pendingReviewExpiredAt, pendingConfirmationExpiresAt };
  };
    ...
    switch (code) {
      ...
      default:
        return sanitizeUiErrorMessage(
          value,
          "Não foi possível gerar os jogos.",
        );
    }
  };
  ...
  const championName = useMemo(() => {
    ...
    const resolveWinner = (
      round?:
        | [
            string,
            Array<{ teamA: string; teamB: string; winner: "A" | "B" | null }>,
          ]
        | null,
    ) => {
  ...
  async function saveScoreRules(
    nextRules: PadelScoreRules,
    scope: "selected" | "global" = "selected",
  ) {
    const organizationId = configRes?.config?.organizationId;
    const format = generationFormatBase;
    if (!organizationId || !eventId) return;
    const shouldUseCategoryScope =
      scope === "selected" && selectedCategoryId !== null;
    setConfigMessage(null);
    const payload = shouldUseCategoryScope
      ? {
          ...
          eventId,
          format,
          scoreRulesByCategory: { [String(selectedCategoryId)]: nextRules },
        }
      : { organizationId, eventId, format, scoreRules: nextRules };
    const res = await fetch(`/api/padel/tournaments/config`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  async function saveFormatProfileConfig(
    ...
    const targetKey =
      scope === "global" || selectedCategoryId === null
        ? "global"
        : String(selectedCategoryId);
    ...
    const requestedFormat =
      typeof patch.format === "string" ? patch.format : nextProfile.format;
    const isKnownRequestedFormat =
      typeof requestedFormat === "string" &&
      (PADEL_FORMAT_PROFILE_OPTIONS as readonly string[]).includes(
        requestedFormat,
      );
    const nextFormat = isKnownRequestedFormat
      ? requestedFormat
      : generationFormatBase;
    ...
    if (isNonStopFormatValue(nextFormat)) {
      ...
      const roundsSource =
        patch.nonStopRounds !== undefined
          ? patch.nonStopRounds
          : (toPositiveInt(nextProfile.nonStopRounds) ??
            toPositiveInt(nextProfile.roundsHint) ??
  ...
  async function generateMatches(phase: "GROUPS" | "KNOCKOUT") {
    ...
    if (phase === "GROUPS" && !canGeneratePrimaryRound) {
      setGenerationPhase(phase);
      setGenerationError(
        `Formato atual: ${formatLabel(generationFormat)}. Usa outro fluxo de geração.`,
      );
      return;
    }
    ...
    const needsOverride =
      phase === "KNOCKOUT" && supportsGroups && groupMissing > 0;
    if (needsOverride) {
      if (!isOwnerRole) {
        setGenerationPhase(phase);
        setGenerationError(
          "Só Dono/Co-dono pode forçar eliminatórias com grupos incompletos.",
        );
        return;
      }
    ...
    try {
      ...
      if (json?.stage === "GROUPS") {
        ...
        const count = Number.isFinite(json?.matches) ? json.matches : null;
        const qualifiers = Number.isFinite(json?.qualifiers)
          ? json.qualifiers
          : null;
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (Array.isArray(data?.errors))
          setImportErrors(data.errors as ImportErrorItem[]);
        const message =
  ...
  const updateKoDraft = (
    match: Match,
    update: Partial<{ pairingAId: number | null; pairingBId: number | null }>,
  ) => {
    const current = getKoDraft(match);
    setKoEdits((prev) => ({ ...prev, [match.id]: { ...current, ...update } }));
  };
  ...
  const hasKoDraftChanges = (
    match: Match,
    draft: { pairingAId: number | null; pairingBId: number | null },
  ) => {
  ...
  async function saveTvMonitorSettings() {
    ...
    const res = await fetch(`/api/padel/tournaments/config`, {
      ...
      body: JSON.stringify({
        ...
        eventId,
        format,
        tvMonitor: { footerText: tvFooterText.trim() || null, sponsors },
      }),
    });
    if (res.ok) {
      mutateConfig();
  ...
  async function handleSwapPairings() {
    ...
    try {
      const res = await fetch(
        `/api/org/${resolvedOrgIdForTournamentActions}/tournaments/pairings/swap`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  async function sendBroadcast() {
    ...
    try {
      const res = await fetch(
        `/api/org/${resolvedOrgIdForTournamentActions}/tournaments/broadcast`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      if (res.ok) {
        setBroadcastResult(
          `Mensagem enviada a ${data?.recipients ?? 0} utilizadores.`,
        );
        setBroadcastMessage("");
        setBroadcastTitle("");
  ...
  async function savePolicy(update: {
    allowSecondCategory?: boolean | null;
  ...
  async function generateSeedsFromRanking() {
    ...
    try {
      const res = await fetch("/api/padel/tournaments/seeds", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ eventId, categoryId: selectedCategoryId }),
      });
      const json = await res.json().catch(() => null);
      if (!res.ok || json?.ok === false) {
        flashConfigMessage(
  ...
  async function submitResult(matchId: number) {
    ...
    if (resultType === "NORMAL") {
      if (timedMode) {
        if (
          !Number.isFinite(gamesA) ||
          !Number.isFinite(gamesB) ||
          gamesA < 0 ||
          gamesB < 0
        ) {
          updateResultDraft(matchId, {
            error: "Indica jogos válidos (ex: 6 vs 4).",
          });
          return;
        }
        if (!draft.allowTimedDraw && gamesA === gamesB) {
          updateResultDraft(matchId, {
            error: "Empate não permitido neste modo.",
          });
          return;
        }
      } else if (sets.length === 0) {
        updateResultDraft(matchId, {
          error: "Indica o resultado (ex: 6-3, 6-4).",
        });
        return;
      }
    ...
    const timedWinnerSide =
      Number.isFinite(gamesA) && Number.isFinite(gamesB) && gamesA !== gamesB
        ? gamesA > gamesB
          ? "A"
          : "B"
        : null;
    ...
    const res = isSpecialResult
      ...
      : await fetch(`/api/padel/matches/${matchId}/result/submit`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ score, clientRequestId }),
        });
    if (!res.ok) {
      const data = await res.json().catch(() => null);
      const error = resolveResultSaveError(
  ...
  async function runResultWorkflowAction(
    matchId: number,
    path: string,
    ...
    try {
      ...
      if (!res.ok || json?.ok === false) {
        setWorkflowError((prev) => ({
          ...prev,
          [matchId]: sanitizeUiErrorMessage(
            json?.error,
            "Erro no workflow de resultado.",
          ),
        }));
        return false;
      }
      ...
      setWorkflowError((prev) => ({
        ...prev,
        [matchId]: "Erro no workflow de resultado.",
      }));
      return false;
    } finally {
  ...
  async function rejectResult(matchId: number) {
    ...
    await runResultWorkflowAction(matchId, "reject", {
  ...
  async function overrideResult(matchId: number) {
    ...
    if (!reasonText.trim()) return;
    const evidence =
      window.prompt("Anexo/evidência (URL ou referência) obrigatório") ?? "";
  ...
  async function savePartialScore(matchId: number) {
    ...
    if (
      ...
      updateResultDraft(matchId, {
        error: "Indica parcial válido (jogos A/B).",
      });
      return;
    }
    if (!timedMode && sets.length === 0) {
      updateResultDraft(matchId, {
        error: "Indica parcial de sets (ex: 4-3).",
      });
      return;
    }
    updateResultDraft(matchId, { saving: true, error: null });
  ...
  async function resolveMatchDispute(matchId: number) {
    if (!isAdminRole) {
      setDisputeError((prev) => ({
        ...prev,
        [matchId]: "Apenas ADMIN pode resolver.",
      }));
      return;
    }
    ...
    try {
      ...
      setDisputeError((prev) => ({
        ...prev,
        [matchId]: "Erro ao resolver disputa.",
      }));
    } finally {
  ...
  async function uploadResultPhoto(matchId: number, file: File) {
    ...
    try {
      ...
      const res = await fetch(
        `/api/upload?scope=padel-match&organizationId=${organizationId}`,
  ...
  const renderResultControls = (m: Match) => {
    ...
    const pendingExpiredMinutes =
      pendingReviewExpiredAt && !Number.isNaN(pendingReviewExpiredAt.getTime())
        ? Math.max(
            0,
            Math.floor((Date.now() - pendingReviewExpiredAt.getTime()) / 60000),
          )
        : null;
    const lockedByWorkflow =
      m.status === "PENDING_CONFIRMATION" ||
      m.status === "PENDING_REVIEW_EXPIRED";
    const lockedByDispute =
      (disputeOpen || m.status === "DISPUTED") && !isAdminRole;
    const fallbackEnabled = manualFallbackEnabled[m.id] === true;
    const lockInputs =
      (lockedByWorkflow || lockedByDispute) && !fallbackEnabled;
    ...
    const showTimedInputs =
      draft.resultType === "NORMAL" && draft.scoreMode === "TIMED_GAMES";
    const showSetsInputs =
      draft.resultType === "NORMAL" && draft.scoreMode === "SETS";
    return (
      <div className="space-y-2 text-[12px]">
        {m.status === "PENDING_CONFIRMATION" && (
          <div className="rounded-lg border border-sky-300/35 bg-sky-500/10 px-3 py-2 text-[11px] text-sky-100 space-y-2">
            ...
            <p className="text-sky-100/80">
              Progressão bloqueada até confirmar/rejeitar.
            </p>
        ...
        {m.status === "PENDING_REVIEW_EXPIRED" && (
          <div className="rounded-lg border border-rose-300/35 bg-rose-500/10 px-3 py-2 text-[11px] text-rose-100 space-y-2">
            <p className="font-semibold">
              Pendente expirado · fila operacional
            </p>
        ...
        {m.status === "RESULT_SUBMITTED" && (
          <div className="rounded-lg border border-indigo-300/35 bg-indigo-500/10 px-3 py-2 text-[11px] text-indigo-100 space-y-2">
            ...
            <p className="text-indigo-100/80">
              Confirma para oficial ou ajusta o score antes de fechar.
            </p>
            <div className="flex flex-wrap items-center gap-2">
              <button
                type="button"
        ...
        {disputeOpen && (
          <div className="rounded-lg border border-amber-400/40 bg-amber-500/10 px-3 py-2 text-[11px] text-amber-100 space-y-1">
            ...
            {dispute.reason && (
              <p className="text-amber-100/80">Motivo: {dispute.reason}</p>
            )}
            <div className="flex flex-wrap items-center gap-2">
              {isAdminRole ? (
                <button
                ...
                <span className="text-amber-100/70">
                  Apenas ADMIN pode resolver.
                </span>
              )}
              {disputeMsg && (
                <span className="text-amber-200">{disputeMsg}</span>
              )}
            </div>
          </div>
        )}
        ...
        {(lockedByWorkflow || lockedByDispute) && isAdminRole && (
          <div className="rounded-lg border border-white/15 bg-black/25 px-3 py-2 text-[11px] text-white/80 space-y-2">
            ...
            <p className="text-white/65">
              Em incidentes de operação podes desbloquear inputs para corrigir
              score e voltar a submeter.
            </p>
            <button
              type="button"
              onClick={() =>
          ...
          {draft.resultType === "NORMAL" && (
            <select
              ...
              onChange={(e) =>
                updateResultDraft(m.id, {
                  scoreMode:
                    e.target.value === "TIMED_GAMES" ? "TIMED_GAMES" : "SETS",
                })
              }
              disabled={lockInputs}
              className="rounded-lg border border-white/15 bg-black/30 px-2 py-1 disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-55 outline-none transition focus:border-white/35 focus-visible:ring-2 focus-visible:ring-white/25"
        ...
        {showTimedInputs && (
          <div className="grid gap-2 sm:grid-cols-2">
            <input
              ...
              value={draft.gamesA}
              onChange={(e) =>
                updateResultDraft(m.id, { gamesA: e.target.value })
              }
              disabled={lockInputs}
            ...
            <input
              ...
              value={draft.gamesB}
              onChange={(e) =>
                updateResultDraft(m.id, { gamesB: e.target.value })
              }
              disabled={lockInputs}
            ...
            <label className="flex items-center gap-2 rounded-lg border border-white/15 bg-black/20 px-2 py-1 text-[11px] text-white/75">
              <input
                ...
                onChange={(e) =>
                  updateResultDraft(m.id, { allowTimedDraw: e.target.checked })
                }
                disabled={lockInputs}
              />
              Permitir empate
            ...
            <label className="flex items-center gap-2 rounded-lg border border-white/15 bg-black/20 px-2 py-1 text-[11px] text-white/75">
              <input
                ...
                onChange={(e) =>
                  updateResultDraft(m.id, { endedByBuzzer: e.target.checked })
                }
                disabled={lockInputs}
              />
              Terminou no buzzer
            ...
            <div className="sm:col-span-2 flex flex-wrap items-center gap-2 text-[11px] text-white/70">
              ...
              {[
                ...
                <button
                  ...
                  onClick={() =>
                    updateResultDraft(m.id, {
                      gamesA: preset.gamesA,
                      gamesB: preset.gamesB,
                    })
                  }
                  disabled={lockInputs}
                  className="rounded-full border border-white/20 px-2 py-0.5 text-[11px] text-white/80 hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/35 disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-55"
        ...
        {draft.resultType !== "NORMAL" && (
          <select
            ...
            onChange={(e) =>
              updateResultDraft(m.id, {
                winnerSide: e.target.value as "" | "A" | "B",
              })
            }
            disabled={lockInputs}
            className="w-full rounded-lg border border-white/15 bg-black/30 px-2 py-1 disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-55 outline-none transition focus:border-white/35 focus-visible:ring-2 focus-visible:ring-white/25"
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {draft.error && (
            <span className="text-[11px] text-amber-200">{draft.error}</span>
          )}
          {actionError && (
            <span className="text-[11px] text-amber-200">{actionError}</span>
          )}
        </div>
      </div>
    );
  ...
  const renderGenerationPlanPanel = () => {
    ...
    const blockingReasons = Array.isArray(generationPlanDetails.blockingReasons)
      ...
      : [];
    const warnings = Array.isArray(generationPlanDetails.warnings)
      ? generationPlanDetails.warnings
      : [];
    ...
    const unscheduledMatches = Number(
      generationPlanDetails.unscheduledMatches ?? 0,
    );
    return (
      <div className="rounded-xl border border-amber-300/35 bg-amber-500/10 p-3 text-[12px] text-amber-100 space-y-2">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <span className="text-[11px] text-amber-200/85">
            Slots {totalSlots} · Jogos {matchesNeeded} · Em falta
            {Math.max(0, unscheduledMatches)}
          </span>
        </div>
        {categories.length > 0 && (
          <div className="space-y-1 text-[11px] text-amber-100/90">
            {categories.slice(0, 4).map((category) => (
              <p key={`plan-${category.key ?? category.label ?? "cat"}`}>
                • {category.label || "Categoria"}: {category.teams ?? 0} equipas
                · mínimo {category.minTeams ?? 0} · jogos
                {category.matchesNeeded ?? 0}/{category.allocatedSlots ?? 0}
                slots
                {typeof category.recommendedMaxTeams === "number"
                  ? ` · recomendado ${category.recommendedMaxTeams}`
                  : ""}
                {typeof category.hardCapMax === "number"
                  ? ` · hard cap ${category.hardCapMax}`
                  : ""}
              </p>
            ))}
          </div>
        ...
        {blockingReasons.length > 0 && (
          <p className="text-[11px] text-amber-100/90">
            Bloqueios: {blockingReasons.join(" ·")}
          </p>
        )}
        {warnings.length > 0 && (
          <p className="text-[11px] text-amber-100/90">
            Avisos: {warnings.slice(0, 2).join(" ·")}
          </p>
        )}
  ...
  const renderPlanningPreviewPanel = () => {
    ...
    const categories = Array.isArray(planningPreview.categories)
      ? planningPreview.categories
      : [];
    const blockingReasons = Array.isArray(planningPreview.blockingReasons)
      ? planningPreview.blockingReasons
      : [];
    const warnings = Array.isArray(planningPreview.warnings)
      ? planningPreview.warnings
      : [];
    ...
    return (
      <div
        className={`rounded-xl border p-3 text-[12px] space-y-2 ${planningPreview.feasible ? "border-emerald-300/35 bg-emerald-500/10 text-emerald-100" : "border-amber-300/35 bg-amber-500/10 text-amber-100"}`}
      >
        <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        {categories.length > 0 && (
          <div className="space-y-1 text-[11px]">
            {categories.slice(0, 4).map((category) => (
              <p
                key={`preview-plan-${category.key ?? category.label ?? "cat"}`}
              >
                • {category.label || "Categoria"}: {category.teams ?? 0} equipas
                · mínimo {category.minTeams ?? 0} · jogos
                {category.matchesNeeded ?? 0}/{category.allocatedSlots ?? 0}
        ...
        {(warnings.length > 0 || blockingReasons.length > 0) && (
          <p className="text-[11px] opacity-90">
            {warnings.length > 0
              ...
              : ""}
            {blockingReasons.length > 0
              ? `Bloqueios técnicos: ${blockingReasons.join(" ·")}.`
              : ""}
          </p>
        )}
      </div>
  ...
  const surfaceTabs = [
    {
      ...
      label: "Duplas",
      active: tab === "duplas",
      onClick: () => setTab("duplas" as const),
    },
    ...(showGroupsTab
      ? [
          {
            ...
            active: tab === "grupos",
            onClick: () => setTab("grupos" as const),
          },
        ]
      : []),
    ...(showKnockoutTab
      ? [
          {
            id: "tab-eliminatorias",
            label: "Eliminatórias",
            active: tab === "eliminatorias",
            onClick: () => setTab("eliminatorias" as const),
          },
        ]
      : []),
  ];
        ...
        >
          <div className={`${SURFACE_CARD_CLASS} space-y-4`}>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        saveFormatProfileConfig(
                          {
                            nonStopMode:
                              e.target.value === "HARD_CAP_WAITLIST"
                                ? "HARD_CAP_WAITLIST"
                                : "ACTIVE_QUEUE",
                          },
                          "selected",
            ...
            <div className="rounded-lg border border-white/10 bg-black/20 px-2 py-2 text-[11px] text-white/65">
              {selectedCategoryId ? "Override ativo." : "Perfil global."}
            </div>
          </div>
          ...
          <div className={`${SURFACE_CARD_CLASS} space-y-3`}>
            <div className="flex items-center justify-between gap-3">
              ...
              <span className="text-[11px] text-white/50">
                {auditItems.length} eventos
              </span>
            </div>
            {auditItems.length === 0 && (
              <p className="text-white/60">Sem eventos de auditoria.</p>
            )}
            {auditItems.length > 0 && (
          ...
          >
            ...
            <a
              href={orgApi(
                `/padel/exports/bracket?eventId=${eventId}&format=pdf`,
              )}
              target="_blank"
              rel="noreferrer"
              className="rounded-full border border-white/20 px-3 py-1 text-[11px] text-white/80 hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/35"
            ...
            <a
              href={orgApi(
                `/padel/exports/bracket?eventId=${eventId}&format=html`,
              )}
              target="_blank"
              rel="noreferrer"
              className="rounded-full border border-white/20 px-3 py-1 text-[11px] text-white/80 hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/35"
            ...
            <a
              href={orgApi(
                `/padel/exports/calendario?eventId=${eventId}&format=pdf`,
              )}
              target="_blank"
              rel="noreferrer"
              className="rounded-full border border-white/20 px-3 py-1 text-[11px] text-white/80 hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/35"
          ...
          {formatRequested &&
            ...
            formatRequested !== formatEffective && (
              <div className="rounded-xl border border-amber-400/30 bg-amber-500/10 p-3 text-[12px] text-amber-50">
                Pedido: {formatLabel(formatRequested)}. Em uso:{" "}
                {formatLabel(formatEffective)} (Beta).
              </div>
            )}
            ...
            <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4 text-[12px] text-white/80 flex items-center justify-between gap-3 flex-wrap">
              ...
              {supportsGroups && groupMissing > 0 && (
                <span className="rounded-full bg-amber-500/15 px-3 py-1 text-amber-100">
                  Faltam {groupMissing} jogo{groupMissing === 1 ? "" : "s"} para
              ...
              <div className="space-y-1">
                ...
                <div className="flex flex-wrap items-center gap-2 pt-1">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setPlanningMode(
                        e.target.value === "capacity" ? "capacity" : "runtime",
                      )
                    }
                ...
                <div className="flex flex-wrap gap-2 pt-1 text-[11px]">
                  ...
                  <span className="rounded-full border border-white/20 bg-black/30 px-2 py-1 text-white/80">
                    Calendário:{" "}
                    {matches.length > 0 ? `${scheduleCoverage}%` : "sem jogos"}
                  </span>
                  <span className="rounded-full border border-white/20 bg-black/30 px-2 py-1 text-white/80">
          ...
          {matches.length > 0 && (
            <div className={`${SURFACE_CARD_CLASS} space-y-2`}>
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <span className="text-[11px] text-white/60">
                  Ativo: {liveOpsFilterLabel}
                </span>
              </div>
              <div className="flex flex-wrap gap-2">
                {[
                  {
                    key: "ALL" as LiveOpsFilter,
                    label: "Todos",
                    count: liveOpsCounters.all,
                  },
                  {
                    key: "ACTION_REQUIRED" as LiveOpsFilter,
                    label: "Ação",
                    count: liveOpsCounters.actionRequired,
                  },
                  {
                  ...
                  {
                    key: "DISPUTED" as LiveOpsFilter,
                    label: "Disputa",
                    count: liveOpsCounters.disputed,
                  },
                  {
                    key: "UNSCHEDULED" as LiveOpsFilter,
                    label: "Sem horário",
                    count: liveOpsCounters.unscheduled,
                  },
                ].map((item) => (
                  <button
          ...
          {tab === "grupos" && (
            <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4 text-sm space-y-3">
                ...
                <div className="rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-[12px] text-white/80 space-y-2">
                  ...
                  {isNonStopFormat && (
                    <>
                      <p className="text-[11px] text-white/65">
                        ...
                        {selectedNonStopRounds
                          ? ` · ${selectedNonStopRounds} ronda(s)`
                          : ""}
                        {nonStopRuntimeQueue.length > 0
                          ? ` · fila ${nonStopRuntimeQueue.length}`
                          : ""}
                      </p>
                      {nonStopRuntimeActivePairs.length > 0 && (
                        <div className="space-y-1 text-[11px] text-white/70">
                          {nonStopRuntimeActivePairs
                            .slice(0, 6)
                            .map((entry) => (
                  ...
                  {isAmMxFormat && (
                    <p className="text-[11px] text-white/65">
                      Progressão:
                      {amMxProgressionMode === "ROUND_BY_ROUND"
                        ? "Dinâmica ronda a ronda"
                        : "Ronda fixa"}
                      {selectedAmMxMode === "FIXED_PAIR"
                        ? " · pares fixos"
                        : " · rotação individual"}
                    </p>
                  )}
              ...
              <div className="rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-[12px] text-white/80 space-y-2">
                ...
                {activeRuleSet?.tieBreakRules &&
                  activeRuleSet.tieBreakRules.length > 0 && (
                ...
                {activeRuleSet?.pointsTable && (
                  <p className="text-[11px] text-white/70">
                    Pontos: WIN {activeRuleSet.pointsTable.WIN ?? 3} / LOSS
                    {activeRuleSet.pointsTable.LOSS ?? 0}
                  </p>
                )}
              </div>
              <div className="rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-[12px] text-white/80 space-y-2">
                ...
                <div className="grid gap-2 sm:grid-cols-2">
                  <select
                    ...
                    onChange={(e) => {
                      const presetId = e.target
                        .value as typeof scoreRulesPreset;
                      const nextRules = buildScoreRulesFromPreset(
                        presetId,
                        scoreRules,
                        scoreRules.deuceMode,
                      );
                ...
                <p className="text-[11px] text-white/65">
                  Regra ativa: {effectiveScoreRuleSummary.label}.
                </p>
              ...
              <div className="rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-[12px] text-white/80 space-y-2">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-[11px] uppercase tracking-[0.16em] text-white/60">
                      Workflow de resultado live
                    </p>
                    <p className="text-[12px] text-white/70">
                      Submissão e validação do resultado.
                    </p>
                  </div>
                  ...
                  <input
                    ...
                    onBlur={(e) => {
                      ...
                      const nextValue = Math.max(
                        1,
                        Math.min(240, Math.floor(raw)),
                      );
                      if (nextValue === pendingConfirmationWindowMinutes)
                        return;
                      saveLiveWorkflowConfig({
                        pendingConfirmationWindowMinutes: nextValue,
                      });
                    }}
                  />
                </div>
                <p className="text-[11px] text-white/65">
                  Janela pendente: {pendingConfirmationWindowMinutes} min · modo
                  atual:
                ...
                <>
                    ...
                    <div className="inline-flex rounded-full border border-white/15 bg-black/40 p-1 text-[12px]">
                      {[
                        ...
                        <button
                          ...
                          onClick={() =>
                            saveGroupsConfig({
                              mode: opt.key as "AUTO" | "MANUAL",
                            })
                          }
                          className={`rounded-full px-3 py-1 transition ${groupMode === opt.key ? "bg-white text-black font-semibold shadow" : "text-white/75 hover:bg-white/5"}`}
                        >
                          {opt.label}
                        </button>
                      ))}
                  ...
                  {groupMode === "MANUAL" && (
                    <p className="text-[11px] text-white/60">
                      Manual: escolhe grupo no separador Duplas.
                    </p>
                  )}
              ...
              <div className="flex items-center justify-between gap-2 rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-[12px] text-white/70">
                ...
                <button
                  ...
                  className={`rounded-full px-3 py-1 text-[12px] font-semibold ${waitlistEnabled ? "bg-white text-black" : "border border-white/20 text-white/80"}`}
                >
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <div className="flex items-center justify-between gap-2 rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-[12px] text-white/70">
                    ...
                    <button
                      ...
                      onClick={() =>
                        savePolicy({
                          allowSecondCategory: !allowSecondCategory,
                        })
                      }
                      className={`rounded-full px-3 py-1 text-[12px] font-semibold ${allowSecondCategory ? "bg-white text-black" : "border border-white/20 text-white/80"}`}
                    >
          ...
          {tab === "duplas" && (
            <div className="space-y-2">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                    ...
                    {matchmakingQueue.length === 0 ? (
                      ...
                      <div className="mt-2 space-y-2">
                        {matchmakingQueue.slice(0, 5).map((pairing) => (
                          <div
                            key={`queue-${pairing.id}`}
                            className="flex items-center justify-between gap-2 text-[12px]"
                          >
                            <span className="font-semibold text-white">
                              {nameFromSlots(pairing, locale)}
                            </span>
                            <span className="text-white/60">
                              {resolvePairingStatusLabel(pairing, locale)} ·
                              {resolvePaymentModeLabel(
                                pairing.paymentMode,
                                locale,
                              )}
                            </span>
                        ...
                        {matchmakingQueue.length > 5 && (
                          <p className="text-[11px] text-white/50">
                            +{matchmakingQueue.length - 5} em fila
                          </p>
                        )}
                      </div>
                    )}
                  ...
                  <div className="rounded-lg border border-white/10 bg-black/30 px-3 py-2">
                    ...
                    {matchmakingFormed.length === 0 ? (
                      <p className="mt-2 text-[12px] text-white/60">
                        Sem duplas formadas ainda.
                      </p>
                    ) : (
                      <div className="mt-2 space-y-2">
                        {matchmakingFormed.slice(0, 5).map((pairing) => (
                          <div
                            key={`formed-${pairing.id}`}
                            className="flex items-center justify-between gap-2 text-[12px]"
                          >
                            <span className="font-semibold text-white">
                              {nameFromSlots(pairing, locale)}
                ...
                {importPreview?.categories && (
                  <p className="text-[11px] text-white/60">
                    ...
                    {Object.entries(importPreview.categories)
                      .map(
                        ([key, count]) =>
                          `${categoryLabelById.get(key) ?? `Categoria ${key}`}: ${count}`,
                      )
                      .join(" ·")}
                  </p>
                )}
              ...
              {filteredPairings.length === 0 && (
                <p className="text-sm text-white/70">Ainda não há duplas.</p>
              )}
                  ...
                  {waitlistItems.map((item) => (
                    ...
                    >
                      ...
                      <span className="text-white/60 shrink-0">
                        {resolveWaitlistStatusLabel(item.status, locale)}
                      </span>
                    </div>
                  ))}
                </div>
            ...
            <div className="space-y-3">
              {standingsGroups.length > 0 ? (
                        ...
                        {rows.map((row, index) => {
                          const pairing =
                            typeof row.pairingId === "number"
                              ? (pairingsById.get(row.pairingId) ?? null)
                              : null;
                          const setDiff = row.setsFor - row.setsAgainst;
                          return (
                            <div
                              key={`stand-${row.entityId}`}
                              className="flex items-center justify-between gap-2 text-[12px]"
                            >
                              ...
                              <div className="flex flex-wrap items-center gap-2 text-[10px] text-white/60">
                                ...
                                <span>
                                  Sets {row.setsFor}-{row.setsAgainst}
                                </span>
                                <span>
                ...
                <div className="rounded-xl border border-white/12 bg-white/5 p-3 text-[12px] text-white/70">
                  {`Sem classificações calculadas para ${groupsTabLabel.toLowerCase()}.`}
                </div>
              )}
              {primaryRoundMatches.length === 0 && (
                <p className="text-sm text-white/70">Sem jogos nesta fase.</p>
              )}
              {primaryRoundMatches.length > 0 &&
                filteredPrimaryRoundMatches.length === 0 && (
                  <p className="text-sm text-white/70">
                    Sem jogos para o filtro operacional selecionado.
                  </p>
                )}
              {filteredPrimaryRoundMatches.length > 0 &&
                filteredPrimaryRoundMatches.length <
                  primaryRoundMatches.length && (
                  <p className="text-[11px] text-white/60">
                    ...
                    <div className="flex items-center gap-2 min-w-0">
                      <span className="rounded-full border border-white/20 px-2.5 py-0.5 text-[11px] text-white/70">
                        {supportsGroups
                          ...
                          : m.roundLabel ||
                            (isNonStopFormat
                              ? `Ronda ${m.groupLabel || "NS"}`
                              : `Ronda ${m.groupLabel || "A"}`)}
                      </span>
                      <p className="font-semibold truncate">
                        {`${nameFromSlots(m.pairingA as Pairing, locale)} vs ${nameFromSlots(m.pairingB as Pairing, locale)}`}
                      </p>
                    </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between gap-3 rounded-2xl border border-white/10 bg-white/[0.04] px-4 py-3 text-[12px] text-white/80">
                <div>
                  ...
                  <p className="text-[12px] text-white/70">
                    {supportsKnockout
                      ? "Gera o quadro a partir da classificação."
                      : `Formato atual: ${formatLabel(generationFormat)}. Não usa eliminatórias.`}
                  </p>
                  {supportsKnockout && groupMissing > 0 && (
                    <p className="text-[11px] text-amber-200">
                      Faltam {groupMissing} jogo{groupMissing === 1 ? "" : "s"}
                      de grupos. Só Dono/Co-dono pode forçar.
                    </p>
                  )}
                </div>
                <button
              ...
              {koEditMode && (
                <p className="text-[11px] text-white/60">
                  Edição manual disponível só antes do início dos jogos.
                </p>
              )}
              {koEditMessage && (
                <p
              ...
              {koRounds.length > 0 &&
                koVisibleMatchesCount === 0 &&
                liveOpsFilter !== "ALL" && (
                  <p className="text-sm text-white/70">
                    Sem jogos de eliminatórias para o filtro operacional
                    selecionado.
                  </p>
                )}
              {koGeneratedAt && (
                <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4 text-[12px] text-white/80 space-y-1">
                  ...
                  {koOverride && (
                    <p className="text-amber-200">
                      Override: grupos incompletos.
                    </p>
                  )}
                  {koManual && (
                    <p className="text-amber-200">
                      Override manual
                      {koManualAt ? `: ${formatDate(koManualAt)}.` : "."}
                    </p>
                  )}
              ...
              {koRounds.length > 0 &&
                ...
                liveOpsFilter !== "ALL" && (
                  <p className="text-[11px] text-white/60">
                    {`A mostrar ${koVisibleMatchesCount} jogo(s) de eliminatórias em ${liveOpsFilterLabel}.`}
                  </p>
                )}
              {koRounds.length > 0 && (
                      ...
                      const visibleGames =
                        liveOpsFilter === "ALL"
                          ...
                          : games.filter((game) => {
                              const fullMatch = matchById.get(game.id);
                              return fullMatch
                                ? doesMatchPassLiveOpsFilter(
                                    fullMatch,
                                    liveOpsFilter,
                                  )
                                : false;
                            });
                      if (visibleGames.length === 0 && liveOpsFilter !== "ALL")
                        return null;
                      const isLast = roundIdx === koRounds.length - 1;
                      return (
                        <div
                          key={roundKey}
                          className="relative min-w-[220px] rounded-2xl border border-white/10 bg-white/[0.04] p-3 space-y-2"
                        >
                          {!isLast && (
                            <div className="absolute top-3 right-[-12px] h-[90%] w-px bg-white/10 hidden lg:block" />
                          )}
                          <p className="text-[11px] uppercase tracking-[0.18em] text-white/60">
                          ...
                          {visibleGames.map((g) => {
                            ...
                            const roundLabel =
                              fullMatch?.roundLabel || roundKey;
                            const used =
                              koUsedByRound.get(roundLabel) ?? new Set();
                            const draft = fullMatch
                              ? getKoDraft(fullMatch)
                              : null;
                            const available = fullMatch
                              ? confirmedPairings.filter((p) => {
                                  const currentA =
                                    fullMatch.pairingA?.id ??
                                    fullMatch.pairingAId ??
                                    null;
                                  const currentB =
                                    fullMatch.pairingB?.id ??
                                    fullMatch.pairingBId ??
                                    null;
                                  return (
                                    !used.has(p.id) ||
                                    p.id === currentA ||
                            ...
                            const availableA = draft
                              ? available.filter(
                                  (p) => p.id !== (draft.pairingBId ?? null),
                                )
                              : [];
                            const availableB = draft
                              ? available.filter(
                                  (p) => p.id !== (draft.pairingAId ?? null),
                                )
                              : [];
                            const hasChanges =
                              fullMatch && draft
                                ? hasKoDraftChanges(fullMatch, draft)
                                : false;
                            return (
                              <div
                                key={g.id}
                              ...
                              >
                                ...
                                {koEditMode && fullMatch && draft && (
                                  <div className="space-y-2 rounded-lg border border-white/10 bg-black/30 p-2 text-[11px] text-white/70">
                                    ...
                                    <button
                                      type="button"
                                      disabled={
                                        !hasChanges || koSaving[fullMatch.id]
                                      }
                                      onClick={() =>
                                ...
                                {fullMatch
                                  ? renderResultControls(fullMatch)
                                  : null}
                              </div>
                            );
                          })}
