...
export default function ChannelRequestsPanel() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const json = (await res
        .json()
        .catch(() => null)) as ChannelRequestsResponse | null;
      if (res.status === 403) {
        setForbidden(true);
        setItems([]);
        return;
  ...
  const subtitle = useMemo(() => {
    ...
    if (hasItems)
      return `${items.length} pedido${items.length > 1 ? "s" : ""} pendente${items.length > 1 ? "s" : ""}.`;
  ...
  return (
    <section className="rounded-2xl border border-white/10 bg-[rgba(6,10,20,0.75)] px-4 py-3">
      {" "}
