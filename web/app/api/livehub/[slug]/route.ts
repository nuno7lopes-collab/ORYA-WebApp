...
export async function GET(req: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  ...
  try {
    ...
    const requestedCategoryIdRaw = req.nextUrl.searchParams.get("categoryId");
    const requestedCategoryId = requestedCategoryIdRaw ? Number(requestedCategoryIdRaw) : null;
    const normalizedRequestedCategoryId =
      requestedCategoryId && Number.isFinite(requestedCategoryId) ? requestedCategoryId : null;
    const padelCategoryLinks = isPadelEvent && Array.isArray(event.padelCategoryLinks) ? event.padelCategoryLinks : [];
    const enabledPadelLinks = padelCategoryLinks.filter((link) => link.isEnabled);
    const visiblePadelLinks = enabledPadelLinks.filter((link) => !link.isHidden);
    const enabledCategoryIds = new Set(enabledPadelLinks.map((link) => link.padelCategoryId));
    const defaultCategoryId = event.padelTournamentConfig?.defaultCategoryId ?? null;
    const activePadelCategoryId = isPadelEvent
      ? normalizedRequestedCategoryId && enabledCategoryIds.has(normalizedRequestedCategoryId)
        ? normalizedRequestedCategoryId
        : defaultCategoryId && enabledCategoryIds.has(defaultCategoryId)
          ? defaultCategoryId
          : visiblePadelLinks[0]?.padelCategoryId ?? enabledPadelLinks[0]?.padelCategoryId ?? null
      : null;
    const activePadelCategoryLink =
      activePadelCategoryId != null
        ? enabledPadelLinks.find((link) => link.padelCategoryId === activePadelCategoryId) ?? null
        : null;
    const padelCategoriesPayload = isPadelEvent
      ? enabledPadelLinks.map((link) => ({
          padelCategoryId: link.padelCategoryId,
          linkId: link.id,
          label: link.category?.label ?? `Categoria ${link.padelCategoryId}`,
          isHidden: link.isHidden,
        }))
      : [];
    const effectiveLiveStreamUrl = activePadelCategoryLink?.liveStreamUrl ?? event.liveStreamUrl;
  ...
  if (event.tournament) {
    ...
    const featuredMatchId =
      typeof (config as Record<string, unknown>).featuredMatchId === "number"
        ...
        : null;
    if (structure) {
      let pairingCategoryMap: Map<number, number | null> | null = null;
      if (activePadelCategoryId) {
        ...
        if (pairingIdsList.length > 0) {
          const [padelPairingsRaw, tournamentEntriesRaw] = await Promise.all([
            prisma.padelPairing.findMany({
              where: { eventId: event.id, id: { in: pairingIdsList } },
          ...
          pairingCategoryMap = new Map<number, number | null>();
          padelPairingsRaw.forEach((pairing) => {
            pairingCategoryMap?.set(pairing.id, pairing.categoryId ?? null);
          });
          tournamentEntriesRaw.forEach((entry) => {
            if (!pairingCategoryMap?.has(entry.id)) {
              pairingCategoryMap?.set(entry.id, entry.categoryId ?? null);
            }
          });
        } else {
          pairingCategoryMap = new Map<number, number | null>();
      ...
      const matchMatchesCategory = (match: { pairing1Id: number | null; pairing2Id: number | null }) => {
        if (!activePadelCategoryId || !pairingCategoryMap) return true;
        const p1 = typeof match.pairing1Id === "number" ? pairingCategoryMap.get(match.pairing1Id) ?? null : null;
        const p2 = typeof match.pairing2Id === "number" ? pairingCategoryMap.get(match.pairing2Id) ?? null : null;
        if (!p1 && !p2) return enabledPadelLinks.length <= 1;
        if (p1 && p1 !== activePadelCategoryId) return false;
        if (p2 && p2 !== activePadelCategoryId) return false;
        return true;
      };
      ...
      const stages = structure.stages.map((s) => ({
        ...
        groups: s.groups.map((g) => {
          const matches = g.matches.filter(matchMatchesCategory).map(buildMatch);
          return {
            id: g.id,
            name: g.name,
            standings: computeStandingsForGroup(matches, tieBreakRules, structure.generationSeed || undefined),
            matches,
          };
        }),
        matches: s.matches.filter((m) => !m.groupId).filter(matchMatchesCategory).map(buildMatch),
      }));

      const flatMatches = stages.flatMap((s) => [...s.matches, ...s.groups.flatMap((g) => g.matches)]);
      const activeFeaturedMatchId =
        featuredMatchId && flatMatches.some((match) => match.id === featuredMatchId) ? featuredMatchId : null;
      ...
      if (userId) {
        const pairing = await prisma.padelPairing.findFirst({
          where: {
            eventId: event.id,
            ...(activePadelCategoryId ? { categoryId: activePadelCategoryId } : {}),
      ...
      tournamentPayload = {
        ...
        lastMatch,
        championPairingId,
        featuredMatchId: activeFeaturedMatchId,
  ...
  const res = NextResponse.json(
    {
      ...
      event: {
        ...
        liveStreamUrl: effectiveLiveStreamUrl,
      ...
      pairings,
      padelCategories: padelCategoriesPayload,
      activePadelCategoryId,
    },
    { status: 200 },
  );
