...
import OrganizationEventDetailPage from "@/app/org/_internal/core/(dashboard)/eventos/[id]/page";
...
export default async function OrganizationTorneioDetailPage(props: PageProps) {
  ...
  if (event.templateType !== "PADEL") {
    const target = await appendOrganizationIdToRedirectHref(`/org/eventos/${eventId}`);
    redirect(target);
  }
