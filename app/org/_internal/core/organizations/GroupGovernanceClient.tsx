...
type Props = { group: OrgHubGroupPayload };
type CodesState = {
...
export default function GroupGovernanceClient({ group }: Props) {
  ...
  const [joinOrganizationByGroup, setJoinOrganizationByGroup] = useState<
    Record<number, string>
  >({});
  const [exitOrganizationByGroup, setExitOrganizationByGroup] = useState<
    Record<number, string>
  >({});
  const [exitModeByGroup, setExitModeByGroup] = useState<
    Record<number, ExitMode>
  >({});
  const [exitTargetByGroup, setExitTargetByGroup] = useState<
    Record<number, string>
  >({});
  const [transferTargetByGroup, setTransferTargetByGroup] = useState<
    Record<number, string>
  >({});
  const [transferTokenById, setTransferTokenById] = useState<
    Record<string, string>
  >({});
  const [participantByRequest, setParticipantByRequest] = useState<
    Record<string, string>
  >({});
  const [codesByRequest, setCodesByRequest] = useState<
    Record<string, CodesState>
  >({});
  ...
  const [governanceInviteByGroup, setGovernanceInviteByGroup] = useState<
    Record<number, string>
  >({});
  const [governanceRoleByGroup, setGovernanceRoleByGroup] = useState<
    Record<number, string>
  >({});
  const [governanceRoleByMember, setGovernanceRoleByMember] = useState<
    Record<string, string>
  >({});
  const [
    linkedOrganizationsVisibilityByGroup,
    setLinkedOrganizationsVisibilityByGroup,
  ] = useState<Record<number, boolean>>({});
  const summary = useMemo(() => {
    const organizations = initialGroups.reduce(
      (sum, group) => sum + group.organizations.length,
      0,
    );
    const openRequests = initialGroups.reduce(
      (sum, group) => sum + group.openRequests.length,
      0,
    );
    const pendingTransfers = initialGroups.reduce(
      (sum, group) => sum + group.pendingTransfers.length,
      0,
    );
    const actionable = initialGroups.reduce(
      (sum, group) =>
        sum +
        group.actionableRequestCount +
        group.pendingTransfers.filter((item) => item.isActionable).length,
      0,
    );
    return { organizations, openRequests, pendingTransfers, actionable };
  }, [initialGroups]);
  const groupDashboardHref = useMemo(
    () => buildOrgHubHref(`/groups/${group.groupId}`),
    [group.groupId],
  );
  ...
  const submitJson = async (
    ...
    if (!response.ok || json?.ok === false) {
      throw new Error(
        json?.error || json?.message || json?.errorCode || "Operação falhou",
      );
    }
    return json;
  };
  ...
  const startJoinRequest = (groupId: number) => {
    ...
    if (!Number.isInteger(selected) || selected <= 0) {
      setFeedback({
        text: "Seleciona uma organização para entrada.",
        tone: "error",
      });
      return;
    }
    ...
    const candidate =
      group?.joinCandidates.find((item) => item.organizationId === selected) ??
      null;
    if (candidate?.hasOpenJoinRequest) {
      setFeedback({
        text: "Já existe um pedido de entrada aberto para essa organização.",
        tone: "error",
      });
      return;
    }
  ...
  const startExitRequest = (groupId: number) => {
    ...
    if (!Number.isInteger(selectedOrgId) || selectedOrgId <= 0) {
      setFeedback({
        text: "Seleciona a organização para saída.",
        tone: "error",
      });
      return;
    }
    ...
    if (selectedMode === "TRANSFER_OWNER" && !targetOwnerIdentifier) {
      setFeedback({
        text: "Indica o utilizador/email do próximo owner.",
        tone: "error",
      });
      return;
    }
    void runAction(`exit:${groupId}`, async () => {
      await submitJson("/api/org-hub/groups/exit-requests", {
        ...
        targetOwnerIdentifier:
          selectedMode === "TRANSFER_OWNER" ? targetOwnerIdentifier : undefined,
      });
  ...
  const startOwnerTransfer = (groupId: number) => {
    ...
    if (!targetUserId) {
      setFeedback({
        text: "Indica o utilizador/email para transferência do owner.",
        tone: "error",
      });
      return;
    }
    void runAction(`transfer:start:${groupId}`, async () => {
      await submitJson(`/api/org-hub/groups/${groupId}/owner/transfer/start`, {
        targetUserId,
      });
  ...
  const cancelOwnerTransfer = (transfer: OrgHubGroupOwnerTransfer) => {
    void runAction(`transfer:cancel:${transfer.id}`, async () => {
      await submitJson(
        `/api/org-hub/groups/${transfer.groupId}/owner/transfer/cancel`,
  ...
  const confirmOwnerTransfer = (transfer: OrgHubGroupOwnerTransfer) => {
    ...
    if (!token) {
      setFeedback({
        text: "Indica o token da transferência de owner.",
        tone: "error",
      });
      return;
    }
    void runAction(`transfer:confirm:${transfer.id}`, async () => {
      await submitJson(
        `/api/org-hub/groups/${transfer.groupId}/owner/transfer/confirm`,
        { token },
      );
    ...
    void runAction(`request:code:${request.id}`, async () => {
      ...
      const participantLabel =
        typeof result?.participant === "string"
          ...
          : "participant";
      return code
        ? `Código (${formatParticipantLabel(participantLabel)}): ${code}`
        : "Código gerado.";
    });
  };
  const verifyCodes = (request: OrgHubGroupOpenRequest) => {
    if (request.type === "EXIT_KEEP_OWNER") {
      setFeedback({
        text: "Este pedido não usa validação por códigos.",
        tone: "error",
      });
      return;
    }
    ...
    void runAction(`request:verify:${request.id}`, async () => {
      await submitJson(
        `/api/org-hub/groups/${base}/${request.id}/verify-codes`,
        payload,
      );
  ...
  const resendEmails = (request: OrgHubGroupOpenRequest) => {
    ...
    void runAction(`request:email:${request.id}`, async () => {
      await submitJson(
        `/api/org-hub/groups/${base}/${request.id}/email/resend`,
      );
  ...
  const confirmToken = (request: OrgHubGroupOpenRequest) => {
    ...
    void runAction(`request:confirm:${request.id}`, async () => {
      await submitJson(
        `/api/org-hub/groups/${base}/${request.id}/email/confirm`,
        { token },
      );
      return "Token confirmado.";
    });
  };
  const formatParticipantLabel = (raw: string) =>
    PARTICIPANT_LABELS[raw] ?? raw;
  ...
  const saveLinkedOrganizationsVisibility = (
    groupId: number,
      ...
      <div className="mx-auto w-full max-w-[1240px] px-4 py-12 text-white sm:px-6 lg:px-8">
        ...
        <section className="rounded-3xl border border-white/12 bg-white/[0.04] p-6 ">
          ...
          <p className="mt-2 text-sm text-white/70">
            {" "}
            Ainda não tens grupos ativos para gerir. Cria ou entra numa
    ...
    >
      ...
      <div className="space-y-6">
          ...
          <div className="mt-4 flex flex-wrap items-start justify-between gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-white/75">
                Renomeia o grupo, gere membros de governança e controla
                operações.
              </p>{" "}
            </div>{" "}
            <Link
        ...
        {initialGroups.map((group) => {
          ...
          const selectedJoinCandidateId =
            Number(joinOrganizationByGroup[group.groupId] ?? "0") || null;
          const selectedJoinCandidate =
            typeof selectedJoinCandidateId === "number"
              ? (group.joinCandidates.find(
                  (candidate) =>
                    candidate.organizationId === selectedJoinCandidateId,
                ) ?? null)
              : null;
          const selectedExitOrgId =
            Number(exitOrganizationByGroup[group.groupId] ?? "0") || null;
          const selectedExitOrg =
            typeof selectedExitOrgId === "number"
              ? group.organizations.find(
                  (organization) =>
                    organization.organizationId === selectedExitOrgId,
                )
              : null;
          const defaultGroupOrgHref = group.organizations[0]
            ? buildOrgHref(group.organizations[0].organizationId, "/team")
            : null;
          const groupDisplayName = resolveGroupDisplayName(
            group.groupName,
            group.groupId,
          );
          ...
          const linkedOrganizationsVisibleValue =
            linkedOrganizationsVisibilityByGroup[group.groupId] ??
            group.showLinkedOrganizationsPublicly;
          const governanceMembers = group.governanceMembers ?? [];
          const ownerMember =
            governanceMembers.find((member) => member.role === "OWNER") ?? null;
          const actionableOpenRequests = group.openRequests.filter(
            (request) => request.isActionable,
          ).length;
          return (
            <article
              id="resumo"
              key={group.groupId}
              className="rounded-3xl border border-white/15 bg-white/[0.04] p-4 sm:p-5"
            >
              {" "}
              <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-[12px] text-white/72">
                    {" "}
                    {group.organizationCount} subsidiária
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {group.viewerIsGroupOwner && (
                    <button
                      ...
                      onClick={() =>
                        router.push(
                          `${buildOrgHubHref("/create")}?groupMode=EXISTING_GROUP&groupId=${group.groupId}`,
                        )
                      }
                      className="rounded-full border border-white/20 bg-white/10 px-3 py-1.5 text-[11px] font-semibold text-white transition hover:bg-white/16 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#22D3EE]/55"
                    >
              ...
              <div
                id="governanca"
                className="mb-4 rounded-2xl border border-white/12 bg-white/6 p-4"
              >
                {" "}
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-white/78">
                      {" "}
                      Owner {ownerMember ? 1 : 0} · Co-owners{" "}
                      {group.governance.coOwnerCount} · Admins{" "}
                ...
                <div className="mt-4 grid gap-3 lg:grid-cols-[minmax(0,1fr),320px]">
                  ...
                  <div className="rounded-2xl border border-white/10 bg-white/4 p-3">
                    ...
                    <p className="text-[11px] uppercase tracking-[0.2em] text-white/55">
                      Equipa de governança
                    </p>{" "}
                    {governanceMembers.length === 0 ? (
                      <p className="mt-2 text-sm text-white/60">
                        Sem membros de governança registados.
                      </p>
                    ) : (
                                ...
                                {group.viewerIsGroupOwner &&
                                  member.role !== "OWNER" && (
                                    <>
                                      ...
                                      <select
                                        value={
                                          governanceRoleByMember[memberKey] ??
                                          member.role
                                        }
                                        onChange={(event) =>
                                          setGovernanceRoleByMember((prev) => ({
                                            ...prev,
                  ...
                  <div className="space-y-3 rounded-2xl border border-white/10 bg-white/4 p-3">
                    ...
                    <div className="space-y-2">
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          renameGroup(group.groupId, group.groupName)
                        }
                        disabled={!group.viewerIsGroupOwner}
                    ...
                    <div className="space-y-2">
                      {" "}
                      <label className="text-[11px] text-white/60">
                        Visibilidade pública de perfis associados
                      </label>{" "}
                      <select
                        value={
                          linkedOrganizationsVisibleValue ? "VISIBLE" : "HIDDEN"
                        }
                        onChange={(event) =>
                      ...
                      >
                        ...
                        <option value="HIDDEN">
                          Ocultar no perfil público
                        </option>{" "}
                      </select>{" "}
                      <button
                        type="button"
                        onClick={() =>
                          saveLinkedOrganizationsVisibility(
                            group.groupId,
                            group.showLinkedOrganizationsPublicly,
                          )
                        }
                    ...
                    <div className="space-y-2">
                      {" "}
                      <label className="text-[11px] text-white/60">
                        Adicionar co-owner/admin
                      </label>{" "}
                      <input
                        type="text"
                        value={governanceInviteByGroup[group.groupId] ?? ""}
                        onChange={(event) =>
                          setGovernanceInviteByGroup((prev) => ({
                            ...prev,
                            [group.groupId]: event.target.value,
                          }))
                        }
                      ...
                      <div className="flex gap-2">
                        ...
                        <select
                          value={
                            governanceRoleByGroup[group.groupId] ?? "CO_OWNER"
                          }
                          onChange={(event) =>
                            setGovernanceRoleByGroup((prev) => ({
                              ...prev,
                              [group.groupId]: event.target.value,
                            }))
                          }
              ...
              <div className="grid gap-4 xl:grid-cols-[1.2fr_1fr]">
                ...
                <section className="rounded-2xl border border-white/15 bg-white/[0.07] p-4">
                  ...
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <h3 className="text-sm font-semibold uppercase tracking-[0.16em] text-white/80">
                      Organizações no grupo
                    </h3>{" "}
                    <span className="text-[11px] text-white/55">
                      {group.organizations.length} visíveis
                    </span>{" "}
                  </div>{" "}
                  <div className="mt-3 grid gap-3 sm:grid-cols-2">
                    {" "}
                    {group.organizations.map((organization) => {
                      const orgOverviewHref = buildOrgHref(
                          ...
                          <div className="mt-2 flex flex-wrap gap-1 text-[10px] uppercase tracking-[0.16em]">
                            ...
                            <span
                              className={cn(
                                ...
                                toneForStatus(organization.status),
                              )}
                            >
                              {" "}
                            ...
                            {organization.viewerRole && (
                              <span
                                className={cn(
                                  "rounded-full border px-2 py-1",
                                  toneForRole(organization.viewerRole),
                                )}
                              >
                                {" "}
                                {organization.viewerRole}{" "}
                ...
                >
                  ...
                  <p className="mt-1 text-[12px] text-white/60">
                    Entradas, saídas e transferência de owner.
                  </p>{" "}
                  <div className="mt-3 space-y-4">
                    ...
                    <div className="space-y-2 rounded-2xl border border-white/12 bg-black/25 p-3">
                      ...
                      <p className="text-[12px] font-semibold uppercase tracking-[0.16em] text-white/75">
                        Entrada de organização
                      </p>{" "}
                      {group.viewerIsGroupOwner ? (
                        <>
                          {" "}
                          <label
                            htmlFor={`join-org-${group.groupId}`}
                            className="sr-only"
                          >
                            {" "}
                          ...
                          <select
                            ...
                            onChange={(event) =>
                              setJoinOrganizationByGroup((prev) => ({
                                ...prev,
                                [group.groupId]: event.target.value,
                              }))
                            }
                          ...
                          >
                            {" "}
                            <option value="">
                              Seleciona organização owner tua
                            </option>{" "}
                          ...
                          {selectedJoinCandidate?.hasOpenJoinRequest && (
                            <p className="text-[12px] text-amber-200">
                              {" "}
                              Já existe um pedido de entrada aberto para esta
                              organização.{" "}
                            </p>
                          )}{" "}
                          <button
                        ...
                        <p className="text-sm text-white/66">
                          Só o owner do grupo pode iniciar entradas.
                        </p>
                      )}{" "}
                    </div>{" "}
                    <div className="space-y-2 rounded-2xl border border-white/12 bg-black/25 p-3">
                      ...
                      {group.viewerIsGroupOwner ? (
                        <>
                          {" "}
                          <label
                            htmlFor={`exit-org-${group.groupId}`}
                            className="sr-only"
                          >
                            {" "}
                          ...
                          <select
                            ...
                            onChange={(event) =>
                              setExitOrganizationByGroup((prev) => ({
                                ...prev,
                                [group.groupId]: event.target.value,
                              }))
                            }
                          ...
                          >
                            {" "}
                            <option value="">
                              Seleciona organização para saída
                            </option>{" "}
                            {group.organizations.map((organization) => (
                              <option
                                key={organization.organizationId}
                          ...
                          <label
                            htmlFor={`exit-mode-${group.groupId}`}
                            className="sr-only"
                          >
                            {" "}
                            Modo de saída{" "}
                          ...
                          <select
                            ...
                            onChange={(event) =>
                              setExitModeByGroup((prev) => ({
                                ...prev,
                                [group.groupId]:
                                  event.target.value === "TRANSFER_OWNER"
                                    ? "TRANSFER_OWNER"
                                    : "KEEP_OWNER",
                              }))
                            }
                          ...
                          >
                            {" "}
                            <option value="KEEP_OWNER">
                              {EXIT_MODE_LABELS.KEEP_OWNER}
                            </option>{" "}
                            <option value="TRANSFER_OWNER">
                              {EXIT_MODE_LABELS.TRANSFER_OWNER}
                            </option>{" "}
                          </select>{" "}
                          {exitMode === "TRANSFER_OWNER" && (
                            <>
                              {" "}
                              <label
                                htmlFor={`exit-target-${group.groupId}`}
                                className="sr-only"
                              >
                                {" "}
                          ...
                          {selectedExitOrg?.viewerIsOrgOwner &&
                            exitMode === "KEEP_OWNER" && (
                              <p className="text-[12px] text-amber-200">
                                {" "}
                                Organização já pertence ao teu owner. Em muitos
                                casos usa TRANSFER_OWNER.{" "}
                              </p>
                            )}{" "}
                          <button
                            type="button"
                        ...
                        <p className="text-sm text-white/66">
                          Só o owner do grupo pode iniciar saídas.
                        </p>
                      )}{" "}
                    </div>{" "}
                    {group.viewerIsGroupOwner && (
                      <div className="space-y-2 rounded-2xl border border-white/12 bg-black/25 p-3">
                        ...
                        <label
                          htmlFor={`transfer-target-${group.groupId}`}
                          className="sr-only"
                        >
                          {" "}
                          Identificador do próximo owner{" "}
                        ...
                        >
                          {" "}
                          {isBusy(`transfer:start:${group.groupId}`)
                            ? "A iniciar..."
                            : "Transferir owner"}{" "}
                        </button>{" "}
                      </div>
                ...
                <details className="mt-4 rounded-2xl border border-white/14 bg-white/6 p-4">
                  ...
                  <div className="mt-3 space-y-2">
                    ...
                    {group.pendingTransfers.map((transfer) => (
                      <div
                        key={transfer.id}
                        className="rounded-xl border border-white/12 bg-black/25 p-3 text-sm text-white/82"
                      >
                        {" "}
                        <p>
                          {" "}
                        ...
                        <p className="text-[12px] text-white/60">
                          Expira em {formatDateTime(transfer.expiresAt)}
                        </p>{" "}
                        {transfer.toLabel === "Tu" && (
                          <div className="mt-2 grid gap-2 sm:grid-cols-[1fr_auto]">
                            {" "}
                            <label
                              htmlFor={`transfer-token-${transfer.id}`}
                              className="sr-only"
                            >
                              {" "}
                            ...
                            <button
                              ...
                              disabled={isBusy(
                                `transfer:confirm:${transfer.id}`,
                              )}
                              className="rounded-full border border-white/20 bg-white/10 px-4 py-2 text-[12px] font-semibold text-white transition hover:bg-white/16 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#22D3EE]/55 disabled:opacity-50"
                            >
                              {" "}
                              {isBusy(`transfer:confirm:${transfer.id}`)
                                ? "A confirmar..."
                                : "Confirmar owner"}{" "}
                            </button>{" "}
                          </div>
                        ...
                        {group.viewerIsGroupOwner &&
                          transfer.fromUserId === group.ownerUserId && (
                            <button
                              type="button"
                            ...
                            >
                              {" "}
                              {isBusy(`transfer:cancel:${transfer.id}`)
                                ? "A cancelar..."
              ...
              <details
                id="pedidos"
                className="mt-4 rounded-2xl border border-white/14 bg-white/6 p-4"
              >
                {" "}
                <summary className="cursor-pointer list-none text-sm font-semibold uppercase tracking-[0.16em] text-white/80">
                  {" "}
                  Pedidos em aberto ({group.openRequests.length})
                  {actionableOpenRequests > 0
                    ? ` · ${actionableOpenRequests} com ação tua`
                    : ""}{" "}
                </summary>{" "}
                <div className="mt-3 space-y-3">
                  ...
                  {group.openRequests.length === 0 && (
                    <p className="text-sm text-white/66">
                      Sem pedidos abertos neste grupo.
                    </p>
                  )}{" "}
                  {group.openRequests.map((request) => {
                    ...
                    const participantOptions = isJoin
                      ...
                      : request.type === "EXIT_TRANSFER_OWNER"
                        ...
                        : [];
                    const allowedParticipants = participantOptions.filter(
                      (participant) => {
                        if (participant === "GROUP_OWNER")
                          return request.canActAsGroupOwner;
                        if (participant === "ORG_OWNER")
                          return request.canActAsOrgOwner;
                        if (participant === "TARGET_OWNER")
                          return request.canActAsTargetOwner;
                        return false;
                      },
                    );
                        ...
                        <div className="flex flex-wrap items-start justify-between gap-2">
                          ...
                          <div className="space-y-1">
                            ...
                            <p className="text-[11px] text-white/56">
                              {" "}
                              Criado {formatDateTime(request.createdAt)} ·
                              Expira {formatDateTime(request.expiresAt)}{" "}
                            </p>{" "}
                            <p className="text-[11px] text-white/50">
                              {" "}
                              Códigos: {formatDateTime(request.codeExpiresAt)} ·
                              Email token:{" "}
                              {formatDateTime(request.emailTokenExpiresAt)} ·
                              Reenvios: {request.resendCount}{" "}
                            </p>{" "}
                          </div>{" "}
                          <div className="flex flex-wrap gap-1 text-[10px] uppercase tracking-[0.16em]">
                            {" "}
                            {request.canActAsGroupOwner && (
                              <span className="rounded-full border border-[#22D3EE]/45 bg-[#22D3EE]/12 px-2 py-1 text-[#CCFCFF]">
                                Owner do grupo
                              </span>
                            )}{" "}
                            {request.canActAsOrgOwner && (
                              <span className="rounded-full border border-white/20 bg-white/8 px-2 py-1 text-white/75">
                                Owner da org
                              </span>
                            )}{" "}
                        ...
                        {!request.isActionable ? (
                          ...
                          <>
                            ...
                            {request.type === "EXIT_KEEP_OWNER" ? (
                              <div className="mt-3 rounded-2xl border border-white/10 bg-white/[0.04] p-3 text-sm text-white/70">
                                {" "}
                                Este pedido não usa códigos. Confirma por email
                                quando receberes o token.{" "}
                              </div>
                            ) : (
                              <div className="mt-3 rounded-2xl border border-white/10 bg-white/[0.04] p-3">
                                ...
                                <div className="mt-2 grid gap-2 lg:grid-cols-2">
                                  ...
                                  {allowedParticipants.length > 1 && (
                                    <>
                                      {" "}
                                      <label
                                        htmlFor={`participant-${request.id}`}
                                        className="sr-only"
                                      >
                                        {" "}
                                ...
                                <div className="mt-2 grid gap-2 sm:grid-cols-2">
                                  ...
                                  <input
                                    ...
                                    onChange={(event) =>
                                      setCodesByRequest((prev) => ({
                                        ...
                                        [request.id]: {
                                          ...prev[request.id],
                                          orgOwnerCode: event.target.value,
                                        },
                                      }))
                                    }
                                    ...
                                    <input
                                      ...
                                      onChange={(event) =>
                                        setCodesByRequest((prev) => ({
                                          ...
                                          [request.id]: {
                                            ...prev[request.id],
                                            groupOwnerCode: event.target.value,
                                          },
                                        }))
                                      }
                                    ...
                                    <input
                                      ...
                                      onChange={(event) =>
                                        setCodesByRequest((prev) => ({
                                          ...
                                          [request.id]: {
                                            ...prev[request.id],
                                            targetOwnerCode: event.target.value,
                                          },
                                        }))
                                      }
                                  ...
                                  >
                                    {" "}
                                    {isBusy(`request:verify:${request.id}`)
                                      ? "A validar..."
                                      : "Verificar códigos"}{" "}
                                  </button>{" "}
                                </div>{" "}
                            ...
                            <div className="mt-3 rounded-2xl border border-white/10 bg-white/[0.04] p-3">
                              ...
                              <div className="mt-2 grid gap-2 sm:grid-cols-2">
                                ...
                                >
                                  {" "}
                                  {isBusy(`request:email:${request.id}`)
                                    ? "A enviar..."
                                    : "Reenviar emails"}{" "}
                                </button>{" "}
                                <label
                                  htmlFor={`email-token-${request.id}`}
                                  className="sr-only"
                                >
                                  {" "}
                                ...
                                <button
                                  ...
                                  disabled={isBusy(
                                    `request:confirm:${request.id}`,
                                  )}
                                  className="rounded-full border border-white/20 bg-white/10 px-4 py-2 text-[12px] font-semibold text-white transition hover:bg-white/16 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#22D3EE]/55 disabled:opacity-50 sm:col-span-2"
                                >
                                  {" "}
                                  {isBusy(`request:confirm:${request.id}`)
                                    ? "A confirmar..."
                                    : "Confirmar token"}{" "}
                                </button>{" "}
                              </div>{" "}
