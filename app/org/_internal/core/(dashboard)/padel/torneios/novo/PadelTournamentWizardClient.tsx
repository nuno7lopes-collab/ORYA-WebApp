...
const createDailyWindowFromDate = (date: Date, durationMinutes = 60): DailyWindowDraft => {
...
const normalizeDailyWindows = (windows: DailyWindowDraft[]) =>
  windows
    .map((windowItem) => ({
      ...
      start: parseDateTimeLocal(combineDateAndTime(windowItem.date, windowItem.startTime)),
      end: parseDateTimeLocal(combineDateAndTime(windowItem.date, windowItem.endTime)),
    }))
    .filter((item): item is { date: string; startTime: string; endTime: string; start: Date; end: Date } =>
...
export default function PadelTournamentWizardClient({ organizationId }: { organizationId: number }) {
  ...
  const selectableClubs = useMemo(() => clubs.filter((club) => club.kind !== "PARTNER"), [clubs]);
  ...
  useEffect(() => {
    ...
    if (organizationTimezone) {
      setTimezone((prev) => (prev === "Europe/Lisbon" ? organizationTimezone : prev));
    }
  ...
  useEffect(() => {
    ...
    setDailyWindows([createDailyWindowFromDate(baseDate, asNumber(durationMinutes) ?? 60)]);
    // intentionally seeded once
  ...
  const normalizedDailyWindows = useMemo(() => normalizeDailyWindows(dailyWindows), [dailyWindows]);
  ...
  const tabs = [
    ...
    { id: "wizard-registration", label: "Inscrições", href: "#wizard-registration" },
    { id: "wizard-categories", label: "Categorias", href: "#wizard-categories" },
    ...
    <TournamentFormSurface
        ...
        <div className="space-y-5">
          <section id="wizard-identity" className="space-y-4 rounded-2xl border border-white/10 bg-white/[0.03] p-4">
            <div className="space-y-1 border-b border-white/10 pb-3">
              ...
              <p className="text-sm text-white/75">Nome do torneio, clube e localização.</p>
            </div>
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <label className="space-y-1 text-sm text-white/70">
                <span className="text-[11px] uppercase tracking-[0.18em] text-white/50">Início oficial</span>
                <input
              ...
              <label className="space-y-1 text-sm text-white/70">
                <span className="text-[11px] uppercase tracking-[0.18em] text-white/50">Fim oficial</span>
                <input
          ...
          <section id="wizard-registration" className="space-y-4 rounded-2xl border border-white/10 bg-white/[0.03] p-4">
            <div className="space-y-1 border-b border-white/10 pb-3">
              <p className="text-[10px] uppercase tracking-[0.2em] text-white/55">Inscrições e Agenda</p>
              <p className="text-sm text-white/75">Janela de inscrições e horários por dia.</p>
            </div>
            ...
            <div className="grid gap-3 md:grid-cols-3">
              ...
              <label className="space-y-1 text-sm text-white/70">
                <span className="text-[11px] uppercase tracking-[0.18em] text-white/50">Inscrições fecham</span>
                <OryaDateTimeField
                  ...
                  return (
                    ...
                    <label className="space-y-1 text-[12px] text-white/70">
                      ...
                      <OryaTimeField
                        ...
                        onChange={(next) =>
                          setDailyWindows((prev) =>
                            prev.map((entry) => (entry.id === windowItem.id ? { ...entry, startTime: next } : entry)),
                          )
                        }
                    ...
                    <label className="space-y-1 text-[12px] text-white/70">
                      <span className="text-[10px] uppercase tracking-[0.16em] text-white/50">Fim</span>
                      <OryaTimeField
                        value={windowItem.endTime}
                        onChange={(next) =>
                          setDailyWindows((prev) =>
                            prev.map((entry) => (entry.id === windowItem.id ? { ...entry, endTime: next } : entry)),
                          )
                        }
                    ...
                    <div className="flex flex-col items-end justify-end gap-2">
                      ...
                      <button
                        ...
                        onClick={() => setDailyWindows((prev) => prev.filter((entry) => entry.id !== windowItem.id))}
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <label className="space-y-1 text-sm text-white/70">
                <span className="text-[11px] uppercase tracking-[0.18em] text-white/50">Duração do jogo (min)</span>
                <input
                  type="number"
                  min={10}
              ...
              <label className="space-y-1 text-sm text-white/70">
                <span className="text-[11px] uppercase tracking-[0.18em] text-white/50">Intervalo entre jogos (min)</span>
                <input
                  type="number"
                  min={0}
            ...
            <div className="rounded-2xl border border-white/10 bg-black/30 px-4 py-3 text-[12px] text-white/80">
              ...
              {showAdvancedPolicies && (
                <div className="mt-3 grid gap-3 md:grid-cols-2">
                  ...
                  <label className="space-y-1 text-sm text-white/70">
                    <span className="text-[11px] uppercase tracking-[0.18em] text-white/50">Buffer técnico (min)</span>
                    <input
                      type="number"
                      min={0}
                      value={bufferMinutes}
                  ...
                  <label className="space-y-1 text-sm text-white/70">
                    <span className="text-[11px] uppercase tracking-[0.18em] text-white/50">Modo de planeamento</span>
                    <select
                      value={plannerMode}
            ...
            {capacityPlan && (
              ...
              >
                ...
                <p className="text-[11px] opacity-90">
                  Slots {capacityPlan.totalSlots} · Jogos {capacityPlan.matchesNeeded} · Em falta{" "}
                ...
                {(capacityPlan.warnings.length > 0 || capacityPlan.blockingReasons.length > 0) && (
                  <p className="mt-2 text-[11px] opacity-90">
                    {capacityPlan.warnings.length > 0 ? `Avisos: ${capacityPlan.warnings.slice(0, 2).join(" · ")}. ` : ""}
            ...
            {categories.length === 0 ? (
              ...
              <div className="grid max-h-[280px] gap-2 overflow-y-auto rounded-2xl border border-white/10 bg-black/25 p-3 md:grid-cols-2">
                {categories.map((cat) => {
                  const draft = categoryDrafts[cat.id];
                  return (
                    <label key={`cat-${cat.id}`} className="flex items-center justify-between rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-sm text-white/80">
                      ...
                      <span className="text-[11px] text-white/55">
                        {[cat.genderRestriction, cat.minLevel, cat.maxLevel].filter(Boolean).join(" · ")}
                      </span>
                    </label>
                  );
                })}
          ...
          <section id="wizard-operation" className="space-y-4 rounded-2xl border border-white/10 bg-white/[0.03] p-4">
            <div className="space-y-1 border-b border-white/10 pb-3">
      ...
      footer={
        <div className="flex flex-wrap items-center gap-3">
          <button type="button" onClick={handleSubmit} disabled={saving} className={`${CTA_PRIMARY} disabled:opacity-60`}>
            {isSubmitting ? "A criar torneio..." : "Criar torneio"}
          </button>
          <span className="text-[12px] text-white/60">O torneio é criado em rascunho.</span>
        </div>
      }
    />
  );
