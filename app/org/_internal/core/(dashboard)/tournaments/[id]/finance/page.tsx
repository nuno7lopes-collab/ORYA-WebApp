// app/org/tournaments/[id]/finance/page.tsx
import { notFound, redirect } from "next/navigation";
import { createSupabaseServer } from "@/lib/supabaseServer";
import ObjectiveSubnav from "@/app/org/_internal/core/ObjectiveSubnav";
import { getAppBaseUrl } from "@/lib/appBaseUrl";

type PageProps = { params: Promise<{ id: string }>; searchParams?: { organizationId?: string } };

async function fetchFinance(tournamentId: number, organizationId: number, cookieHeader: string | null) {
  const baseUrl = getAppBaseUrl();
  const res = await fetch(`${baseUrl}/api/org/${organizationId}/tournaments/${tournamentId}/finance`, {
...
export default async function TournamentFinancePage({ params, searchParams }: PageProps) {
  ...
  if (!organizationId || !Number.isFinite(organizationId)) notFound();
  const finance = await fetchFinance(tournamentId, organizationId, cookie);
  if (!finance?.ok) {
    if (finance?.error === "FORBIDDEN") {
      redirect(`/org/${organizationId}/overview`);
    }
    notFound();
  }
