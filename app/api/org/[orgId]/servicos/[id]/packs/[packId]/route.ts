...
async function _PATCH(
  ...
  try {
    ...
    if (Object.prototype.hasOwnProperty.call(updates, "packPriceCents")) {
      const paidWriteGate = evaluatePaidWriteGate({
        ...
        amountCents: Number(updates.packPriceCents),
      });
      if (!paidWriteGate.ok) {
        return respondError(
          ctx,
    ...
    console.error("PATCH /api/org/[orgId]/servicos/[id]/packs/[packId] error:", err);
...
async function _DELETE(
  ...
  try {
    ...
    console.error("DELETE /api/org/[orgId]/servicos/[id]/packs/[packId] error:", err);
