...
async function _POST(req: NextRequest) {
  ...
  try {
    ...
    const ipAllowlist = resolveInfraIpAllowlist("REDIS_START");
    ...
    if (!guard.ok) return fail(ctx, guard.status, guard.error, guard.message);

    const result = await runScript(ctx, "aws/redis-start.sh", [], { APP_ENV: targetEnv });
    await auditInfraAction(ctx, admin, "ADMIN_INFRA_REDIS_START", { targetEnv, ok: result.ok });

    if (!result.ok) return fail(ctx, 500, "INFRA_REDIS_START_FAILED", trim(result.stderr));

    return respondOk(ctx, { action: "redis_start", stdout: trim(result.stdout), stderr: trim(result.stderr) });
  } catch (err) {
    logError("admin.infra.redis_start_failed", err);
