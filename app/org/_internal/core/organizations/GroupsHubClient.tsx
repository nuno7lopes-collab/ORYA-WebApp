...
type Props = { initialGroups: OrgHubGroupPayload[] };
...
export default function GroupsHubClient({ initialGroups }: Props) {
  ...
  const visibleGroups = useMemo(() => {
    ...
    if (viewFilter === "actionable") {
      return initialGroups.filter(
        (group) =>
          group.actionableRequestCount > 0 ||
          group.pendingTransfers.some((transfer) => transfer.isActionable),
      );
    }
  ...
  const summary = useMemo(() => {
    ...
    const ownedGroups = initialGroups.filter(
      (group) => group.viewerIsGroupOwner,
    ).length;
    ...
    return { groups, ownedGroups, openRequests, actionable };
  }, [initialGroups]);
    ...
    <div className="mx-auto w-full max-w-[1240px] px-4 py-10 text-white sm:px-6 md:py-12 lg:px-8">
      ...
      <div className="space-y-6">
          ...
          <div className="mt-4 flex flex-wrap items-start justify-between gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-white/75">
                Diretório de grupos com acesso rápido ao dashboard e à
                governança.
              </p>{" "}
            </div>{" "}
            <button
        ...
        {visibleGroups.length === 0 && (
          <section className="rounded-3xl border border-white/12 bg-white/[0.04] p-6 text-sm text-white/75 ">
            {" "}
        ...
        {visibleGroups.length > 0 && (
          <section className="grid gap-4 lg:grid-cols-2">
            ...
            {visibleGroups.map((group) => {
              const groupLabel = resolveGroupDisplayName(
                group.groupName,
                group.groupId,
              );
              const organizationsPreview = group.organizations.slice(0, 4);
              const pendingOperations =
                group.openRequests.length + group.pendingTransfers.length;
              return (
                <article
                  key={`group-${group.groupId}`}
                  className="rounded-3xl border border-white/14 bg-white/[0.04] p-5 "
                >
                  {" "}
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    ...
                    <Link
                      href={buildOrgHubHref(
                        `/groups/${group.groupId}/governance`,
                      )}
                      className="rounded-full border border-white/20 bg-white/10 px-3 py-1.5 text-[11px] font-semibold uppercase tracking-[0.16em] text-white transition hover:bg-white/16 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#22D3EE]/55"
                    >
                      {" "}
