...
export default async function OrgStorePage({ params, searchParams }: PageProps) {
  ...
  const subParam = Array.isArray(resolvedSearchParams.sub) ? resolvedSearchParams.sub[0] : resolvedSearchParams.sub;
  ...
  const baseHref = `/org/${organization.id}/loja`;
  const endpointBase = `/api/org/${organization.id}/store`;
      ...
      <div className="orya-page-width px-4 pb-16 pt-10">
        ...
        {!store ? null : view === "overview" ? (
          <section className="mt-6 space-y-4">
            ...
            <StoreOverviewPanel endpoint={`${endpointBase}/overview`} />
          </section>
        ) : view === "catalog" ? (
          <section className="mt-6 space-y-4">
            ...
            <StoreProductsPanel
              endpointBase={`${endpointBase}/products`}
              categoriesEndpoint={`${endpointBase}/categories`}
          ...
          <section className="mt-6 space-y-4">
            ...
            <StoreOrdersPanel endpointBase={`${endpointBase}/orders`} storeEnabled={isStoreFeatureEnabled()} />
          </section>
        ) : view === "shipping" ? (
          <section className="mt-6 space-y-4">
            <header className="space-y-1">
            ...
            <StoreShippingSettingsPanel endpoint={`${endpointBase}/shipping/settings`} storeEnabled={isStoreFeatureEnabled()} />
          </section>
        ) : view === "marketing" ? (
          <section className="mt-6 space-y-4">
            <header className="space-y-1">
            ...
            <div className="space-y-4">
              <StoreBundlesPanel
                endpointBase={`${endpointBase}/bundles`}
                storeLocked={store.catalogLocked}
                storeEnabled={isStoreFeatureEnabled()}
              />
              <StoreBundleItemsPanel
                bundlesEndpoint={`${endpointBase}/bundles`}
                productsEndpoint={`${endpointBase}/products`}
                storeLocked={store.catalogLocked}
                storeEnabled={isStoreFeatureEnabled()}
              />
          ...
          <section className="mt-6 space-y-4">
            ...
            <StoreSettingsPanel endpoint={`${endpointBase}/settings`} storeEnabled={isStoreFeatureEnabled()} />
          </section>
        )}
      </div>
    </main>
