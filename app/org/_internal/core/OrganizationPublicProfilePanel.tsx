...
import { CTA_PRIMARY, CTA_SECONDARY, CTA_NEUTRAL } from "@/app/org/_internal/core/dashboardUi";
...
export default function OrganizationPublicProfilePanel({
  ...
  const profileHref = appendOrganizationIdToHref("/org/profile", organizationId);
  ...
  const handleAvatarUpload = async (file: File | null) => {
    ...
    try {
      ...
      const res = await fetch(`/api/upload?scope=avatar&organizationId=${organizationId}`, {
        method: "POST",
        body: formData,
      });
  ...
  const storeModuleContent = showStoreModule ? (
    <section className="rounded-3xl border border-white/12 bg-white/5 p-4 sm:p-5 shadow-[0_24px_70px_rgba(0,0,0,0.55)] backdrop-blur-2xl">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2">
        ...
        {canEdit && (
          <Link
            href={organization?.id ? `/org/${organization.id}/store?view=catalog&sub=products` : "/org/organizations"}
