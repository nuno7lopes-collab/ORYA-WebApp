...
type Props = { initialOrgs: OrgItem[]; activeId: number | null };
...
function getOrganizationDisplayName(item: OrgItem) {
  return (
    item.organization.publicName || item.organization.businessName || "Clube"
  );
}
export default function OrganizationsHubClient({
  initialOrgs,
  activeId,
}: Props) {
  ...
  const sortedOrgs = useMemo(() => {
    return [...orgs].sort((a, b) => {
      ...
      return getOrganizationDisplayName(a).localeCompare(
        getOrganizationDisplayName(b),
        "pt",
      );
    });
  }, [orgs, currentActive]);
    ...
    return Array.from(grouped.values())
      .map((bucket) => ({
        ...
        organizations: [...bucket.organizations].sort((a, b) =>
          getOrganizationDisplayName(a).localeCompare(
            getOrganizationDisplayName(b),
            "pt",
          ),
        ),
      ...
      .sort((a, b) => {
        const aHasActive = a.organizations.some(
          (item) => item.organizationId === currentActive,
        )
          ? 1
          : 0;
        const bHasActive = b.organizations.some(
          (item) => item.organizationId === currentActive,
        )
          ? 1
          : 0;
  ...
  const summary = useMemo(() => {
    const pending = groupBuckets.reduce(
      (sum, bucket) => sum + bucket.pendingJoinCount + bucket.pendingExitCount,
      0,
    );
    const actionable = groupBuckets.reduce(
      (sum, bucket) => sum + bucket.actionableCount,
      0,
    );
    return {
  ...
  const handleSwitch = async (organizationId: number) => {
    ...
    try {
      const res = await fetch(
        resolveCanonicalOrgApiPath("/api/org-hub/organizations/switch"),
        {
          method: "POST",
      ...
      try {
        ...
        console.warn(
          "[org switch] não foi possível escrever cookie no browser",
          err,
        );
      }
  ...
  const renderOrgCard = (item: OrgItem) => {
    ...
    const handle = item.organization.username
    ...
    return (
      <article
        ...
        className={cn(
          "flex min-h-[198px] flex-col justify-between rounded-3xl border p-5 transition",
          isActive
            ? "border-[#22D3EE]/55 bg-white/[0.04]"
            : "border-white/14 bg-white/[0.04] hover:border-white/22 hover:bg-white/[0.09]",
        )}
      >
        {" "}
        ...
        <div className="mt-4 flex items-center justify-between gap-3">
          ...
          <p className="text-[12px] text-white/60">
            {isActive ? "Clube ativo" : "Pronto para entrar"}
          </p>{" "}
          <button
            type="button"
            onClick={() => {
  ...
  return (
    <div
      ...
      className={cn(
        "mx-auto w-full max-w-[1240px] px-4 py-10 text-white sm:px-6 md:py-12 lg:px-8",
      )}
    >
      {" "}
      <div className="space-y-6">
          ...
          <div className="mt-4 flex flex-wrap items-start justify-between gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-white/75">
                {" "}
                Entra rapidamente no clube certo e começa a operação
                diária.{" "}
              </p>{" "}
            </div>{" "}
            <button
          ...
          <section className="space-y-4">
            ...
            {groupBuckets.map((group) => {
              const pendingOperations =
                group.pendingJoinCount + group.pendingExitCount;
              ...
              return (
                ...
                >
                  ...
                  <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                    {group.organizations.map(renderOrgCard)}
                  </div>{" "}
                </article>
              );
            })}{" "}
            <button
              ...
              className="flex min-h-[190px] w-full flex-col justify-between rounded-3xl border border-dashed border-white/24 bg-white/6 p-5 text-left transition hover:-translate-y-[2px] hover:border-[#22D3EE]/45 hover:bg-white/10"
            >
              {" "}
              <div className="space-y-3">
                {" "}
                ...
                <div>
                  ...
                  <p className="text-sm text-white/68">
                    Cria um novo clube e entra diretamente no dashboard.
                  </p>{" "}
                </div>{" "}
              </div>{" "}
        ...
        {emptyState && (
          <section className="space-y-4 rounded-3xl border border-white/12 bg-white/[0.04] p-6 ">
            {" "}
            <div className="space-y-2">
              {" "}
              <h2 className="text-2xl font-semibold">
                Ainda não tens nenhum clube
              </h2>{" "}
              <p className="text-sm text-white/70">
                Cria o teu primeiro clube para começar.
              </p>{" "}
            </div>{" "}
            <button
              type="button"
