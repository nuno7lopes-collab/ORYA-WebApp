  ...
  return {
    trainerProfile: {
      ...
      update: async (args: any) => {
        ...
        if (!row) throw new Error("TRAINER_NOT_FOUND");
...
describe("backfillTrainerProfessionalLinks", () => {
  it("dry-run não cria nem altera ligações", async () => {
    ...
    const summary = await backfillTrainerProfessionalLinks(prisma as any, {
      dryRun: true,
      limit: 50,
    });
    ...
    expect(summary.linkedTrainerProfiles).toBe(1);
  ...
  it("apply cria profissional e liga treinador; segunda execução é idempotente", async () => {
    ...
    const first = await backfillTrainerProfessionalLinks(prisma as any, {
      dryRun: false,
      limit: 50,
    });
    const second = await backfillTrainerProfessionalLinks(prisma as any, {
      dryRun: false,
      limit: 50,
    });
    ...
    expect(first.linkedTrainerProfiles).toBe(1);
    expect(second.createdProfessionals).toBe(0);
    expect(second.linkedTrainerProfiles).toBe(0);
