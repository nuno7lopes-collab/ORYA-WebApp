/**
 * Backfill trainer_profiles.reservation_professional_id and enforce canonical trainer/professional linkage.
 *
 * Usage:
 *   node -r ./scripts/load-env.js -r ts-node/register scripts/backfill_trainer_professional_links.ts --dry-run --limit=200
 *   node -r ./scripts/load-env.js -r ts-node/register scripts/backfill_trainer_professional_links.ts --apply --limit=200 --cursor=0
 */
import { PrismaClient } from "@prisma/client";
import { PrismaPg } from "@prisma/adapter-pg";
import { Pool } from "pg";
import { backfillTrainerProfessionalLinks } from "../lib/reservas/backfillTrainerProfessionalLinks";
...
async function main() {
  ...
  const totals = {
    ...
    linkedTrainerProfiles: 0,
    duplicateProfessionalGroups: 0,
    unchanged: 0,
    errors: 0,
  };

  for (let batch = 1; batch <= maxBatches; batch += 1) {
    const summary = await backfillTrainerProfessionalLinks(prisma, {
      dryRun,
      limit,
      afterId: cursor,
    ...
    totals.createdProfessionals += summary.createdProfessionals;
    totals.reactivatedProfessionals += summary.reactivatedProfessionals;
    totals.linkedTrainerProfiles += summary.linkedTrainerProfiles;
    totals.duplicateProfessionalGroups += summary.duplicateProfessionalGroups;
    ...
    console.log(
      `[trainer_prof_backfill] CHECKPOINT batch=${batch}/${maxBatches} lastId=${cursor ?? "none"} scanned=${summary.scanned} linked=${summary.linkedTrainerProfiles}`,
    );

    if (summary.scanned < summary.limit) {
      console.log("[trainer_prof_backfill] No more trainer rows to process in this run.");
      break;
    }
  }

  console.log("[trainer_prof_backfill] Totals:");
  console.log(JSON.stringify({ dryRun, nextCursor: cursor, ...totals }, null, 2));
}

main()
  .catch((err) => {
    console.error("[trainer_prof_backfill] Error:", err);
    process.exitCode = 1;
  })
  .finally(async () => {
