...
async function _GET(req: NextRequest) {
  try {
    ...
    logError("org.telemetry.funnels_list_failed", err);
...
async function _POST(req: NextRequest) {
  try {
    ...
    const created = await createTelemetryFunnelDefinition(parsed.value, access.userId);
    if (!created) {
      return jsonWrap(
        {
          ok: false,
    ...
    logError("org.telemetry.funnel_create_failed", err);
    return jsonWrap({ ok: false, error: "INTERNAL_ERROR" }, { status: 500, req });
  }
}
