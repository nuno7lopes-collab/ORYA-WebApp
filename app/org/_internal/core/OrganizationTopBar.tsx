...
export default function OrganizationTopBar({
  ...
  const normalizedPathname = useMemo(
    () => normalizeOrganizationPathname(pathname),
    [pathname],
  );
  ...
  const dashboardHref = activeOrg?.id
    ? buildOrgHref(activeOrg.id, "/overview")
  ...
  const resolvedToolSubnav = useMemo(() => {
    ...
    if (activeTool === "calendar")
      return <CalendarSubnav orgId={orgId} className="w-full max-w-full" />;
    if (activeTool === "check-in")
      return <CheckInSubnav orgId={orgId} className="w-full max-w-full" />;
    if (activeTool === "events")
      return <EventsSubnav orgId={orgId} className="w-full max-w-full" />;
    if (activeTool === "padel-tournaments") {
      return (
        <PadelTournamentsSubnav orgId={orgId} className="w-full max-w-full" />
      );
    }
    if (activeTool === "policies")
      return <PoliciesSubnav orgId={orgId} className="w-full max-w-full" />;
    if (activeTool === "finance")
      return <FinanceSubnav orgId={orgId} className="w-full max-w-full" />;
    if (activeTool === "analytics")
      return <AnalyticsSubnav orgId={orgId} className="w-full max-w-full" />;
    if (activeTool === "crm") {
      return <CrmToolSubnav orgId={orgId} className="w-full max-w-full" />;
    }
    if (activeTool === "store")
      return <StoreToolSubnav orgId={orgId} className="w-full max-w-full" />;
    if (activeTool === "forms")
      return <FormsSubnav orgId={orgId} className="w-full max-w-full" />;
    if (activeTool === "chat")
      return <ChatSubnav orgId={orgId} className="w-full max-w-full" />;
    if (activeTool === "team")
      return <TeamSubnav orgId={orgId} className="w-full max-w-full" />;
    if (activeTool === "padel-club")
      return <PadelClubSubnav orgId={orgId} className="w-full max-w-full" />;
    if (activeTool === "marketing")
      return <MarketingSubnav orgId={orgId} className="w-full max-w-full" />;
    if (activeTool === "settings")
      return <SettingsSubnav orgId={orgId} className="w-full max-w-full" />;
    return null;
  ...
  const {
    data: orgData,
    error: orgDataError,
    mutate: mutateOrgData,
  } = useSWR<OrganizationMeResponse>(orgMeUrl, fetcher);
  const roleBadge =
    role && ROLE_BADGE_ROLE_SET.has(role as RoleBadgeRole)
  ...
  const shouldAutoRefreshOrg = useMemo(() => {
    ...
    const officialEmailNormalized = normalizeOfficialEmail(
      orgData.organization?.officialEmail ?? null,
    );
    const emailVerified = Boolean(
      officialEmailNormalized && orgData.organization?.officialEmailVerifiedAt,
    );
    const pendingEmailNormalized = normalizeOfficialEmail(
      orgData.organization?.officialEmailPending?.newEmail ?? null,
    );
    ...
    const paymentsReady =
      paymentsMode === "PLATFORM" || paymentsStatus === "READY";
  ...
  useEffect(() => {
    ...
    const syncTopbarHeight = () => {
      ...
      const nextHeight = Math.min(
        TOPBAR_MAX_RENDERED_HEIGHT,
        Math.max(TOPBAR_MIN_RENDERED_HEIGHT, measuredHeight),
      );
  ...
  const activationItems = useMemo(() => {
    ...
    const officialEmailVerifiedAt =
      orgData.organization?.officialEmailVerifiedAt ?? null;
    const officialEmail = normalizeOfficialEmail(
      orgData.organization?.officialEmail ?? null,
    );
    ...
    if (!emailVerified) {
      items.push({
        ...
        href: activeOrg?.id
          ? buildOrgHref(activeOrg.id, "/settings")
          : buildOrgHubHref("/organizations"),
        tone: "danger",
      });
    ...
    if (
      ...
      items.push({
        ...
        href: activeOrg?.id
          ? buildOrgHref(activeOrg.id, "/finance")
          : buildOrgHubHref("/organizations"),
        tone: "warning",
      });
    }
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      const sameDirection =
        ...
        Math.sign(currentDirectionDelta) === Math.sign(delta);
      const nextDirectionDelta = sameDirection
        ? currentDirectionDelta + delta
        : delta;
  ...
  const handleMenuToggle =
    (key: "org" | "user") => (event: SyntheticEvent<HTMLDetailsElement>) => {
    ...
    >
      ...
      <div
        className={cn(
          ...
          isAtTop
            ? "border-white/18 bg-[#080c14]"
            : "border-white/22 bg-[#070b13]",
        )}
      >
        {" "}
        <div
        ...
        >
          ...
          <div className="flex min-w-0 items-center gap-2">
            ...
            <Link
              href={dashboardHref}
              className="group flex h-10 min-w-0 shrink-0 items-center gap-2.5 rounded-full border border-white/22 bg-black/25 px-3.5 text-sm text-white transition hover:border-[#22D3EE]/45 hover:bg-[#22D3EE]/12"
          ...
          <div className="order-2 ml-auto flex items-center gap-2 lg:order-none">
            ...
            <details
              ref={orgMenuRef}
              className={cn("relative", openMenu === "org" && "z-50")}
              onToggle={handleMenuToggle("org")}
            >
              {" "}
              <summary className="list-none cursor-pointer rounded-full border border-white/22 bg-black/25 px-3 text-sm text-white/90 flex h-11 items-center">
                {" "}
              ...
              <div className="absolute right-0 mt-2 w-64 rounded-2xl orya-menu-surface p-2">
                {" "}
                <p className="px-2 pb-2 text-[10px] uppercase tracking-[0.24em] text-white/50">
                  Clubes
                </p>{" "}
            ...
            <details
              ref={userMenuRef}
              className={cn("relative", openMenu === "user" && "z-50")}
              onToggle={handleMenuToggle("user")}
            >
              {" "}
              <summary className="list-none cursor-pointer rounded-full border border-white/22 bg-black/25 px-2.5 text-sm text-white/90 flex h-11 items-center">
                {" "}
                <div className="flex items-center gap-2">
                  {" "}
                  {roleBadge && (
              ...
              <div className="absolute right-0 mt-2 w-56 rounded-2xl orya-menu-surface p-2">
                {" "}
                <p className="px-2 pb-2 text-[10px] uppercase tracking-[0.24em] text-white/50">
                  Conta
                </p>{" "}
