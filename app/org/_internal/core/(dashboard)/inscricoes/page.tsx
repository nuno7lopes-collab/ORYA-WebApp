    ...
    <div className={wrapperClass}>
      ...
      {activeSection === "responses" && (
        <section className="space-y-4">
          <div className="rounded-2xl border border-white/12 bg-white/5 px-4 py-3 text-[12px] text-white/70">
            Estás a ver respostas da organização ativa neste painel. Cada organização mantém respostas separadas.
          </div>
          <div className="grid gap-3 md:grid-cols-3">
            <div className="rounded-2xl border border-white/12 bg-white/5 p-4">
              <p className="text-[11px] uppercase tracking-[0.2em] text-white/50">Respostas totais</p>
              <p className="text-2xl font-semibold text-white">{totalResponses}</p>
              <p className="text-[12px] text-white/60">Em {formsWithResponses.length} formulário(s).</p>
            </div>
            <div className="rounded-2xl border border-white/12 bg-white/5 p-4">
              <p className="text-[11px] uppercase tracking-[0.2em] text-white/50">Formulários ativos</p>
              <p className="text-2xl font-semibold text-white">{viewCounts.ativos}</p>
              <p className="text-[12px] text-white/60">Publicados e a aceitar respostas.</p>
            </div>
            <div className="rounded-2xl border border-white/12 bg-white/5 p-4">
              <p className="text-[11px] uppercase tracking-[0.2em] text-white/50">Ação rápida</p>
              <Link
          ...
          {loadingForms && <p className="text-sm text-white/60">A carregar respostas...</p>}
          {loadError && <p className="text-sm text-red-300">{loadError}</p>}
          ...
          {!loadingForms && !loadError && formsWithResponses.length > 0 && (
            <div className="space-y-3">
              {formsWithResponses.map((form) => (
                <div
                  key={form.id}
                  className="rounded-2xl border border-white/12 bg-gradient-to-br from-white/8 via-[#0b1124]/70 to-[#050810]/90 p-4 shadow-[0_20px_60px_rgba(0,0,0,0.5)] backdrop-blur-2xl"
                >
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="space-y-1">
                      <h3 className="text-base font-semibold text-white">{form.title}</h3>
                      <p className="text-[12px] text-white/60">
                        {form.submissionsCount} resposta{form.submissionsCount === 1 ? "" : "s"} · {statusLabel[form.status]}
                      </p>
                    </div>
                    <div className="flex flex-wrap gap-2 text-[12px]">
                      ...
                      <Link
                        href={
                          organizationId
                            ? buildOrgHref(organizationId, `/forms/${form.id}`, { tab: "respostas", respostas: "exportar" })
                            : buildOrgHubHref("/organizations")
                        }
                        className="rounded-full border border-white/20 px-3 py-1 text-white/80 hover:bg-white/10"
                      >
      ...
      {activeSection === "settings" && (
        <section className="space-y-4">
          <div className="grid gap-3 md:grid-cols-3">
            <div className="rounded-2xl border border-white/12 bg-white/5 p-4">
              <p className="text-[11px] uppercase tracking-[0.2em] text-white/50">Rascunhos</p>
              <p className="text-2xl font-semibold text-white">{viewCounts.rascunhos}</p>
              <Link href={buildViewHref("rascunhos")} className="text-[12px] text-white/70 hover:text-white">
                Rever rascunhos
              </Link>
            </div>
            <div className="rounded-2xl border border-white/12 bg-white/5 p-4">
              <p className="text-[11px] uppercase tracking-[0.2em] text-white/50">Sem datas</p>
              <p className="text-2xl font-semibold text-white">{formsWithoutDates}</p>
              <p className="text-[12px] text-white/60">Formulários sem janela temporal.</p>
            </div>
            <div className="rounded-2xl border border-white/12 bg-white/5 p-4">
              <p className="text-[11px] uppercase tracking-[0.2em] text-white/50">Ativos sem limite</p>
              <p className="text-2xl font-semibold text-white">{publishedWithoutCapacity}</p>
              <p className="text-[12px] text-white/60">Avalia se precisas de capacidade.</p>
            </div>
          </div>

          <div className="rounded-3xl border border-white/12 bg-gradient-to-br from-white/8 via-[#0b1124]/70 to-[#050810]/90 p-5 shadow-[0_24px_80px_rgba(0,0,0,0.55)] backdrop-blur-2xl space-y-3">
            <h2 className="text-lg font-semibold">Checklist de qualidade</h2>
            <p className="text-[13px] text-white/70">
              Mantém títulos claros, descrição objetiva, datas definidas e revisão de respostas frequente.
            </p>
            <div className="grid gap-2 text-[12px] text-white/70 md:grid-cols-2">
              <p>1. Define sempre objetivo e público no título/descrição.</p>
