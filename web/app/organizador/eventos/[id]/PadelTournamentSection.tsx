...
const formatOptions: Array<{ value: PadelFormat; label: string }> = [
  { value: "QUADRO_ELIMINATORIO", label: "KO" },
  { value: "GRUPOS_ELIMINATORIAS", label: "Grupos + KO" },
  { value: "TODOS_CONTRA_TODOS", label: "Liga" },
];
...
export default function PadelTournamentSection({ eventId, organizerId }: Props) {
  ...
  const formatLabelMap = useMemo(
    () => ({
      QUADRO_ELIMINATORIO: "KO",
      GRUPOS_ELIMINATORIAS: "Grupos + KO",
      TODOS_CONTRA_TODOS: "Liga",
      CAMPEONATO_LIGA: "Liga",
      NON_STOP: "Liga",
      QUADRO_AB: "KO",
    }),
    [],
  ...
  const configFormatRaw = (configRes?.config?.format as PadelFormat | undefined) ?? formatEffective ?? formatRequested ?? null;

  const normalizePadelFormat = (value?: PadelFormat | string | null): PadelFormat => {
    if (!value) return "TODOS_CONTRA_TODOS";
    const normalized = String(value).toUpperCase();
    if (["KO", "QUADRO_ELIMINATORIO"].includes(normalized)) return "QUADRO_ELIMINATORIO";
    if (["GROUPS_KO", "GRUPOS_ELIMINATORIAS"].includes(normalized)) return "GRUPOS_ELIMINATORIAS";
    if (["LEAGUE", "TODOS_CONTRA_TODOS", "CAMPEONATO_LIGA", "NON_STOP"].includes(normalized)) return "TODOS_CONTRA_TODOS";
    if (["QUADRO_AB"].includes(normalized)) return "QUADRO_ELIMINATORIO";
    return "TODOS_CONTRA_TODOS";
  };

  useEffect(() => {
    if (formatDirtyRef.current) return;
    if (!configFormatRaw) return;
    setFormat(normalizePadelFormat(configFormatRaw));
  }, [configFormatRaw]);
