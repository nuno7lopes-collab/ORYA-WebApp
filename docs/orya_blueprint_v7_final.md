ORYA — Blueprint Final v7 (SSOT)

**Versão:** v7.0  
**Data:** 23 Jan 2026 (Europe/Lisbon)  
**Estado:** Fechado para execução (Fase 1–3)  
**Autoridade (SSOT):** Este documento substitui **todas** as versões anteriores (v3.x, v4, v5) e é a única fonte normativa.  
...
## CHANGELOG (v7.0) — correções finais anti-drift
...
- Editorial: numeração 15.1 corrigida + referência “blueprint” alinhada para v7.
...
Nota de produto: Padel é um vertical inicial forte, mas o blueprint v7 garante que Eventos/Serviços/Digital são igualmente first-class.

⸻


4) Decisions Locked v7 (não avançar sem isto)
...
**Nota de integração:** este anexo detalha o vertical Padel. Tudo o que for pagamento, identidade, entitlements, refunds, RBAC, endereços e notificações **obedece** às decisões e SSOTs do blueprint principal (v7). Se houver conflito, vence o blueprint.
...
**Regra de hierarquia (obrigatória):** este anexo é subordinado ao **ORYA — Blueprint Final v7 (SSOT)**.
Em caso de conflito, vence o **v7**.
O Padel define apenas domínio/UX; owners e contratos permanecem os do v7.
...
- **SSOT v7:** Financas/Reservas/Check-in/CRM/RBAC/Address seguem o v7; Padel nunca duplica logica.
...
- **SSOT em Reservas (v7):** Padel consome a Agenda Engine via contrato, nao cria agenda propria.
...
- RBAC centralizado (v7) com **Role Packs**: CLUB_MANAGER, TOURNAMENT_DIRECTOR, FRONT_DESK, COACH, REFEREE.
...
- **MVP (v7):** eliminacao simples, grupos+eliminacao, round-robin.
...
**Regra D1 (v7):** todo torneio Padel tem **eventId obrigatorio** (Eventos e owner da base).
...
**Prioridade v7 (D3/D3.1):** HardBlock > MatchSlot > Booking > SoftBlock, e **MatchSlot e sempre hard-block**.
...
**Owner:** Reservas. Politicas sao aplicadas via Reservas e **snapshot imutavel** por booking (v7).
...
**Regra v7:** transicoes publicam EventLog e aplicam politica de refund versionada (Financas).
...
| Waitlist auto-promoção | Parcial | F2 | Em v7, waitlist é Fase 2. |
...
**Pre-requisito (v7 F1-A):** EventBus/EventLog, Financas como gateway, Address Service, RBAC v2, contratos base concluídos.

**Fase 0 — Alinhamento e Fundação**
- **Confirmar e fixar implementação do que já está FECHADO no v7** (sem reabrir decisões).
- Definir taxonomias e SSOT (clubes, courts, staff, categorias).
- Padrões de UX, acessibilidade e logs alinhados com v7.
...
**Fase 1 — MVP Premium (Padel alinhado com v7 F1-B/C)**
...
**Regra v7:** owner = Reservas; MatchSlot e sempre hard-block (D3/D3.1).
...
**Decisão (alinhada com v7):** MVP com formatos base; Americano/Mexicano entra em Fase 2.
...
### 10.5 Check-in
**Decisão:** Check-in via **Entitlement + Check-in Policy** (v7).
