...
async function _GET(req: NextRequest) {
  try {
    ...
    logError("admin.telemetry.rules_list_failed", err);
...
async function _POST(req: NextRequest) {
  try {
    ...
    const created = await createTelemetryAlertRule(parsed.value, admin.userId);
    if (!created) {
      return jsonWrap(
        {
          ok: false,
    ...
    logError("admin.telemetry.rule_create_failed", err);
    return jsonWrap({ ok: false, error: "INTERNAL_ERROR" }, { status: 500, req });
  }
}
