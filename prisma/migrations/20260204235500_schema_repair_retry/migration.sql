-- Schema repair (retry): APPLE_MAPS + env isolation + address SSOT

-- Location source enum update (create if missing)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_type t
    JOIN pg_namespace n ON n.oid = t.typnamespace
    WHERE n.nspname = 'app_v3' AND t.typname = 'location_source'
  ) THEN
    CREATE TYPE app_v3.location_source AS ENUM (
      'OSM',
      'MANUAL',
      'APPLE_MAPS'
    );
