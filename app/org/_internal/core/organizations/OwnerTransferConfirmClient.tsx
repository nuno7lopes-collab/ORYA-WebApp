...
type Props = { groupId: number; token: string };
...
export default function OwnerTransferConfirmClient({ groupId, token }: Props) {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        ...
        if (!cancelled) {
          setState("ok");
          setMessage("Transferência de owner confirmada.");
        }
      } catch (err) {
        if (!cancelled) {
          setState("error");
    ...
    <div className="mx-auto w-full max-w-2xl px-4 py-16 text-white md:px-6">
      ...
      <section className="rounded-3xl border border-white/12 bg-white/[0.04] p-6 ">
        ...
        <h1 className="mt-1 text-2xl font-semibold">
          Confirmação de transferência de owner
        </h1>{" "}
        <p className="mt-2 text-sm text-white/75">
          {" "}
          {state === "loading"
            ? "A confirmar a transferência..."
            : state === "ok"
              ? "A transferência foi aplicada ao grupo."
              : "Não foi possível confirmar a transferência."}{" "}
        </p>{" "}
        {message && (
          <div
            className={`mt-4 rounded-2xl border px-4 py-3 text-sm ${state === "ok" ? "border-emerald-300/40 bg-emerald-400/12 text-emerald-50" : state === "error" ? "border-red-300/35 bg-red-500/10 text-red-100" : "border-white/14 bg-white/6 text-white/80"}`}
          >
            {" "}
            {message}{" "}
          </div>
