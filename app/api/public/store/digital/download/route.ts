...
async function issueSignedUrl(params: { grantId: number; assetId: number; productId: number }) {
  ...
  if (signed.error || !signed.data?.signedUrl) {
    console.error("/api/public/store/digital/download signed url error", signed.error);
...
async function _GET(req: NextRequest) {
  try {
    ...
    console.error("GET /api/public/store/digital/download error:", err);
...
async function _POST(req: NextRequest) {
  try {
    ...
    console.error("POST /api/public/store/digital/download error:", err);
    return jsonWrap({ ok: false, error: "Erro ao gerar download." }, { status: 500 });
  }
}
