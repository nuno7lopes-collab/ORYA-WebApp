...
const CRM_NAV = [
  ...
  { id: "segmentos", label: "Segmentos", href: "/org/crm/segments" },
  {
    id: "campanhas",
    label: "Campanhas",
    href: "/org/crm/campaigns",
    feature: "CRM_CAMPAIGNS" as const,
  },
...
export default function CrmSubnav({
  ...
  const wrapperClass = cn(
    isTopbar
      ? "relative w-full max-w-full rounded-full border border-white/12 bg-white/5 px-1 py-1 text-[12px] overflow-hidden"
      : "rounded-2xl border border-white/10 bg-white/5 px-2 py-2",
    className,
  );
  const listClass = cn(
    "flex items-center",
    isTopbar ? "flex-nowrap gap-1" : "flex-wrap gap-2",
  );
