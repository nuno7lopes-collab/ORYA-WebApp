...
import { CTA_PRIMARY, CTA_SECONDARY, DASHBOARD_CARD, DASHBOARD_LABEL, DASHBOARD_MUTED } from "@/app/organizador/dashboardUi";
...
export default function NovoServicoPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch("/api/organizador/servicos", {
        method: "POST",
      ...
      router.push(`/organizador/reservas/${json.service.id}`);
    } catch (err) {
      ...
      <section className={cn(DASHBOARD_CARD, "p-5 space-y-4")}> 
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <button
            ...
            onClick={() => router.push("/organizador/reservas")}
          >
            Cancelar
          </button>
        </div>
