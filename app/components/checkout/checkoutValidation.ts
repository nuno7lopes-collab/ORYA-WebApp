import { isValidPhone, normalizePhone, sanitizePhone } from "@/lib/phone";
...
export function validateGuestDetails(
  ...
  options?: { phoneLocale?: string | null },
  ...
  const phoneNormalized =
    phoneSanitized && isValidPhone(phoneSanitized)
      ? normalizePhone(phoneSanitized, { defaultLocale: options?.phoneLocale ?? null })
      : "";
  ...
  if (phoneSanitized && !phoneNormalized) {
    errors.phone =
  ...
  return {
    errors,
    normalized: { name, email, phone: phoneNormalized || undefined },
