...
async function _POST(req: NextRequest) {
  ...
  try {
    ...
    const ipAllowlist = resolveInfraIpAllowlist("REDIS_STOP");
    ...
    const result = await runScript(ctx, "aws/redis-stop.sh", [], { APP_ENV: targetEnv });
    await auditInfraAction(ctx, admin, "ADMIN_INFRA_REDIS_STOP", { targetEnv, ok: result.ok });

    if (!result.ok) return fail(ctx, 500, "INFRA_REDIS_STOP_FAILED", trim(result.stderr));

    return respondOk(ctx, { action: "redis_stop", stdout: trim(result.stdout), stderr: trim(result.stderr) });
  } catch (err) {
    logError("admin.infra.redis_stop_failed", err);
