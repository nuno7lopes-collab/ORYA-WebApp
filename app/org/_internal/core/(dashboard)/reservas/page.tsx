...
const CHIP_BASE =
  "rounded-full border border-white/10 bg-white/[0.04] px-3 py-1 text-[12px] text-white/65 transition hover:border-white/20 hover:bg-white/10 hover:text-white";
const CHIP_ACTIVE = "border-white/35 bg-white/18 text-white";
...
const isPendingBooking = (status: string) =>
  ["PENDING_CONFIRMATION", "PENDING"].includes(status);
const isConfirmedBooking = (status: string) =>
...
const getBookingMode = (booking: BookingItem): ReservationFilterMode => {
  const serviceKind =
    typeof booking.service?.kind === "string"
      ? booking.service.kind.trim().toUpperCase()
      : "";
  if (serviceKind && serviceKind !== "COURT" && !booking.resource)
    return "PROFESSIONAL";
  const normalized =
    typeof booking.assignmentMode === "string"
      ? booking.assignmentMode.toUpperCase()
      : "";
  if (
    ["RESOURCE", "RESOURCE_ONLY", "PROFESSIONAL_AND_RESOURCE"].includes(
      normalized,
    )
  )
    return "RESOURCE";
...
type PadelCourtSummary = { id: number; name: string; isActive?: boolean };
type ClientItem = {
  id: string;
  fullName: string | null;
  username: string | null;
...
export default function ReservasDashboardPage() {
  ...
  const participantsKey = drawerBooking
    ? resolveCanonicalOrgApiPath(
        `/api/org/[orgId]/reservas/${drawerBooking.id}/participants`,
      )
    : null;
  const splitKey =
  ...
  const { data: splitData, mutate: mutateSplit } = useSWR<any>(
    splitKey,
    fetcher,
  );
  ...
  const [createAddressLabel, setCreateAddressLabel] = useState<string | null>(
    null,
  );
  ...
  const bookingConfigKey =
    organizationId && Number.isFinite(organizationId)
      ...
      : null;
  const { data: bookingConfigData } = useSWR<BookingConfigResponse>(
    bookingConfigKey,
    fetcher,
  );
  const padelClubsKey =
    organizationId && Number.isFinite(organizationId)
      ...
      : null;
  const { data: padelClubsData } = useSWR<{
    ok: boolean;
    items?: PadelClubSummary[];
  }>(padelClubsKey, fetcher);
  ...
  const padelCourtsKey =
    selectedPadelClubId && Number.isFinite(selectedPadelClubId)
      ...
      : null;
  const { data: padelCourtsData } = useSWR<{
    ok: boolean;
    items?: PadelCourtSummary[];
  }>(padelCourtsKey, fetcher);
  ...
  useEffect(() => {
    ...
    if (
      selectedPadelClubId &&
      padelClubs.some((club) => club.id === selectedPadelClubId)
    )
      return;
  ...
  const hasNonCourtServices = services.some(
    (service) => service.kind !== "COURT",
  );
  ...
  const bookingGridMinutes =
    typeof bookingConfigData?.data?.gridMinutes === "number" &&
    Number.isFinite(bookingConfigData.data.gridMinutes)
      ? Math.max(5, bookingConfigData.data.gridMinutes)
      : 30;
  const bookingTimeStepMinutes = normalizeStepMinutes(bookingGridMinutes);
  const assignmentMode: ReservationFilterMode =
    requiresResourceForAssignmentMode(organizationAssignmentMode)
      ? "RESOURCE"
      : "PROFESSIONAL";
  const canFilterByResource =
    requiresResourceForAssignmentMode(organizationAssignmentMode) &&
    hasCourtServices;
  const canFilterByProfessional =
    requiresProfessionalForAssignmentMode(organizationAssignmentMode) ||
    hasNonCourtServices;
  ...
  const [filterMode, setFilterMode] = useState<ReservationFilterMode>(() =>
    canFilterByResource && assignmentMode === "RESOURCE"
      ? "RESOURCE"
      : "PROFESSIONAL",
  );
  const createAssignmentConfig = useMemo(
    ...
    [
      organizationAssignmentMode,
      selectedCreateService?.assignmentMode,
      selectedCreateService?.kind,
    ],
  );
    ...
    () =>
      new Set(
        (orgData?.organization?.tools ?? [])
          .map((tool) =>
            typeof tool === "string" ? tool.trim().toUpperCase() : "",
          )
          .filter(Boolean),
      ),
    [orgData?.organization?.tools],
  ...
  useEffect(() => {
    ...
    const splitParticipants = Array.isArray(split?.participants)
      ? split.participants
      : [];
    const baseParticipants: SplitParticipantForm[] = inviteItems.length
      ? inviteItems.map((invite) => {
          const label =
            invite.targetName || invite.targetContact || "Convidado";
          const splitParticipant = splitParticipants.find(
            (item: any) => item.inviteId === invite.id,
          );
          const baseShare = splitParticipant?.baseShareCents ?? 0;
          const percentValue =
            baseTotal > 0
              ? ((baseShare / baseTotal) * 100).toFixed(2).replace(/\.00$/, "")
              : "";
          return {
            inviteId: invite.id,
            label,
      ...
      : splitParticipants
          ...
          .map((participant: any) => ({
            ...
            include: true,
            amount: participant.baseShareCents
              ? formatCentsInput(participant.baseShareCents)
              : "",
            percent:
              baseTotal > 0 && participant.baseShareCents
                ? ((participant.baseShareCents / baseTotal) * 100)
                    .toFixed(2)
                    .replace(/\.00$/, "")
                : "",
    ...
    if (!split && included.length > 0 && baseTotal > 0) {
      ...
      included.forEach((participant, idx) => {
        ...
        participant.percent =
          baseTotal > 0
            ? (((distributed[idx] ?? 0) / baseTotal) * 100)
                .toFixed(2)
                .replace(/\.00$/, "")
            : "";
      });
  ...
  useEffect(() => {
    ...
    const parsedProfessionalId = professionalIdRaw
      ? Number(professionalIdRaw)
      : NaN;
    ...
    const handle = window.setTimeout(async () => {
      ...
      try {
        const res = await fetch(
          resolveCanonicalOrgApiPath(
            `/api/org/[orgId]/reservas/clientes?q=${encodeURIComponent(query)}`,
          ),
  ...
  const padelClubQuery = selectedPadelClubId
    ? `&padelClubId=${selectedPadelClubId}`
    : "";
  const padelCourtQuery = selectedPadelCourtId
    ? `&courtId=${selectedPadelCourtId}`
    : "";
  const upcomingRange = useMemo(() => {
    ...
    const end = buildZonedDate(
      addDaysToParts(todayParts, operationalWindowDays),
      timezone,
      0,
      0,
    );
  ...
  } = useSWR<{ ok: boolean; items: BookingItem[] }>(
    !requiresPadelClubSelection
      ? resolveCanonicalOrgApiPath(
          `/api/org/[orgId]/reservas?from=${encodeURIComponent(upcomingRange.start.toISOString())}&to=${encodeURIComponent(upcomingRange.end.toISOString())}${padelClubQuery}${padelCourtQuery}`,
        )
      : null,
    fetcher,
  );
  ...
  const selectedServiceProfessionalIds =
    selectedCreateService?.professionalLinks?.map(
      (link) => link.professionalId,
    ) ?? [];
  const selectedServiceResourceIds =
    selectedCreateService?.resourceLinks?.map((link) => link.resourceId) ?? [];
  ...
  const availableProfessionalsForService = hasServiceProfessionalLinks
    ? activeProfessionals.filter((professional) =>
        selectedServiceProfessionalIds.includes(professional.id),
      )
    : activeProfessionals;
  const availableResourcesForService = hasServiceResourceLinks
    ? activeResources.filter((resource) =>
        selectedServiceResourceIds.includes(resource.id),
      )
    : activeResources;
  const delayScope = useMemo(() => {
    ...
    if (filterMode === "PROFESSIONAL" && selectedProfessionalId) {
      ...
      return {
        ...
        scopeId: selectedProfessionalId,
        label,
      };
    }
  ...
  const delayKey =
    organizationId && Number.isFinite(organizationId)
      ? resolveCanonicalOrgApiPath(
          `/api/org/[orgId]/reservas/delays?scopeType=${delayScope.scopeType}&scopeId=${delayScope.scopeId}&organizationId=${organizationId}`,
        )
      : null;
  const { data: delayData, mutate: mutateDelay } = useSWR<{
    ok: boolean;
    delay: {
      id: number;
      delayMinutes: number;
  ...
  useEffect(() => {
    ...
    if (createAssignmentMode === "PROFESSIONAL") {
      if (
        ...
        !availableProfessionalsForService.some(
          (professional) => professional.id === createProfessionalId,
        )
      ) {
    ...
    if (createAssignmentMode === "RESOURCE") {
      if (
        createResourceId &&
        !availableResourcesForService.some(
          (resource) => resource.id === createResourceId,
        )
      ) {
  ...
  useEffect(() => {
    ...
    const selectedResource = availableResourcesForService.find(
      (resource) => resource.id === createResourceId,
    );
  ...
  const operationalBookings = useMemo(() => {
    ...
    return items.sort(
      (a, b) => getOperationalStartMs(a) - getOperationalStartMs(b),
    );
  }, [
    filterMode,
    selectedProfessionalId,
  ...
  const filteredUpcomingBookings = useMemo(
    () =>
      operationalBookings.filter((booking) =>
        matchesQueueStatus(booking, queueStatusFilter),
      ),
  ...
  const pendingBookings = operationalBookings.filter((booking) =>
    isPendingBooking(booking.status),
  ).length;
  const confirmedBookings = operationalBookings.filter((booking) =>
    isConfirmedBooking(booking.status),
  ).length;
  const delayedBookings = operationalBookings.filter((booking) =>
    isDelayedBooking(booking),
  ).length;
  const confirmedRevenueCents = operationalBookings.reduce(
    (sum, booking) =>
      isConfirmedBooking(booking.status) ? sum + booking.price : sum,
    0,
  );
  ...
  const operationsGuidance = useMemo(() => {
    if (operationalMode === "HYBRID") {
      return {
        ...
        body: "Esta área controla aulas por slots. Eventos e torneios pontuais continuam no fluxo próprio.",
      };
    }
    return {
      badge: "Modo reservas",
      title: "Operação diária de reservas",
      body: "Usa esta fila para confirmar, reagendar e resolver exceções. A disponibilidade define a base dos slots vendidos.",
    };
  }, [operationalMode]);
  const drawerBookingClosed = drawerBooking
    ? [
        ...
        "DISPUTED",
        "NO_SHOW",
      ].includes(drawerBooking.status)
    : false;
  const drawerBookingStarted = drawerBooking
    ? new Date(drawerBooking.startsAt) <= new Date()
    : false;
  const canMarkNoShow =
    !!drawerBooking &&
    !drawerBookingClosed &&
    ["CONFIRMED", "PENDING_CONFIRMATION", "PENDING"].includes(
  ...
  const participantSummary = useMemo(() => {
    ...
    if (!drawerBooking) return fallback;
    if (drawerBooking.participantSummary)
      return drawerBooking.participantSummary;
  ...
  const splitSummary = useMemo(() => {
    ...
    if (included.length === 0) {
      return {
        valid: false,
        message: "Seleciona pelo menos um convidado.",
        diffLabel: "",
      };
    }
    if (splitState.pricingMode === "FIXED") {
      ...
      if (!fixedCents || fixedCents <= 0) {
        return {
          valid: false,
          message: "Indica o preço por pessoa.",
          diffLabel: "",
        };
      }
      ...
      return {
        valid: diff === 0,
        message:
          diff === 0 ? null : "O total não coincide com o valor da reserva.",
        diffLabel:
          diff === 0
            ? "Total certo"
            : `Diferença: ${formatCurrency(diff, splitState.currency)}`,
      };
    }
    if (splitState.dynamicMode === "PERCENT") {
      ...
      if (bpsValues.some((value) => value == null)) {
        return {
          valid: false,
          message: "Indica todas as percentagens.",
          diffLabel: "",
        };
      }
      const sum = (bpsValues as number[]).reduce(
        (acc, value) => acc + value,
        0,
      );
    ...
    if (amounts.some((value) => value == null)) {
      return {
        valid: false,
        message: "Indica os valores de todos os convidados.",
        diffLabel: "",
      };
    }
  ...
  const splitLocked =
    drawerBookingClosed ||
    (splitState
      ? splitState.paidCents > 0 || splitState.status === "SETTLED"
      : false);
  const handleModeChange = async (
    mode: "PROFESSIONAL_ONLY" | "RESOURCE_ONLY",
  ) => {
    ...
    try {
      ...
      if (!res.ok || !json?.ok) {
        throw new Error(
          json?.message || json?.error || "Erro ao atualizar modo.",
        );
      }
      ...
      const message =
        err instanceof Error ? err.message : "Erro ao atualizar modo.";
      alert(message);
    } finally {
      setModeSaving(null);
    }
  ...
  const handleCancel = async (bookingId: number) => {
    ...
    const confirmed = window.confirm(
      "Cancelar reserva? O reembolso segue a politica definida.",
    );
    if (!confirmed) return;
    ...
    try {
      const res = await fetch(
        resolveCanonicalOrgApiPath(
          `/api/org/[orgId]/reservas/${bookingId}/cancel`,
        ),
      ...
      if (!res.ok || !json?.ok) {
        throw new Error(
          json?.message || json?.error || "Erro ao cancelar reserva.",
        );
      }
      mutateUpcoming();
      if (json.booking) {
        setDrawerBooking((prev) =>
          prev ? { ...prev, ...json.booking } : prev,
        );
      }
    } catch (err) {
      const message =
  ...
  const handleReschedule = async () => {
    ...
    const [year, month, day] = rescheduleDate
      .split("-")
      .map((part) => Number(part));
    const [hour, minute] = rescheduleTime
      .split(":")
      .map((part) => Number(part));
    if (
      !year ||
    ...
    try {
      const res = await fetch(
        resolveCanonicalOrgApiPath(
          `/api/org/[orgId]/reservas/${drawerBooking.id}/reschedule`,
        ),
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      if (!res.ok || !json?.ok) {
        throw new Error(
          json?.message || json?.error || "Erro ao reagendar reserva.",
        );
      }
      mutateUpcoming();
      if (json.request) {
        ...
        setDrawerBooking((prev) =>
          prev ? { ...prev, changeRequest: json.request } : prev,
        );
        setRescheduleDate("");
        setRescheduleTime("");
      ...
      setRescheduleError(
        err instanceof Error ? err.message : "Erro ao reagendar reserva.",
      );
    } finally {
    ...
    try {
      const url =
        organizationId && Number.isFinite(organizationId)
          ...
          : resolveCanonicalOrgApiPath(
              `/api/org/[orgId]/reservas/${drawerBooking.id}/invites`,
            );
      ...
      if (!res.ok || !json?.ok) {
        throw new Error(
          json?.message || json?.error || "Erro ao enviar convite.",
        );
      }
      setInviteName("");
      setInviteContact("");
  ...
  const updateSplitParticipant = (
    inviteId: number,
    patch: Partial<SplitParticipantForm>,
  ) => {
  ...
  const applyEqualSplit = () => {
    ...
    setSplitState((prev) => {
      ...
      if (prev.pricingMode === "FIXED") {
        ...
        return {
          ...prev,
          fixedShare: formatCentsInput(prev.totalCents / included.length),
        };
      }
      if (prev.dynamicMode === "AMOUNT") {
        ...
        const updated = prev.participants.map((participant) => {
          const idx = included.findIndex(
            (p) => p.inviteId === participant.inviteId,
          );
          if (idx < 0) return participant;
          return {
            ...participant,
            amount: formatCentsInput(distributed[idx] ?? 0),
          };
        });
      ...
      const updated = prev.participants.map((participant) => {
        ...
        return {
          ...participant,
          percent: value.toFixed(2).replace(/\.00$/, ""),
        };
      });
      return { ...prev, participants: updated };
    });
  ...
  const saveSplit = async () => {
    ...
    if (!organizationId || Number.isNaN(organizationId)) {
      setSplitState((prev) =>
        prev ? { ...prev, error: "Seleciona uma organização primeiro." } : prev,
      );
      return;
    }
    ...
    if (!splitSummary?.valid) {
      setSplitState((prev) =>
        prev ? { ...prev, error: "Revê os valores do split." } : prev,
      );
      return;
    }
    const included = splitState.participants.filter((p) => p.include);
    if (included.length === 0) {
      setSplitState((prev) =>
        prev ? { ...prev, error: "Seleciona pelo menos um convidado." } : prev,
      );
      return;
    }
    const fixedCents =
      splitState.pricingMode === "FIXED"
        ? parseAmountToCents(splitState.fixedShare)
        : null;
    if (
      splitState.pricingMode === "FIXED" &&
      ...
      setSplitState((prev) =>
        prev ? { ...prev, error: "Indica o preço por pessoa." } : prev,
      );
      return;
    }
    ...
    try {
          ...
          body: JSON.stringify({
            ...
            dynamicMode:
              splitState.pricingMode === "DYNAMIC"
                ? splitState.dynamicMode
                : undefined,
            deadlineAt: splitState.deadlineAt
              ? new Date(splitState.deadlineAt).toISOString()
              : null,
      ...
      if (!res.ok || !json?.ok) {
        throw new Error(
          json?.message ||
            json?.error ||
            "Erro ao configurar pagamento dividido.",
        );
      }
      setSplitState((prev) =>
        prev ? { ...prev, saving: false, error: null } : prev,
      );
  ...
  const handleDelaySave = async (overrideMinutes?: number) => {
    ...
    try {
      ...
      const res = await fetch(
        resolveCanonicalOrgApiPath("/api/org/[orgId]/reservas/delays"),
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      if (!res.ok || !json?.delay) {
        throw new Error(
          json?.message || json?.error || "Erro ao atualizar atraso.",
        );
      }
      await mutateDelay();
      ...
      setDelayError(
        err instanceof Error ? err.message : "Erro ao atualizar atraso.",
      );
    } finally {
  ...
  const handleNoShow = async (bookingId: number) => {
    ...
    const confirmed = window.confirm(
      "Marcar como no-show? O cliente será notificado.",
    );
    if (!confirmed) return;
    setNoShowBusy(true);
    try {
      const res = await fetch(
        resolveCanonicalOrgApiPath(
          `/api/org/[orgId]/reservas/${bookingId}/no-show`,
        ),
        { method: "POST", headers: { "Content-Type": "application/json" } },
      );
      const json = await res.json().catch(() => null);
      if (!res.ok || !json?.ok) {
        throw new Error(
          json?.message || json?.error || "Erro ao atualizar reserva.",
        );
      }
      mutateUpcoming();
      if (json.booking) {
  ...
  const openCreateDrawer = (startsAt: Date) => {
    if (!servicesLoading && activeServices.length === 0) {
      router.push(
        appendOrganizationIdToHref("/org/academy/classes/new", organizationId),
      );
      return;
    }
    ...
    const initialServiceKind =
      initialServiceId != null
        ? (activeServices.find((service) => service.id === initialServiceId)
            ?.kind ?? null)
        : null;
    ...
    setCreateProfessionalId(
      initialAssignmentMode === "PROFESSIONAL" ? selectedProfessionalId : null,
    );
    setCreateResourceId(
      initialAssignmentMode === "RESOURCE" ? selectedResourceId : null,
    );
    const selectedResource = activeResources.find(
      (resource) => resource.id === selectedResourceId,
    );
    setCreatePartySize(
      initialAssignmentMode === "RESOURCE"
  ...
  const handleQuickCreateBooking = () => {
    ...
    const nextBucketMinute =
      Math.ceil((timeParts.hour * 60 + timeParts.minute + 5) / step) * step;
    const carryDay = nextBucketMinute >= 24 * 60;
    const targetDay = carryDay ? addDaysToParts(dayParts, 1) : dayParts;
    const minuteOfDay = carryDay
      ? nextBucketMinute - 24 * 60
      : nextBucketMinute;
  ...
  const handleCreateBooking = async () => {
    ...
    const parsedPartySize =
      createAssignmentMode === "RESOURCE" ? Number(createPartySize) : null;
    if (
    ...
    try {
      const payload = {
        ...
        professionalId:
          createAssignmentMode === "PROFESSIONAL" ? createProfessionalId : null,
        resourceId:
          createAssignmentMode === "RESOURCE" ? createResourceId : null,
      ...
      const res = await fetch(
        resolveCanonicalOrgApiPath("/api/org/[orgId]/reservas"),
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      const checkoutRes = await fetch(
        resolveCanonicalOrgApiPath(
          `/api/org/[orgId]/reservas/${bookingId}/checkout`,
        ),
        { method: "POST", headers: { "Content-Type": "application/json" } },
      );
  ...
  return (
    <div className="space-y-5">
      ...
      <header className="space-y-3">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div>
            ...
            <p className={DASHBOARD_MUTED}>
              {" "}
              Fluxo transacional de reservas por slots. Configuração funcional
              em Aulas e Disponibilidade.{" "}
            </p>{" "}
          </div>{" "}
        ...
        <div className="rounded-2xl border border-cyan-300/20 bg-white/[0.04] p-4">
          {" "}
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <div className="max-w-3xl">
              ...
              <p className="text-[11px] uppercase tracking-[0.2em] text-cyan-100/75">
                {operationsGuidance.badge}
              </p>{" "}
              <h2 className="mt-1 text-lg font-semibold text-white">
                {operationsGuidance.title}
              </h2>{" "}
              <p className="mt-2 text-sm text-white/80">
                {operationsGuidance.body}
              </p>{" "}
            </div>{" "}
        ...
        <div className={cn(DASHBOARD_CARD, "p-3 space-y-3")}>
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <button
                  ...
                  className={cn(
                    CHIP_BASE,
                    operationalWindowDays === 1 && CHIP_ACTIVE,
                  )}
                >
                  {" "}
                  24h{" "}
                ...
                <button
                  ...
                  className={cn(
                    CHIP_BASE,
                    operationalWindowDays === 3 && CHIP_ACTIVE,
                  )}
                >
                  {" "}
                  3 dias{" "}
                ...
                <button
                  ...
                  className={cn(
                    CHIP_BASE,
                    operationalWindowDays === 7 && CHIP_ACTIVE,
                  )}
                >
                  {" "}
                  7 dias{" "}
                ...
                <button
                  ...
                  className={cn(
                    CHIP_BASE,
                    operationalWindowDays === 14 && CHIP_ACTIVE,
                  )}
                >
                  {" "}
                  14 dias{" "}
          ...
          {isStaffMember ? (
            ...
            <div className="flex flex-wrap items-center gap-2 text-[12px]">
              ...
              {modeSaving && (
                <span className="text-[11px] text-white/60">A guardar...</span>
              )}{" "}
            </div>
          )}{" "}
        </div>{" "}
        {(canFilterByProfessional || canFilterByResource) && (
          <div className="space-y-2 text-[12px] text-white/70">
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
              {" "}
              <span className="shrink-0 text-[10px] uppercase tracking-[0.24em] text-white/50">
                Filtrar
              </span>{" "}
              {!isStaffMember && (
                <button
                  ...
                  className={cn(
                    ...
                    (filterMode === "PROFESSIONAL"
                      ? !selectedProfessionalId
                      : !selectedResourceId) && CHIP_ACTIVE,
                  )}
                >
                  {" "}
                  Todos{" "}
        ...
        {hasPadelClubs && (
          <div className="space-y-2 text-[12px] text-white/70">
              ...
              <div className="flex items-center gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
                ...
                <button
                  ...
                  className={cn(
                    CHIP_BASE,
                    "shrink-0",
                    !selectedPadelCourtId && CHIP_ACTIVE,
                  )}
                >
                  {" "}
                  Todos{" "}
            ...
            {requiresPadelClubSelection && (
              <span className="text-white/40">
                Seleciona um clube para carregar a agenda.
              </span>
            )}{" "}
          </div>
        )}{" "}
        ...
        <section className={cn(DASHBOARD_CARD, "p-4 space-y-3")}>
          ...
          <div className="flex flex-wrap items-center gap-2 text-[12px]">
            ...
            <button
              ...
              className={cn(
                CHIP_BASE,
                queueStatusFilter === "ALL" && CHIP_ACTIVE,
              )}
            >
              {" "}
            ...
            <button
              ...
              className={cn(
                CHIP_BASE,
                queueStatusFilter === "PENDING" && CHIP_ACTIVE,
              )}
            >
              {" "}
            ...
            <button
              ...
              className={cn(
                CHIP_BASE,
                queueStatusFilter === "CONFIRMED" && CHIP_ACTIVE,
              )}
            >
              {" "}
            ...
            <button
              ...
              className={cn(
                CHIP_BASE,
                queueStatusFilter === "DELAYED" && CHIP_ACTIVE,
              )}
            >
              {" "}
          ...
          {upcomingLoading && (
            <p className="text-[12px] text-white/60">A carregar...</p>
          )}{" "}
          ...
          <div className="space-y-2">
            ...
            {upcomingBookings.map((booking) => {
              ...
              const estimatedStart = booking.estimatedStartsAt
                ? new Date(booking.estimatedStartsAt)
                : null;
              const showEstimate =
              ...
              return (
                ...
                >
                  ...
                  <p className="text-[11px] text-white/50">
                    ...
                    {bookingMode === "RESOURCE"
                      ? "Recurso"
                      : "Profissional"}: {assignmentLabel}{" "}
                    {bookingMode === "RESOURCE" && booking.partySize
          ...
          {!upcomingLoading &&
            upcomingBookingsCount > upcomingBookings.length && (
              <p className="text-[11px] text-white/50">
                +{upcomingBookingsCount - upcomingBookings.length} no intervalo
              </p>
            )}{" "}
        </section>{" "}
          ...
          <section className={cn(DASHBOARD_CARD, "p-4 space-y-3")}>
            ...
            <div>
              ...
              <p className="text-[11px] text-white/45">
                janela de{" "}
                {operationalWindowDays === 1
                  ? "24h"
                  : `${operationalWindowDays} dias`}
              </p>{" "}
            </div>{" "}
            ...
            <div className="flex items-center justify-between text-sm text-white/70">
              {" "}
              <span>Confirmadas</span> <span>{confirmedBookings}</span>{" "}
            </div>{" "}
            <div className="flex items-center justify-between text-sm text-white/70">
              {" "}
              <span>Com atraso</span> <span>{delayedBookings}</span>{" "}
            </div>{" "}
            <div className="flex items-center justify-between text-sm text-white/70">
              {" "}
            ...
            <div>
              ...
              {activeDelay && activeDelay.delayMinutes > 0 && (
                <p className="mt-1 text-[12px] text-amber-100/80">
                  ...
                  {formatTimeLabel(
                    new Date(activeDelay.effectiveFrom),
                    timezone,
                  )}{" "}
                </p>
              )}{" "}
          ...
          <aside className="relative ml-auto mt-[var(--org-topbar-height)] h-[calc(100vh-var(--org-topbar-height))] w-full max-w-md overflow-y-auto border-l border-white/10 bg-[#0B0F16] p-6">
            ...
            <div className="flex items-center justify-between">
              ...
              <div>
                ...
                <h2 className="text-xl font-semibold text-white">
                  {drawerBooking.service?.title || "Aula"}
                </h2>{" "}
              </div>{" "}
              <button
                type="button"
            ...
            <div className="mt-4 space-y-3 text-sm text-white/70">
              ...
              <div>
                ...
                <p className="text-white">
                  {" "}
                  {drawerBooking.user?.fullName ||
                    drawerBooking.user?.username ||
                    "Cliente"}{" "}
                </p>{" "}
              </div>{" "}
              <div>
                ...
                <p className="text-white">
                  {" "}
                  {formatLongDate(
                    new Date(drawerBooking.startsAt),
                    timezone,
                  )} ·{" "}
                  {formatTimeLabel(
                    new Date(drawerBooking.startsAt),
              ...
              {drawerBooking.estimatedStartsAt && (
                <div>
                  ...
                  <p className="text-white">
                    {" "}
                    {formatTimeLabel(
                      new Date(drawerBooking.estimatedStartsAt),
                      timezone,
                    )}{" "}
                    {drawerBooking.delayMinutes
                      ? ` (+${drawerBooking.delayMinutes} min)`
                      : ""}{" "}
                  </p>{" "}
                </div>
              )}{" "}
              <div>
                ...
                <p className="text-white">
                  {drawerBooking.durationMinutes} min
                </p>{" "}
              </div>{" "}
              <div>
                {" "}
                ...
                <p className="text-white">
                  {formatCurrency(drawerBooking.price, drawerBooking.currency)}
                </p>{" "}
              </div>{" "}
              <div>
                {" "}
                ...
                <p className="text-white">
                  {formatBookingStatus(drawerBooking.status)}
                </p>{" "}
              </div>{" "}
              ...
              {getBookingMode(drawerBooking) === "RESOURCE" &&
                drawerBooking.resource?.label && (
                  <div>
                    {" "}
              ...
              {getBookingMode(drawerBooking) === "RESOURCE" &&
                drawerBooking.partySize && (
                  <div>
                    {" "}
                    <p className="text-white/50">Capacidade</p>{" "}
                    <p className="text-white">
                      {drawerBooking.partySize} pax
                    </p>{" "}
                  </div>
                )}{" "}
                ...
                <div>
                  ...
                  <p className="text-white text-[12px]">
                    ...
                    {inviteSummary.pending
                      ? ` · ${inviteSummary.pending} pendentes`
                      : ""}{" "}
                    {inviteSummary.declined
                      ? ` · ${inviteSummary.declined} recusados`
                      : ""}{" "}
                  </p>{" "}
                </div>
              )}{" "}
              ...
              <div className="mt-6 space-y-3 rounded-2xl border border-white/10 bg-white/5 p-4 text-sm text-white/70">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <p className="text-[12px] text-white/60">
                    {" "}
                    {participantSummary.confirmed}/
                    {inviteSummary.total || participantSummary.total}{" "}
                    confirmados{" "}
                  </p>{" "}
                </div>{" "}
                <div className="space-y-2 rounded-xl border border-white/10 bg-white/5 p-3">
                  ...
                  {inviteError && (
                    <p className="text-[11px] text-red-200">{inviteError}</p>
                  )}{" "}
                  <button
                    type="button"
                ...
                <div className="space-y-2">
                  ...
                  (participantsData?.invites ?? []).length === 0 ? (
              ...
              <div className="mt-6 space-y-3 rounded-2xl border border-white/10 bg-white/5 p-4 text-sm text-white/70">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div>
                    ...
                    <p className="text-[12px] text-white/60">
                      Divide o valor pelos convidados.
                    </p>{" "}
                  </div>{" "}
                ...
                {splitState.loading ? (
                  ...
                  <div className="space-y-3">
                    ...
                    {splitLocked && (
                      <p className="text-[11px] text-white/50">
                        {" "}
                        Split bloqueado. A reserva está encerrada ou já recebeu
                        pagamentos.{" "}
                      </p>
                    )}{" "}
                    {!splitLocked && splitState.participants.length === 0 && (
                      <p className="text-[11px] text-white/50">
                        {" "}
                        Sem convites. O split precisa de convidados associados à
                        reserva.{" "}
                      </p>
                    )}{" "}
                    {splitEditorOpen && (
                      <div className="space-y-3">
                                  ...
                                  prev
                                    ? {
                                        ...
                                        pricingMode: event.target
                                          .value as SplitState["pricingMode"],
                                      }
                                    : prev,
                                );
                              }}
                                ...
                                onChange={(event) => {
                                  ...
                                  setSplitState((prev) =>
                                    prev
                                      ? {
                                          ...prev,
                                          dynamicMode: event.target
                                            .value as SplitState["dynamicMode"],
                                        }
                                      : prev,
                                  );
                                }}
                        ...
                        <div className="space-y-2">
                          ...
                          {splitState.participants.map((participant) => {
                            ...
                            const showAmount =
                              splitState.pricingMode === "DYNAMIC" &&
                              splitState.dynamicMode === "AMOUNT";
                            const showPercent =
                              splitState.pricingMode === "DYNAMIC" &&
                              splitState.dynamicMode === "PERCENT";
                            return (
                              <div
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  {splitState.pricingMode === "FIXED" && (
                                    <span className="text-[11px] text-white/70">
                                      {" "}
                                      {splitState.fixedShare
                                        ? `${splitState.fixedShare} ${splitState.currency}`
                                        : "--"}{" "}
                                    </span>
                                  )}{" "}
                                  {showAmount && (
                        ...
                        <label className="flex flex-col gap-1 text-[12px] text-white/60">
                          ...
                          <OryaDateTimeField
                            ...
                            onChange={(next) => {
                              splitDirtyRef.current = true;
                              setSplitState((prev) =>
                                prev ? { ...prev, deadlineAt: next } : prev,
                              );
                            }}
                            className="w-full"
            ...
            <div className="mt-6 space-y-3 rounded-2xl border border-white/10 bg-white/5 p-4 text-sm text-white/70">
              ...
              <div className="space-y-2">
                ...
                />{" "}
                {chatError ? (
                  <p className="text-[11px] text-red-200">{chatError}</p>
                ) : null}{" "}
                <div className="flex items-center justify-between">
                  {" "}
                  {chatConversationId ? (
                    <Link
                      href={
                        organizationId
                          ? buildOrgHref(organizationId, "/chat", {
                  ...
                  <button
                    ...
                    disabled={
                      chatSending || drawerBookingClosed || !chatDraft.trim()
                    }
              ...
              <div className="mt-6 space-y-3">
                ...
                {drawerBooking.changeRequest?.status === "PENDING" && (
                  <div className="rounded-xl border border-amber-400/30 bg-amber-500/10 px-3 py-2 text-[12px] text-amber-100">
                    ...
                    Pedido pendente até{""}{" "}
                    {new Date(
                      drawerBooking.changeRequest.expiresAt,
            ...
            <div className="mt-6 space-y-2">
              ...
              {drawerBooking.service?.id && (
                <Link
                  href={appendOrganizationIdToHref(
                    `/org/academy/classes/${drawerBooking.service.id}`,
                    organizationId,
                  )}
                  className={CTA_SECONDARY}
                >
              ...
              {!"CANCELLED CANCELLED_BY_CLIENT CANCELLED_BY_ORG COMPLETED DISPUTED NO_SHOW"
                .split("")
                .includes(drawerBooking.status) && (
                <button
                  type="button"
          ...
          <aside className="relative ml-auto mt-[var(--org-topbar-height)] h-[calc(100vh-var(--org-topbar-height))] w-full max-w-md overflow-y-auto border-l border-white/10 bg-[#0B0F16] p-6">
            ...
            <div className="mt-4 space-y-4 text-sm text-white/70">
              ...
              <div>
                ...
                <p className="text-white">
                  {" "}
                  {formatLongDate(createSlot, timezone)} ·{" "}
                  {formatTimeLabel(createSlot, timezone)}{" "}
                </p>{" "}
              </div>{" "}
              <div className="space-y-2">
                {" "}
                ...
                {activeServices.length === 0 ? (
                  ...
                  <>
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        setCreateServiceId(Number(event.target.value) || null)
                      }
                    >
                      {" "}
                    ...
                    {selectedCreateService && (
                      <div className="rounded-xl border border-white/10 bg-white/5 p-3 text-[12px] text-white/70">
                        ...
                        {formatCurrency(
                          selectedCreateService.unitPriceCents,
                          selectedCreateService.currency,
                        )}{" "}
                      </div>
                    )}{" "}
                  </>
                    ...
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <div>
                        ...
                        <p className="font-semibold text-white">
                          {formatClientLabel(createClient)}
                        </p>{" "}
                        <p className="text-white/60">
                          {" "}
                          {createClient.contactPhone
                            ? createClient.contactPhone
                            : "Sem telemovel"}{" "}
                          {createClient.email
                            ? ` · ${createClient.email}`
              ...
              {createAssignmentMode === "PROFESSIONAL" && (
                <div className="space-y-2">
                  ...
                  {hasServiceProfessionalLinks &&
                    availableProfessionalsForService.length === 0 && (
                      <p className="text-[12px] text-white/50">
                        Sem treinadores ligados a esta aula.
                      </p>
                    )}{" "}
                </div>
              )}{" "}
              {createAssignmentMode === "RESOURCE" && (
                <div className="space-y-2">
                  ...
                  {hasServiceResourceLinks &&
                    availableResourcesForService.length === 0 && (
                      <p className="text-[12px] text-white/50">
                        Sem campos ligados a esta aula.
                      </p>
                    )}{" "}
                </div>
              )}{" "}
                ...
                <div className="space-y-2">
                  ...
                  {createAddressId && (
                    <div className="rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-[12px] text-white/70">
                      {" "}
                      Morada confirmada:{" "}
                      {createAddressLabel || createAddressQuery}{" "}
                    </div>
                  )}{" "}
                </div>
              )}{" "}
