...
export default async function EventLivePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const event = await prisma.event.findUnique({ where: { slug }, select: { slug: true, templateType: true } });
  if (!event) {
      ...
      <section className="relative orya-page-width flex flex-col gap-4 px-4 py-10">
        <div className="flex items-center justify-between">
          ...
          >
            ...
            <span>Voltar ao {event?.templateType === "PADEL" ? "torneio de padel" : "evento"}</span>
          </Link>
        </div>
