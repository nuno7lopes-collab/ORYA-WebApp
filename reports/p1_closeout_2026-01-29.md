...
- E2E P1 flows: BLOCKED (requires running web server + admin session + Stripe/Resend/APNS)
- Mobile/A11y/Perf QA: BLOCKED (requires device farm/Lighthouse)
...
- Runbooks: domain runbooks added (payments/refunds, check-in, reservas, notifications) + metrics/alerts.
- Ops health: includes outbox/operations/payment counts.
- RBAC: added access matrix tests; guardrails remain.

## Tests
- `npm run test` (vitest run): **PASS** (303 tests, 103 files).
...
## E2E P1 (BLOCKED)
### Bloqueios
- Requer servidor Next ativo + sessão admin para /api/admin/*.
- Requer STRIPE_WEBHOOK_SECRET + Stripe test data para disputes/refunds reais.
- APNS envs ausentes para push real (APNS_TEAM_ID, APNS_KEY_ID, APNS_PRIVATE_KEY_BASE64, APNS_TOPIC).
