...
export default function InscricaoDetailPage() {
  ...
  const { data, mutate, isLoading } = useSWR<FormResponse>(
    formId ? `/api/organizador/inscricoes/${formId}` : null,
    fetcher,
  );
  const {
    ...
    formId ? `/api/organizador/inscricoes/${formId}/submissions` : null,
    fetcher,
  );
  ...
  const handleStatusChange = async (submissionId: number, nextStatus: SubmissionStatus) => {
    ...
    try {
      const res = await fetch(`/api/organizador/inscricoes/${formId}/submissions`, {
        method: "PATCH",
  ...
  const handleSave = async () => {
    ...
    setMessage(null);
    try {
      const res = await fetch(`/api/organizador/inscricoes/${formId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
