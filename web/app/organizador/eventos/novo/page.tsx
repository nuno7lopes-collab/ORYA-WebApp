...
export default function NewOrganizerEventPage() {
  ...
  const publicAccessDescription =
    publicAccessMode === "OPEN"
      ...
      : publicAccessMode === "TICKET"
        ? isPadelContext
          ? "Qualquer inscrição criada dá acesso ao público (podes refinar depois)."
  ...
  const participantAccessDescription =
    participantAccessMode === "NONE"
      ...
      : participantAccessMode === "INSCRIPTION"
        ...
        : participantAccessMode === "TICKET"
          ? isPadelContext
            ? "Qualquer inscrição criada marca o utilizador como participante."
  ...
  const paidTicketsBlockedMessage = useMemo(() => {
    ...
    return isPadelContext
      ? `Torneios pagos só ficam ativos depois de ${reasonsText}. Até lá podes criar torneios gratuitos (preço = 0 €).`
      : `Eventos pagos só ficam ativos depois de ${reasonsText}. Até lá podes criar eventos gratuitos (preço = 0 €).`;
  ...
  const { data: padelClubs } = useSWR<{ ok: boolean; items?: PadelClubSummary[] }>(
    isPadelPreset ? "/api/padel/clubs" : null,
    fetcher,
    { revalidateOnFocus: false },
  );
  const { data: padelCourts } = useSWR<{ ok: boolean; items?: PadelCourtSummary[] }>(
    isPadelPreset && selectedPadelClubId ? `/api/padel/clubs/${selectedPadelClubId}/courts` : null,
    fetcher,
    { revalidateOnFocus: false },
  );
  const { data: padelStaff } = useSWR<{ ok: boolean; items?: PadelStaffSummary[] }>(
    isPadelPreset && selectedPadelClubId ? `/api/padel/clubs/${selectedPadelClubId}/staff` : null,
    fetcher,
    { revalidateOnFocus: false },
  );
  ...
  }, [isPadelPreset, padelClubs, selectedPadelClubId]);
  ...
  }, [isPadelPreset, selectedPadelClubId, padelClubs?.items, locationManuallySet, locationName]);
    ...
    keys.forEach((key) => {
      ...
      if (key === "tickets") {
        ...
        if (
          ...
          issues.push({ field: "tickets", message: `Seleciona pelo menos um ${ticketLabel} para o público.` });
        }
        if (
          advancedAccessEnabled &&
          participantAccessMode === "TICKET" &&
          ...
          if (hasBelowMinimum) {
            issues.push({
              ...
              message: isPadelContext
                ? "Para torneios de padel pagos, cada inscrição tem de custar pelo menos 1 €."
                : "Para eventos pagos, cada bilhete tem de custar pelo menos 1 €.",
            });
          }
        }
        if (!isFreeEvent && hasPaidTicket && paidTicketsBlocked) {
          issues.push({
            ...
            message:
              ...
              (isPadelContext
                ? "Liga o Stripe e verifica o email oficial da organização para aceitar inscrições pagas."
      ...
      <div className="w-full px-4 py-8 md:px-6 lg:px-8">
        <div className="rounded-3xl border border-white/12 bg-gradient-to-br from-white/8 via-[#0b1124]/70 to-[#050810]/90 p-6 text-white shadow-[0_24px_80px_rgba(0,0,0,0.55)] backdrop-blur-2xl space-y-3">
          ...
          <p className="text-white/70">
            Precisas de iniciar sessão para criar {eventLabelPlural} como organizador.
          </p>
          <button
            type="button"
    ...
    <div className="space-y-4 animate-fade-slide">
      <div className="flex flex-col gap-2">
        <p className="text-sm text-white/75">
          {isPadelOrg
            ? "A tua organização está configurada para padel. Este torneio já usa o wizard dedicado."
            : "Escolhe o formato. Padel ativa o wizard dedicado; o formato padrão é neutro."}
        </p>
        <p className="text-[12px] text-white/55">
          {isPadelOrg ? "Podes avançar sem mudar o formato." : "Tudo segue a mesma linguagem visual."}
        </p>
        {fieldErrors.preset && (
      ...
      {isAdmin && (
        <label className="flex items-center gap-3 rounded-2xl border border-white/12 bg-black/30 px-3 py-2 text-sm">
          ...
          <span className="text-white/80">
            {isPadelContext ? "Torneio de teste" : "Evento de teste"} (visível só para admin, não aparece em explorar)
          </span>
        </label>
      )}
      ...
      <div className="rounded-2xl border border-white/12 bg-[rgba(14,14,20,0.7)] p-4 space-y-3 shadow-[0_14px_36px_rgba(0,0,0,0.45)]">
        <div className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="text-[12px] text-white/65">Hero do {eventLabel} — legível em mobile.</p>
          </div>
    ...
    <div ref={ticketsRef} className="space-y-5 animate-fade-slide">
      <div className="flex flex-col gap-3 rounded-2xl border border-white/12 bg-[rgba(14,14,20,0.7)] p-4 shadow-[0_14px_36px_rgba(0,0,0,0.45)]">
        <div className="flex items-center justify-between">
          ...
          <div className="inline-flex rounded-full border border-white/15 bg-black/40 p-1 text-[13px]">
            ...
            >
              {isPadelContext ? "Torneio grátis" : "Evento grátis"}
            </button>
          </div>
        </div>
        <p className="text-[12px] text-white/55">
          {isPadelContext
            ? "Torneios pagos precisam de Stripe ligado e email oficial definido e verificado. Torneios grátis focam-se em inscrições e vagas."
        ...
        {paidTicketsBlocked && (
          <div className="rounded-xl border border-amber-400/40 bg-amber-400/10 p-3 text-[12px] text-amber-50 space-y-2">
            <div className="flex items-center gap-2 text-sm font-semibold">
              <span aria-hidden>⚠️</span>
              <span>{isPadelContext ? "Só podes criar torneios gratuitos para já" : "Só podes criar eventos gratuitos para já"}</span>
            </div>
            <p className="text-amber-50/90">
              {paidTicketsBlockedMessage ??
                (isPadelContext
                  ? "Liga o Stripe e verifica o email oficial da organização para aceitar inscrições pagas. Até lá, cria torneios gratuitos (preço = 0 €)."
      ...
      <div className="space-y-3 rounded-2xl border border-white/12 bg-[rgba(14,14,20,0.7)] p-4 shadow-[0_14px_36px_rgba(0,0,0,0.45)]">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-[12px] text-white/65">
              Por defeito, o {eventLabel} é simples: {ticketLabel} pago ou entrada gratuita, sem separação de público.
            </p>
          </div>
          <button
            type="button"
        ...
        {!advancedAccessEnabled ? (
          <div className="rounded-xl border border-white/10 bg-black/30 px-3 py-3 text-[12px] text-white/70">
            ...
            {isPadelContext ? "inscrições pagas" : "bilhetes pagos"} ou convites, ativa o modo avançado.
          </div>
        ) : (
          <>
            <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
                ...
                {publicAccessMode === "INVITE" && (
                  <p className="text-[12px] text-white/60">
                    A lista de convites do público é adicionada depois de criares o {eventLabel}.
                  </p>
                )}
              </div>
              ...
              <div className="space-y-2">
                ...
                {participantAccessMode === "INVITE" && (
                  <p className="text-[12px] text-white/60">
                    A lista de convites de participantes é adicionada depois de criares o {eventLabel}.
                  </p>
                )}
              </div>
            </div>
      ...
      {isFreeEvent ? (
        <div className="space-y-3 rounded-2xl border border-white/12 bg-gradient-to-br from-white/6 via-[#0c1426]/65 to-[#050a14]/88 p-4 shadow-[0_16px_60px_rgba(0,0,0,0.45)]">
          <div className="flex items-center justify-between">
            <p className={labelClass}>{isPadelContext ? "Inscrições gratuitas" : "Bilhetes gratuitos"}</p>
        ...
        <div className="space-y-4 rounded-2xl border border-white/12 bg-gradient-to-br from-white/6 via-[#0c1426]/65 to-[#050a14]/88 p-4 shadow-[0_16px_60px_rgba(0,0,0,0.45)]">
                ...
                >
                  ...
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <div className="space-y-1 flex-1">
                      <label className={labelClass}>
                        {isPadelContext ? "Nome da inscrição" : "Nome do bilhete"} <span aria-hidden>*</span>
                      </label>
                      <input
                        type="text"
                        value={row.name}
                  ...
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                    <div className="space-y-1">
                      ...
                      />
                      <p className="text-[12px] text-white/55">Em {eventLabelPlural} pagos, o preço mínimo é 1,00 €.</p>
                    </div>
                    <div className="space-y-1">
                      <label className={labelClass}>Capacidade (opcional)</label>
                      <input
                    ...
                    ((publicAccessMode === "TICKET" && publicTicketScope === "SPECIFIC") ||
                      ...
                      <div className="flex flex-wrap items-center gap-3 rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-[12px] text-white/75">
                        <span className="text-[11px] uppercase tracking-[0.18em] text-white/60">
                          {isPadelContext ? "Esta inscrição dá acesso a" : "Este bilhete dá acesso a"}
                        </span>
                        {publicAccessMode === "TICKET" && publicTicketScope === "SPECIFIC" && (
          ...
          <div className="space-y-2">
            ...
            <p className="text-[12px] text-white/55">
              Podes ajustar depois no resumo. Para {eventLabelPlural} de plataforma, a taxa ORYA é zero.
            </p>
          </div>
        </div>
      )}
            ...
            <div className="rounded-xl border border-white/12 bg-gradient-to-br from-white/6 via-[#0b1020]/65 to-[#060912]/85 p-3 shadow-[0_12px_38px_rgba(0,0,0,0.45)] transition hover:border-white/25 hover:shadow-[0_16px_48px_rgba(0,0,0,0.55)]">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="font-semibold text-white">
                    {isFreeEvent ? (isPadelContext ? "Torneio grátis" : "Evento grátis") : isPadelContext ? "Torneio pago" : "Evento pago"}
                  </p>
                </div>
                <button
                  type="button"
            ...
            activeStepKey === "tickets"
              ? isFreeEvent
                ? isPadelContext
                  ...
                  : "Bilhetes sem taxas; capacidade é opcional."
                : paidTicketsBlocked
                  ? paidTicketsBlockedMessage ??
                    (isPadelContext
                  ...
                  : isPadelContext
                    ? "Define preços, taxas e capacidade das inscrições."
