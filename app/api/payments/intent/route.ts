...
export async function POST(req: NextRequest) {
  try {
    ...
    const ticketLabel = padelConfig ? "inscrição" : "bilhete";
    const ticketLabelIndefinite = padelConfig ? "uma inscrição" : "um bilhete";
    const ticketLabelTitle = padelConfig ? "Inscrição" : "Bilhete";
    ...
    if (isResaleRequest) {
      ...
      if (items.length !== 1) {
        return intentError("RESALE_SINGLE_ITEM_ONLY", `Revenda suporta apenas ${ticketLabelIndefinite} por compra.`, {
          httpStatus: 400,
          status: "FAILED",
        });
      }
    ...
    if (ticketTypeIds.length === 0) {
      return intentError("INVALID_TICKETS", `IDs de ${ticketLabel} inválidos.`, { httpStatus: 400 });
    }
    ...
    if (ticketTypes.length !== ticketTypeIds.length) {
      return intentError(
        "TICKET_NOT_FOUND",
        padelConfig
          ? "Uma das inscrições não foi encontrada ou não pertence a este evento."
          : "Um dos bilhetes não foi encontrado ou não pertence a este evento.",
        { httpStatus: 400 },
      );
    }
    ...
    for (const item of items) {
      ...
      if (!Number.isFinite(ticketTypeId)) {
        return intentError("INVALID_TICKET_ID", `ID de ${ticketLabel} inválido.`, { httpStatus: 400 });
      }
      ...
      if (resaleContext) {
        if (ticketTypeId !== resaleContext.ticketTypeId) {
          return intentError(
            "RESALE_TICKET_MISMATCH",
            padelConfig
              ? "Revenda não corresponde à inscrição selecionada."
              : "Revenda não corresponde ao bilhete selecionado.",
            {
            httpStatus: 400,
            status: "FAILED",
            },
        ...
        if (qty !== 1) {
          return intentError("RESALE_QUANTITY_INVALID", `Revenda apenas permite 1 ${ticketLabel} por compra.`, {
            httpStatus: 400,
            status: "FAILED",
          });
        }
      ...
      if (
        ...
        if (remaining < qty) {
          return intentError(
            "INSUFFICIENT_STOCK",
            padelConfig ? "Stock insuficiente para uma das inscrições." : "Stock insuficiente para um dos bilhetes.",
            {
            httpStatus: 409,
            status: "FAILED",
            retryable: false,
      ...
      lines.push({
        ticketTypeId,
        name: ticketType.name ?? ticketLabelTitle,
        quantity: qty,
    ...
    if (scenarioAdjusted === "GROUP_FULL" || scenarioAdjusted === "GROUP_SPLIT") {
      ...
      if (resolvedCategoryId && pairing.categoryId && pairing.categoryId !== resolvedCategoryId) {
        return intentError(
          "PADEL_CATEGORY_MISMATCH",
          padelConfig ? "Categoria da inscrição não corresponde à dupla." : "Categoria do bilhete não corresponde à dupla.",
          {
          httpStatus: 400,
          status: "FAILED",
          retryable: false,
