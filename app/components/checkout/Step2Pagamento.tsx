...
export default function Step2Pagamento() {
  ...
  const checkoutVariant =
    safeDados?.additional && typeof safeDados.additional === "object" && typeof safeDados.additional.checkoutUiVariant === "string"
  ...
  const ticketArticle = isPadelVariant ? "a" : "o";
  const ticketPluralArticle = isPadelVariant ? "as" : "os";
  const ticketInLabel = isPadelVariant ? "na inscrição" : "no bilhete";
  const ticketPossessivePlural = isPadelVariant ? "as tuas" : "os teus";
  const eventLabel = isPadelVariant ? "torneio de padel" : "evento";
  const eventLabelPlural = isPadelVariant ? "torneios de padel" : "eventos";
  const ticketPluralPhrase = `${ticketPluralArticle} ${ticketLabelPlural}`;
  const scenarioCopy: Record<string, string> = {
    ...
    RESALE: `Estás a comprar ${isPadelVariant ? "uma" : "um"} ${ticketLabel} em revenda.`,
    FREE_CHECKOUT: `${isPadelVariant ? "Torneio de Padel" : "Evento"} gratuito — só para utilizadores com conta e username.`,
  };
      ...
      try {
        ...
        if (!res.ok || !data?.ok) {
          ...
          if (respCode === "USERNAME_REQUIRED_FOR_FREE") {
            if (!cancelled) {
              setError(`Este ${eventLabel} gratuito requer sessão com username definido.`);
              setPurchaseMode("auth");
              setAuthInfo(
          ...
          if (respCode === "INVITE_REQUIRED") {
            if (!cancelled) {
              setError(`Este ${eventLabel} é apenas por convite.`);
          ...
          const msg =
            respCode === "PRICE_CHANGED"
              ...
              : respCode === "INSUFFICIENT_STOCK"
                ? `Stock insuficiente para ${isPadelVariant ? "uma das inscrições" : "um dos bilhetes"}.`
                : typeof data?.error === "string"
                  ? data.error
          ...
          if (respCode === "ORGANIZER_STRIPE_NOT_CONNECTED") {
            if (!cancelled) {
              setError(
                data?.message ||
                  `Pagamentos desativados para este ${eventLabel} enquanto o organizador não ligar a Stripe.`,
              );
...
// Callback chamado pelo AuthWall quando o utilizador faz login/cria conta com sucesso
  ...
  const handleGuestContinue = () => {
    ...
    if (!guestName.trim()) {
      localErrors.name = `Nome é obrigatório para emitir ${ticketArticle} ${ticketLabel}.`;
    }
    if (!guestEmail.trim()) {
      localErrors.email = `Email é obrigatório para enviar ${ticketPluralArticle} ${ticketLabelPlural}.`;
          ...
          {requiresAuth ? (
            <div className="space-y-2">
              <div className="rounded-xl border border-emerald-400/30 bg-emerald-500/10 px-3 py-2 text-[11px] text-emerald-50">
                Este checkout exige conta. Para {eventLabelPlural} gratuitos precisas de iniciar sessão e ter um username definido.
              </div>
              <AuthWall onAuthenticated={handleAuthenticated} ticketPluralPhrase={ticketPluralPhrase} />
            </div>
          ) : purchaseMode === "guest" ? (
            <GuestCheckoutCard
              guestName={guestName}
              ...
              ticketPossessivePlural={ticketPossessivePlural}
              ticketLabelPlural={ticketLabelPlural}
              ticketInLabel={ticketInLabel}
                ...
                >
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-[11px] text-white/45">
                      {formatMoney(line.unitPriceCents, line.currency || currency)} / {ticketLabel}
                    </p>
                  </div>
                </div>
              ))}
...
function AuthWall({ onAuthenticated, ticketPluralPhrase }: AuthWallProps) {
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-[11px] text-white/60 max-w-sm leading-relaxed">
            Para associar {ticketPluralPhrase} à tua conta ORYA e evitar problemas no
...
type GuestCheckoutCardProps = {
  ...
  onContinue: () => void;
  ticketPossessivePlural: string;
  ticketLabelPlural: string;
  ticketInLabel: string;
  eventLabel: string;
};
    ...
    <div className="flex-1 rounded-2xl border border-white/12 bg-white/[0.06] px-6 py-6 shadow-[0_20px_60px_rgba(0,0,0,0.55)] backdrop-blur-xl flex flex-col gap-4">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-[11px] text-white/60 max-w-sm leading-relaxed">
            Compra em 30 segundos. Guardamos {ticketPossessivePlural} {ticketLabelPlural} pelo email e podes
            criar conta depois para ligar tudo ao teu perfil.
          </p>
          <div className="mt-2 space-y-1 text-[11px] text-white/55">
            <p>• Email é usado para entregar {ticketLabelPlural} e recibo.</p>
            <p>• Telefone ajuda no contacto no dia do {eventLabel} (opcional).</p>
          </div>
        </div>
      ...
      <div className="flex flex-col gap-3 text-[12px]">
        <div className="flex flex-col gap-1">
          ...
          <input
            ...
            placeholder={`Como queres que apareça ${ticketInLabel}`}
            value={guestName}
      ...
      <p className="mt-1 text-[10px] text-white/40 leading-snug">
        Vamos enviar {ticketLabelPlural} para este email. Depois podes criar conta e
        migrar tudo para o teu perfil.
      </p>
    </div>
  );
}
