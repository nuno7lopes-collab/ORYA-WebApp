...
"[project]/app/api/organizador/tournaments/[id]/participants/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
function normalizeParticipants(items, bracketSize) {
    const normalized = [];
    const seen = new Set();
    ...
    const reserveId = (candidate)=>{
        let id = Number.isFinite(candidate) ? Math.trunc(candidate) : null;
        const inRange = typeof id === "number" && Number.isFinite(id) && id >= minInt && id <= maxInt && id < 0;
    ...
    for (const raw of items){
        ...
        const seed = Number.isFinite(rawSeed) && rawSeed >= 1 && (typeof bracketSize !== "number" || rawSeed <= bracketSize) && !usedSeeds.has(rawSeed) ? Math.trunc(rawSeed) : null;
...
async function GET(_req, { params }) {
    ...
    const bracketSize = Number.isFinite(config.bracketSize) ? Number(config.bracketSize) : null;
...
async function POST(req, { params }) {
    ...
    await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"].tournament.update({
        where: {
            id: tournament.id
        },
        data: {
    ...
    const res = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
        ...
        bracketSize: bracketSize ?? (Number.isFinite(config.bracketSize) ? Number(config.bracketSize) : null)
    }, {
        status: 200
    });
    res.headers.set("Cache-Control", "no-store");
...
//# sourceMappingURL=%5Broot-of-the-server%5D__88f3bf8a._.js.map