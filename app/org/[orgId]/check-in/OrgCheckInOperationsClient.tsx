...
type CheckinResultPayload = { code?: string; checkedInAt?: string | null };
const DEVICE_STORAGE_KEY = "oryaCheckinDeviceId";
const MANUAL_LIST_ENABLED =
  process.env.NEXT_PUBLIC_CHECKIN_MANUAL_LIST_ENABLED !== "false";
...
export default function OrgCheckInOperationsClient({
  orgId,
  mode,
}: OrgCheckInOperationsClientProps) {
  ...
  const [selectedForManual, setSelectedForManual] =
    useState<CheckInAttendeeItem | null>(null);
  ...
  const loadEvents = useCallback(async () => {
    ...
    try {
      ...
      setSelectedEventId((prev) => {
        ...
        if (
          queryEventId &&
          nextEvents.some((event) => event.id === queryEventId)
        )
          return queryEventId;
    ...
    async (eventId: number, cursor?: string | null) => {
      ...
      try {
        ...
        if (listStatuses.length > 0)
          params.set("status", listStatuses.join(","));
        if (cursor) params.set("cursor", cursor);
        const res = await fetch(
          `/api/org/${orgId}/events/${eventId}/attendees?${params.toString()}`,
          { cache: "no-store" },
        );
        const json = await res.json().catch(() => null);
        const data = readEnvelopeData<{
          items?: CheckInAttendeeItem[];
  ...
  const loadLogs = useCallback(
    async (eventId: number, cursor?: string | null) => {
      ...
      try {
        ...
        if (!res.ok || !data || !Array.isArray(data.items)) {
          throw new Error(
            data?.error || "Não foi possível carregar os registos.",
          );
        }
        setLogItems((prev) =>
          cursor ? [...prev, ...data.items!] : data.items!,
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-white/65">
            {" "}
            Seleciona um evento e abre scanner, lista operacional ou registos
            sem sair do fluxo.{" "}
          </p>{" "}
        </div>{" "}
        <button
      ...
      <div className="grid gap-3 md:grid-cols-2">
        ...
        {eventsFiltered.map((event) => {
          ...
          return (
            <article
              key={event.id}
              className={`rounded-2xl border px-4 py-4 transition ${selected ? "border-[#22D3EE]/45 bg-[#22D3EE]/10" : "border-white/12 bg-white/5 hover:border-white/30"}`}
            >
              {" "}
              <button
                type="button"
    ...
    <section className="space-y-4 rounded-3xl border border-white/12 bg-white/5 p-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-white/65">
            {" "}
            Pesquisa participantes, pré-valida o estado e confirma manualmente
            com motivo auditável.{" "}
          </p>{" "}
        </div>{" "}
        <div className="flex flex-wrap gap-2">
          ...
          <Link
            href={
              selectedEventId
                ? buildOrgHref(orgId, "/check-in/scanner", {
        ...
        {LIST_STATUS_FILTERS.map((status) => {
          ...
          return (
            <button
              ...
              className={`rounded-full border px-3 py-1 text-[11px] font-semibold transition ${active ? "border-[#22D3EE]/60 bg-[#22D3EE]/15 text-[#C7FFFF]" : "border-white/20 bg-white/5 text-white/70 hover:border-white/35"}`}
            >
              {" "}
              {status.label}{" "}
            </button>
      ...
      {selectedEvent ? (
        <div className="rounded-2xl border border-white/12 bg-black/20 px-4 py-3 text-[12px] text-white/70">
          {" "}
          {selectedEvent.title || `Evento #${selectedEvent.id}`} ·{" "}
          {formatDateTime(selectedEvent.startsAt)}{" "}
          {selectedEvent.locationFormattedAddress
            ? ` · ${selectedEvent.locationFormattedAddress}`
            : ""}{" "}
        </div>
      ) : null}{" "}
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
        ...
        <div className="space-y-3">
          ...
          {listItems.map((item) => {
            const selected =
              selectedForManual?.entitlementId === item.entitlementId;
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-wrap items-center justify-between gap-2">
                  {" "}
                  <p className="text-sm font-semibold text-white">
                    {item.holder.name || "Participante"}
                  </p>{" "}
                  <span
                    className={`rounded-full border px-2.5 py-1 text-[11px] font-semibold ${statusTone(item.status)}`}
                  >
                    {" "}
                    {item.status}{" "}
                  </span>{" "}
                ...
                <p className="mt-1 text-[12px] text-white/55">
                  {" "}
                  {item.snapshot.title || "Sem título"} ·{" "}
                  {formatDateTime(item.snapshot.startAt)}{" "}
                </p>{" "}
                <div className="mt-3 flex flex-wrap gap-2">
                  {" "}
                  <button
        ...
        <aside className="space-y-3 rounded-2xl border border-white/12 bg-black/25 p-4">
          ...
          <p className="text-[11px] uppercase tracking-[0.2em] text-white/55">
            Pré-validação manual
          </p>{" "}
          {!selectedForManual ? (
            <p className="text-sm text-white/65">
              Escolhe um participante para pré-validar.
            </p>
          ) : (
            <>
              {" "}
              <div className="rounded-xl border border-white/12 bg-black/30 px-3 py-2 text-sm text-white/85">
                ...
                <p className="text-[12px] text-white/60">
                  {selectedForManual.holder.email || "Sem email"}
                </p>{" "}
                <p className="mt-2 text-[12px] text-white/70">
                  {" "}
                  Estado atual:{" "}
                  <span className="font-semibold">
                    {selectedForManual.status}
                  </span>{" "}
                </p>{" "}
                {selectedForManual.checkedInAt ? (
                  <p className="text-[12px] text-amber-200">
                    Check-in já registado em{" "}
                    {formatDateTime(selectedForManual.checkedInAt)}.
                  </p>
                ) : (
                  <p className="text-[12px] text-emerald-200">
                    Elegível para confirmação manual, sujeito a policy e janela.
                  </p>
                )}{" "}
              </div>{" "}
      ...
      {manualConfirmOpen && selectedForManual ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4">
          ...
          <div
            ...
            className="w-full max-w-md space-y-4 rounded-2xl border border-white/15 bg-[#0D1424] p-5 text-white"
          >
            {" "}
    ...
    <section className="space-y-4 rounded-3xl border border-white/12 bg-white/5 p-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-white/65">
            Auditoria de entradas com método, operador e motivo manual.
          </p>{" "}
        </div>{" "}
        <button
          type="button"
    ...
    <section className="space-y-4 rounded-3xl border border-white/12 bg-white/5 p-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-white/65">
            {" "}
            Identidade local do posto de check-in e diagnóstico operacional
            básico.{" "}
          </p>{" "}
        </div>{" "}
        <span
          className={`rounded-full border px-3 py-1 text-[11px] font-semibold ${isOnline ? "border-emerald-300/45 bg-emerald-400/15 text-emerald-100" : "border-amber-300/45 bg-amber-400/15 text-amber-100"}`}
        >
          {" "}
      ...
      <div className="rounded-2xl border border-white/12 bg-black/20 px-4 py-4">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          {copyState === "ok" ? (
            <span className="text-[12px] text-emerald-200">Copiado.</span>
          ) : null}{" "}
          {copyState === "error" ? (
            <span className="text-[12px] text-rose-200">Falha ao copiar.</span>
          ) : null}{" "}
        </div>{" "}
      </div>{" "}
      <div className="grid gap-3 md:grid-cols-2">
        ...
        <div className="rounded-2xl border border-white/12 bg-black/20 px-4 py-3">
          ...
          <p className="mt-1 text-sm text-white/85">
            {deviceMeta?.timezone ?? "Desconhecido"}
          </p>{" "}
        </div>{" "}
        <div className="rounded-2xl border border-white/12 bg-black/20 px-4 py-3">
          {" "}
          ...
          <p className="mt-1 text-sm text-white/85">
            {deviceMeta?.language ?? "Desconhecido"}
          </p>{" "}
        </div>{" "}
        <div className="rounded-2xl border border-white/12 bg-black/20 px-4 py-3">
          {" "}
          ...
          <p className="mt-1 text-sm text-white/85">
            {deviceMeta?.platform ?? "Desconhecido"}
          </p>{" "}
        </div>{" "}
        <div className="rounded-2xl border border-white/12 bg-black/20 px-4 py-3">
          {" "}
      ...
      <div className="rounded-2xl border border-white/12 bg-black/20 px-4 py-3">
        ...
        <p className="mt-1 break-words text-xs text-white/70">
          {deviceMeta?.userAgent ?? "Desconhecido"}
        </p>{" "}
      </div>{" "}
    </section>
  );
