...
function isCoachEligibleRole(role: OrganizationMemberRole) {
...
function resolveCoachDisplayName(profile: { fullName: string | null; username: string | null } | null) {
...
async function ensureCoachProfessionalLink(params: {
  ...
  const displayName = resolveCoachDisplayName(profile);
...
async function upsertCoachProfile(params: {
  organizationId: number;
  userId: string;
  reservationProfessionalId: number | null;
}) {
...
async function listCoachItems(organizationId: number): Promise<CoachItem[]> {
  const coachProfiles = await prisma.trainerProfile.findMany({
    where: { organizationId },
    select: { id: true, userId: true, reservationProfessionalId: true },
    orderBy: [{ createdAt: "asc" }, { id: "asc" }],
  });
  if (coachProfiles.length === 0) return [];

  const userIds = Array.from(new Set(coachProfiles.map((entry) => entry.userId)));
  ...
  for (const coachProfile of coachProfiles as CoachProfileRow[]) {
    const member = memberByUserId.get(coachProfile.userId);
    if (!member) continue;
    if (!isCoachEligibleRole(member.role)) continue;

    const linkedProfessional =
      coachProfile.reservationProfessionalId != null
        ? professionalById.get(coachProfile.reservationProfessionalId) ?? null
        : null;
    const professional =
      linkedProfessional ??
      pickCanonicalProfessional(professionalByUserId.get(coachProfile.userId) ?? []);
    const profile = profileByUserId.get(coachProfile.userId) ?? null;
...
async function resolveCoachItemForUser(params: {
  organizationId: number;
  userId: string;
}) {
  const { organizationId, userId } = params;
  const items = await listCoachItems(organizationId);
...
async function _GET(req: NextRequest) {
  ...
  try {
    ...
    if (canManageCoaches(membership.role, membership.rolePack ?? null)) {
      await runAcademyTrainerHardCutHygiene(organization.id);
    }

    const items = await listCoachItems(organization.id);
    return respondOk(ctx, { items }, { status: 200 });
  } catch (err) {
    console.error("[org/padel/coaches][GET]", err);
...
async function _POST(req: NextRequest) {
  ...
  try {
    ...
    const emailGate = ensureOrganizationEmailVerified(organization, { reasonCode: "COACHES" });
    if (!emailGate.ok) {
      return respondError(
        ctx,
        {
    ...
    if (!isCoachEligibleRole(targetMembership.role)) {
      return fail(ctx, 400, "Só membros da equipa podem ser treinador.");
    }

    const professionalId = await ensureCoachProfessionalLink({
      organizationId: organization.id,
      userId,
    });
    ...
    const item = await resolveCoachItemForUser({ organizationId: organization.id, userId });
    ...
    console.error("[org/padel/coaches][POST]", err);
