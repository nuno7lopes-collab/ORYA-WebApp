...
type PageProps = {
  params: { username: string; coach: string } | Promise<{ username: string; coach: string }>;
};
...
export default async function CoachProfilePage({ params }: PageProps) {
  ...
  const coachRaw = resolved.coach ?? "";
  const coachParam = isUuid(coachRaw) ? coachRaw : normalizeUsernameInput(coachRaw);
  ...
  if (rawOrgUsername !== orgUsername) {
    redirect(`/${orgUsername}/treinadores/${encodeURIComponent(coachRaw)}`);
  }
  if (!coachParam) {
    notFound();
  }
  if (!isUuid(coachRaw) && coachRaw !== coachParam) {
    redirect(`/${orgUsername}/treinadores/${encodeURIComponent(coachParam)}`);
  }
  ...
  const profile = await prisma.trainerProfile.findFirst({
    where: {
      ...
      user: {
        OR: [
          { username: coachParam },
          ...(isUuid(coachParam) ? [{ id: coachParam }] : []),
        ],
      },
    },
    include: {
  ...
  const certifications = profile.certifications
    ? profile.certifications
        ...
        .filter(Boolean)
    : [];
  const coachCoverUrl = getProfileCoverUrl(
