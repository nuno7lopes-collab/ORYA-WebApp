...
import { parseOrganizationId, parseOrgIdFromPathnameStrict } from "@/lib/organizationIdUtils";
...
export function TournamentLiveManager({ tournamentId }: TournamentLiveManagerProps) {
  ...
  const organizationId =
    parseOrganizationId(searchParams?.get("organizationId")) ?? parseOrgIdFromPathnameStrict(pathname);
  const tournamentApiBase = organizationId ? `/api/org/${organizationId}/tournaments/${tournamentId}` : null;
  ...
  useEffect(() => {
    ...
    if (authError === "organização") {
      router.replace(organizationId ? `/org/${organizationId}/overview` : "/org-hub/organizations");
    }
  }, [authError, organizationId, router]);

  const { data, error } = useSWR(
    isValidTournamentId && tournamentApiBase ? `${tournamentApiBase}/live` : null,
    fetcher,
  );
  const { data: participantsRes, mutate: mutateParticipants } = useSWR(
    isValidTournamentId && tournamentApiBase ? `${tournamentApiBase}/participants` : null,
    fetcher,
  );
  ...
  const saveParticipants = async (options?: { silent?: boolean }) => {
    if (!tournamentApiBase) {
      setParticipantsMessage("Organização inválida.");
      return;
    }
    ...
    try {
      ...
      const res = await fetch(`${tournamentApiBase}/participants`, {
        method: "POST",
  ...
  const generateBracket = async () => {
    if (!tournamentApiBase) {
      setGenerationMessage("Organização inválida.");
      return;
    }
    ...
    try {
      const res = await fetch(`${tournamentApiBase}/generate`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
export default function OrganizationTournamentLivePage({ params }: PageProps) {
  ...
  const isValidTournamentId = Number.isFinite(tournamentId);
  const { data } = useSWR(
    isValidTournamentId && organizationId ? `/api/org/${organizationId}/tournaments/${tournamentId}/live` : null,
    fetcher,
  );
  ...
  useEffect(() => {
    ...
    if (Number.isFinite(eventId) && organizationId) {
      router.replace(`/org/${organizationId}/events/${eventId}/live?tab=bracket`);
    }
  }, [data?.tournament?.event, organizationId, router]);
