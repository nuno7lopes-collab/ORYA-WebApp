...
export async function GET(req: NextRequest) {
  ...
  if (cached) {
    return NextResponse.json({ ok: true, items: cached }, { headers: { "Cache-Control": "public, max-age=300" } });
  }
  ...
  try {
    ...
    return NextResponse.json({ ok: true, items }, { headers: { "Cache-Control": "public, max-age=300" } });
  } catch (err) {
    console.error("[geo/autocomplete] erro", err);
    return NextResponse.json({ ok: false, error: "Falha ao obter sugestões." }, { status: 502 });
  }
}
