...
const steps: Step[] = [
  {
    id: "welcome",
    title: "Bem-vindo ao painel de organização",
...
const TOUR_KEY = "orya_organization_tour_seen_v2";
...
type OrganizationTourProps = {
  organizationId?: number | null;
};

export function OrganizationTour({ organizationId }: OrganizationTourProps) {
  ...
  const tourKey = useMemo(
    () => (organizationId ? `${TOUR_KEY}:${organizationId}` : TOUR_KEY),
    [organizationId],
  );
  const progressKey = useMemo(
    () => (organizationId ? `${TOUR_PROGRESS_KEY}:${organizationId}` : TOUR_PROGRESS_KEY),
    [organizationId],
  );
  ...
  useEffect(() => {
    const id = requestAnimationFrame(() => {
      ...
      if (!seen && !seenGlobal) {
        const savedStep =
          typeof window !== "undefined"
            ? Number(localStorage.getItem(progressKey))
            : 0;
      ...
      const handler = () => {
        if (typeof window !== "undefined" && localStorage.getItem(TOUR_GLOBAL_KEY)) {
          return;
        }
  ...
  const goNext = () => {
    trackEvent("organization_tour_next", { step: index });
  ...
  const finish = () => {
    trackEvent("organization_tour_finish");
