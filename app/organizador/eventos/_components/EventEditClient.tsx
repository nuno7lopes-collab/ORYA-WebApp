...
export function EventEditClient({ event, tickets }: EventEditClientProps) {
  ...
  const eventLabel = isPadel ? "torneio de padel" : "evento";
  const eventLabelTitle = isPadel ? "Torneio de Padel" : "Evento";
  const ticketLabel = isPadel ? "inscrição" : "bilhete";
  const ticketLabelPlural = isPadel ? "inscrições" : "bilhetes";
  const ticketLabelPluralTitle = isPadel ? "Inscrições" : "Bilhetes";
  const ticketLabelIndefinite = isPadel ? "uma inscrição" : "um bilhete";
  const ticketLabelPluralAll = isPadel ? "Todas as inscrições" : "Todos os bilhetes";
    ...
    () =>
      isFree
        ...
        : [
            { key: "base", label: "Essenciais", desc: "Imagem e localização" },
            { key: "dates", label: "Datas & Local", desc: "Início e fim" },
            { key: "tickets", label: `${ticketLabelPluralTitle}`, desc: "Gestão e vendas" },
          ],
    [isFree, ticketLabelPluralTitle],
  );
  ...
  useEffect(() => {
    ...
    padelCategoryLinks.forEach((link) => {
      nextDrafts[link.padelCategoryId] = {
        ...
        liveStreamUrl: link.liveStreamUrl ?? "",
      };
    });
  ...
  const updatePadelCategoryDraft = (categoryId: number, patch: Partial<PadelCategoryDraft>) => {
    setPadelCategoryDrafts((prev) => {
      const current = prev[categoryId] ?? { isEnabled: true, isHidden: false, capacityTeams: "", liveStreamUrl: "" };
  ...
  const handleAddPadelCategory = async () => {
    ...
    try {
      ...
      await mutatePadelEventCategories();
      pushToast(`Categoria adicionada ao ${eventLabel}.`, "success");
    } catch (err) {
    ...
    publicAccessMode === "OPEN"
      ...
      : publicAccessMode === "TICKET"
        ? publicTicketScope === "SPECIFIC"
          ? isPadel
            ? "Só quem tem inscrições selecionadas pode aceder ao público."
            : "Só quem tem bilhetes selecionados pode aceder ao público."
          : isPadel
            ? `Qualquer inscrição do ${eventLabel} dá acesso ao público.`
            : `Qualquer ${ticketLabel} do ${eventLabel} dá acesso ao público.`
  ...
  const participantAccessDescription =
    participantAccessMode === "NONE"
      ...
      : participantAccessMode === "INSCRIPTION"
        ? isPadel
          ? `Participantes são definidos pelas inscrições do ${eventLabel}.`
          : `Participantes são definidos por inscrição do ${eventLabel}.`
        : participantAccessMode === "TICKET"
          ? participantTicketScope === "SPECIFIC"
            ? isPadel
              ? "Participantes apenas com inscrições selecionadas."
              : "Participantes apenas com bilhetes selecionados."
            : isPadel
              ? "Qualquer inscrição marca o utilizador como participante."
              : `Qualquer ${ticketLabel} marca o utilizador como participante.`
          : "Participantes são escolhidos por convite.";
  const accessWarnings = useMemo(() => {
    const warnings: string[] = [];
    if (publicAccessMode === "TICKET") {
      if (ticketList.length === 0) {
        warnings.push(isPadel ? "Sem inscrições criadas: ninguém conseguirá entrar como público." : "Sem bilhetes criados: ninguém conseguirá entrar como público.");
      } else if (publicTicketScope === "SPECIFIC" && publicTicketTypeIds.length === 0) {
        warnings.push(
          isPadel ? "Seleciona pelo menos um tipo de inscrição para o público." : "Seleciona pelo menos um tipo de bilhete para o público.",
        );
      }
    }
    if (participantAccessMode === "TICKET") {
      if (ticketList.length === 0) {
        warnings.push(isPadel ? "Sem inscrições criadas: ninguém será marcado como participante." : "Sem bilhetes criados: ninguém será marcado como participante.");
      } else if (participantTicketScope === "SPECIFIC" && participantTicketTypeIds.length === 0) {
        warnings.push(
          isPadel ? "Seleciona pelo menos um tipo de inscrição para os participantes." : "Seleciona pelo menos um tipo de bilhete para os participantes.",
        );
      }
    }
  ...
  const handleSave = async () => {
    ...
    if (issues.length > 0) {
      applyErrors(issues);
      setValidationAlert(`Revê os campos assinalados antes de guardar o ${eventLabel}.`);
      setError(issues[0]?.message ?? null);
      return;
    }
    ...
    if (hasPaidTicket && paymentsStatus !== "READY") {
      setStripeAlert(
        isPadel
          ? "Podes gerir o torneio de padel, mas só aceitar inscrições pagas depois de ligares o Stripe."
          : "Podes gerir o evento, mas só vender bilhetes pagos depois de ligares o Stripe.",
      );
      setError(
        isPadel
          ? "Liga o Stripe em Finanças & Payouts para aceitar inscrições pagas."
          : "Liga o Stripe em Finanças & Payouts para vender bilhetes pagos.",
      );
    ...
    if (publicAccessMode === "TICKET") {
      if (ticketList.length === 0) {
        setValidationAlert(`Cria pelo menos ${ticketLabelIndefinite} antes de definir acesso por ${ticketLabel}.`);
        pushToast(`Cria ${ticketLabelPlural} para o acesso do público.`);
        return;
      }
      if (publicTicketScope === "SPECIFIC" && publicTicketTypeIds.length === 0) {
        setValidationAlert(`Seleciona pelo menos um tipo de ${ticketLabel} para o acesso do público.`);
        pushToast(`Seleciona ${ticketLabelPlural} para o acesso do público.`);
        return;
      }
    }
    if (participantAccessMode === "TICKET") {
      if (ticketList.length === 0) {
        setValidationAlert(`Cria pelo menos ${ticketLabelIndefinite} antes de definir participantes por ${ticketLabel}.`);
        pushToast(`Cria ${ticketLabelPlural} para participantes.`);
        return;
      }
      if (participantTicketScope === "SPECIFIC" && participantTicketTypeIds.length === 0) {
        setValidationAlert(`Seleciona pelo menos um tipo de ${ticketLabel} para os participantes.`);
        pushToast(`Seleciona ${ticketLabelPlural} para participantes.`);
        return;
      }
    }
    if (
      ...
      setValidationAlert("Seleciona uma categoria Padel para a nova inscrição.");
      pushToast("Seleciona a categoria da inscrição.");
      return;
    }
    ...
    try {
      ...
      const json = await res.json().catch(() => null);
      if (!res.ok || !json?.ok) {
        throw new Error(json?.error || `Erro ao atualizar ${eventLabel}.`);
      }

      setMessage(`${eventLabelTitle} atualizado com sucesso.`);
      pushToast(`${eventLabelTitle} atualizado com sucesso.`, "success");
      setEndingIds([]);
      ...
      setBackendAlert(err instanceof Error ? err.message : `Erro ao atualizar ${eventLabel}.`);
      pushToast(err instanceof Error ? err.message : `Erro ao atualizar ${eventLabel}.`);
      ctaRef.current?.scrollIntoView({ behavior: "smooth", block: "center" });
    } finally {
      setIsSaving(false);
    }
  ...
  const confirmEnd = async () => {
    ...
    try {
      ...
      const closedLabel = isPadel ? "esta inscrição" : "este bilhete";
      setMessage(`Venda terminada para ${closedLabel}.`);
      pushToast(`Venda terminada para ${closedLabel}.`, "success");
    } catch (err) {
    ...
    const baseBlock = (
      <div className="space-y-4">
        ...
        <div className="space-y-1">
          ...
          <p className="text-[11px] text-white/55">O template não pode ser alterado depois de criar o {eventLabel}.</p>
        </div>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 px-3 py-3 text-sm text-white/75">
          <p className="font-semibold text-white">{isPadel ? "Torneio grátis" : "Evento grátis"}</p>
          <p className="text-[12px] text-white/65">
            ...
            {isFree && (
              <span className="block text-[12px] text-white/60 mt-1">
                Vagas/{ticketLabelPlural}: {freeCapacity != null ? freeCapacity : "Sem limite definido"}.
              </span>
            )}
          </p>
        </div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <>
                      {ticketList.length === 0 && (
                        <p className="text-[11px] text-white/50">
                          {isPadel ? "Ainda não existem inscrições." : "Ainda não existem bilhetes."}
                        </p>
                      )}
            ...
            <div className="space-y-2">
              ...
              {participantAccessMode === "TICKET" && (
                <div className="space-y-2 rounded-xl border border-white/10 bg-black/30 px-3 py-2">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <p className="text-[12px] text-white/70">Participantes por {ticketLabel}</p>
                    <div className="flex flex-wrap gap-2">
                      {([
                        { value: "ALL", label: ticketLabelPluralAll },
                        { value: "SPECIFIC", label: "Tipos específicos" },
                  ...
                  {participantTicketScope === "ALL" && (
                    <p className="text-[11px] text-white/55">
                      {isPadel ? "Qualquer inscrição identifica o participante." : "Qualquer bilhete identifica o participante."}
                    </p>
                  )}
                  {participantTicketScope === "SPECIFIC" && (
                    <>
              ...
              {participantAccessMode === "INSCRIPTION" && (
                <p className="text-[11px] text-white/55">
                  Participantes são definidos pelas inscrições do {eventLabel} (sem necessidade de {ticketLabel} específico).
                </p>
              )}
            </div>
          </div>
    ...
    const ticketsBlock = (
      <div className="space-y-3">
        ...
        {isPadel && (
          <div className="rounded-xl border border-white/12 bg-black/25 p-3 space-y-3">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <p className="text-[11px] text-white/60">
                  Ativa as categorias que aceitam inscrições neste {eventLabel}. Refunds são geridos manualmente nas inscrições.
                </p>
              </div>
              <button
                type="button"
            ...
            {padelCategoryLinks.length === 0 ? (
              <p className="text-[11px] text-white/60">Sem categorias associadas ao {eventLabel}.</p>
            ) : (
              <div className="space-y-2">
                  ...
                  return (
                    <div key={link.id} className="rounded-lg border border-white/10 bg-black/30 p-3 space-y-3">
                      ...
                      <div className="grid gap-2 sm:grid-cols-2">
                        ...
                        <label className="text-[11px] text-white/70 sm:col-span-2">
                          URL da livestream (opcional)
                          <input
                            type="url"
                            value={draft.liveStreamUrl}
                            onChange={(e) => updatePadelCategoryDraft(link.padelCategoryId, { liveStreamUrl: e.target.value })}
            ...
            {availablePadelCategories.length === 0 && padelCategories.length > 0 && (
              <p className="text-[11px] text-white/60">Todas as categorias do organizador já estão ligadas ao {eventLabel}.</p>
            )}
            {padelCategories.length === 0 && (
              <p className="text-[11px] text-white/60">
                Cria categorias no Hub Padel para poderes adicioná-las ao {eventLabel}.
              </p>
            )}
          </div>
        )}
        ...
        <div className="rounded-xl border border-white/12 bg-black/25 p-3 space-y-2">
          <p className="text-[12px] font-semibold">{isPadel ? "Adicionar nova inscrição" : "Adicionar novo bilhete"}</p>
          {isPadel && activePadelCategoryLinks.length === 0 && (
            <p className="text-[11px] text-amber-200">
              Cria categorias Padel no hub e associa-as ao {eventLabel} antes de adicionar inscrições.
            </p>
          )}
          ...
          <p className="text-[11px] text-white/50">
            {isPadel
              ? "Nova inscrição fica ON_SALE por padrão. Não removemos inscrições antigas para manter histórico."
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
          ...
          <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3 text-sm text-white/80">
            <div className="rounded-xl border border-white/10 bg-black/20 p-3">
              <p className="text-[11px] uppercase tracking-wide text-white/60">{eventLabelTitle}</p>
            ...
            <div className="rounded-xl border border-white/10 bg-black/20 p-3 space-y-1">
              ...
              <p className="font-semibold">
                {isFree ? (isPadel ? "Torneio grátis" : "Evento grátis") : isPadel ? "Torneio pago" : "Evento pago"}
              </p>
              {isFree && (
                <p className="text-white/70">
                  Vagas/{ticketLabelPlural}: {freeCapacity != null ? freeCapacity : "Sem limite definido"}.
          ...
          <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/70 backdrop-blur">
            <div className="w-full max-w-sm rounded-2xl border border-white/15 bg-black/90 p-5 shadow-[0_20px_60px_rgba(0,0,0,0.85)] space-y-3">
              <h3 className="text-lg font-semibold">
                {isPadel ? "Terminar venda da inscrição?" : "Terminar venda do bilhete?"}
              </h3>
              <p className="text-sm text-white/70">
                {isPadel
                  ? "Esta ação é definitiva para este tipo de inscrição. Escreve "
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="text-lg font-semibold text-white">
                {isPadel ? "Editar torneio de padel" : "Editar evento"}
              </p>
              <p className="text-sm text-white/60">
                Define o teu {eventLabel} passo a passo. Podes guardar como rascunho em qualquer momento.
              </p>
            </div>
