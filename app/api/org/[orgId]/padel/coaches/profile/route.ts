...
async function _GET(req: NextRequest) {
  ...
  try {
    ...
    console.error("[organizacao/coaches/profile][GET]", err);
...
async function _PATCH(req: NextRequest) {
  ...
  try {
    ...
    const emailGate = ensureOrganizationEmailVerified(organization, { reasonCode: "COACH_PROFILE" });
    if (!emailGate.ok) {
      return respondError(
        ctx,
        {
    ...
    console.error("[organizacao/coaches/profile][PATCH]", err);
    return fail(ctx, 500, "INTERNAL_ERROR");
  }
}
