...
"[project]/app/api/padel/standings/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(req) {
    try {
        ...
        groups.forEach((rows, label)=>{
            ...
            const sortWithMiniLeague = (entries)=>{
            ...
            const groupedByPoints = list.reduce((acc, row)=>{
                acc[row.points] = acc[row.points] || [];
                acc[row.points].push(row);
                return acc;
            }, {});
            const resolved = [];
            const pointKeys = Object.keys(groupedByPoints).map(Number).sort((a, b)=>b - a);
            pointKeys.forEach((pts)=>{
                ...
                if (cluster.length <= 1) {
                    ...
                    scoredMatches.filter((m)=>m.groupLabel === label && cluster.some((c)=>c.pairingId === m.pairingAId || c.pairingId === m.pairingBId)).forEach((m)=>{
                        if (!m.pairingAId || !m.pairingBId) return;
...
//# sourceMappingURL=%5Broot-of-the-server%5D__d24a7e4d._.js.map