...
- Public NS (dig @8.8.8.8):
  ...
  - ns-1234.awsdns-26.org.
  - ns-633.awsdns-15.net.
- MX (public): orya.pt → inbound-smtp.eu-west-1.amazonaws.com (prio 10)
...
- Receipt rule: orya-inbound-store
  - Actions: S3 (bucket, prefix inbound/), WorkMail delivery, SNS publish
...
- Receipt Rule updated to include WorkMailAction → OrganizationArn.

## Lambda forwarding fallback (optional)
- Lambda function: arn:aws:lambda:eu-west-1:495219734037:function:orya-ses-forwarder
- Role: arn:aws:iam::495219734037:role/orya-ses-forwarder-role
- Default env: FORWARD_TO=admin@orya.pt, SOURCE_EMAIL=admin@orya.pt
- Not enabled by default (no S3 notification configured). To enable:
  - Configure S3 bucket notification on `orya-mail-inbound-prod` for prefix `inbound/` to invoke `orya-ses-forwarder`.
...
- WorkMail delivery proof: requires IMAP/Web login (credentials stored in Secrets Manager).
...
## Notes
- Inbound mail is working via SES+S3. WorkMail delivery is configured but needs login verification.
- External senders should work once MX propagated (already OK). Deliveries can be verified via WorkMail or S3.
