...
const normalizePadelFormatList = (value: unknown): PadelFormat[] | null => {
  if (!Array.isArray(value)) return null;
  const mapped = value.map((item) => normalizePadelFormat(item)).filter(Boolean) as PadelFormat[];
...
export async function POST(req: NextRequest) {
  ...
  const format = normalizePadelFormat(body.format);
  ...
  const enabledFormats = normalizePadelFormatList(body.enabledFormats);
  const groupsConfig =
  ...
  try {
    ...
    const config = await prisma.padelTournamentConfig.upsert({
      ...
      create: {
        ...
        enabledFormats: enabledFormats?.filter((f) => allowedFormats.has(f)) ?? undefined,
