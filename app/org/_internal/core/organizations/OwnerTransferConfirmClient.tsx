    ...
    async function run() {
      ...
      try {
        const res = await fetch(
          resolveCanonicalOrgApiPath(`/api/org-hub/groups/${groupId}/owner/transfer/confirm`),
          {
            method: "POST",
        ...
        if (!res.ok || json?.ok === false) {
          throw new Error(extractErrorMessage(json) ?? "Não foi possível confirmar a transferência.");
        }
        if (!cancelled) {
          setState("ok");
