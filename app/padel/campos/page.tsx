...
async function fetchCourts() {
  ...
  const res = await fetch(`${origin}/api/padel/public/services?kind=COURT&limit=24`, {
    cache: "no-store",
  });
    ...
    <main className="min-h-screen bg-[linear-gradient(180deg,#0b1014_0%,#0d1320_50%,#101826_100%)] text-white">
      <section className="orya-page-width px-6 pb-8 pt-12 md:px-10">
        <div className="rounded-3xl border border-white/12 bg-gradient-to-br from-white/8 via-[#0b1226]/75 to-[#050810]/90 p-6 shadow-[0_24px_65px_rgba(0,0,0,0.55)]">
          <p className="text-[11px] uppercase tracking-[0.28em] text-white/60">Campos</p>
          <h1 className="mt-2 text-3xl font-semibold">Reserva courts de padel</h1>
          <p className="mt-2 text-sm text-white/70">
            Consulta disponibilidade pública e reserva campos com preços transparentes.
          </p>
          <Link
            href="/padel"
      ...
      <section className="orya-page-width px-6 pb-16 md:px-10">
        {items.length === 0 ? (
          <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-4 text-sm text-white/60">
            Sem campos públicos disponíveis neste momento.
          </div>
        ) : (
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {items.map((item) => {
              ...
              return (
                ...
                >
                  <p className="text-[11px] uppercase tracking-[0.2em] text-white/55">Campo</p>
