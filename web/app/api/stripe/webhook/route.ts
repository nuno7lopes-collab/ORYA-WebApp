...
export async function handleRefund(charge: Stripe.Charge) {
  ...
  const isFullRefund = chargeAmount > 0 && refundedAmount >= chargeAmount;
  ...
  if (!isFullRefund) {
    ...
    console.log("[handleRefund] partial refund detected", { paymentIntentId, refundedAmount, chargeAmount });
    return;
  }
