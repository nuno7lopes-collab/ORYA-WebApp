...
export default function AdminSettingsPage() {
  ...
  const { data: mfaStatusData, isLoading: isMfaStatusLoading } = useSWR<MfaStatusResponse>(
      ...
      <section className="space-y-6">
        ...
        <div className="admin-card p-4">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              <p className="text-[11px] uppercase tracking-[0.2em] text-white/60">Segurança</p>
              <h2 className="mt-2 text-lg font-semibold text-white/90">2FA do Admin</h2>
              <p className="text-sm text-white/60">
                Controla o estado da autenticação de dois fatores para esta conta administrativa.
              </p>
            </div>
            <Link href="/admin/mfa" className="admin-button-secondary">
              Gerir 2FA
            </Link>
          </div>
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-3">
            <div className="admin-card-soft p-3">
              <p className="text-[10px] uppercase tracking-[0.2em] text-white/45">Estado</p>
              <p className="text-sm text-white/85">
                {isMfaStatusLoading
                  ? "A carregar…"
                  : mfaStatusData && mfaStatusData.ok
                    ? mfaStatusData.enabled
                      ? "Ativo"
                      : "Inativo"
                    : "Indisponível"}
              </p>
              {mfaStatusData && mfaStatusData.ok && mfaStatusData.configMissing && (
                <p className="text-[11px] text-amber-200">ADMIN_TOTP_ENCRYPTION_KEY em falta.</p>
              )}
            </div>
            <div className="admin-card-soft p-3">
              <p className="text-[10px] uppercase tracking-[0.2em] text-white/45">Última atualização</p>
              <p className="text-sm text-white/85">
                {mfaStatusData && mfaStatusData.ok && mfaStatusData.updatedAt
                  ? new Date(mfaStatusData.updatedAt).toLocaleDateString("pt-PT")
                  : "-"}
              </p>
            </div>
            <div className="admin-card-soft p-3">
              <p className="text-[10px] uppercase tracking-[0.2em] text-white/45">Recovery codes</p>
              <p className="text-sm text-white/85">
                {mfaStatusData && mfaStatusData.ok && typeof mfaStatusData.recoveryUnused === "number"
                  ? `${mfaStatusData.recoveryUnused} disponíveis`
                  : "-"}
              </p>
            </div>
          </div>
