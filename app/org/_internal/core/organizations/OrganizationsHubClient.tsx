...
export default function OrganizationsHubClient({ initialOrgs, activeId }: Props) {
  ...
  const handleSwitch = async (organizationId: number, redirectToDashboard = false) => {
    ...
    try {
      ...
      if (redirectToDashboard) {
        ...
        router.replace(`/org/overview?organizationId=${organizationId}`);
        setTimeout(() => {
          if (window?.location?.href.includes(`organizationId=${organizationId}`) === false) {
            window.location.href = `/org/overview?organizationId=${organizationId}`;
          }
        }, 50);
      } else {
    ...
    try {
      ...
      if (!res.ok || json?.ok === false) {
        ...
        if (newId) {
          ...
          router.push(appendOrganizationIdToHref("/org/overview", currentActive));
        }
      }
    } catch (err) {
  ...
  const renderOrgCard = (item: OrgItem) => {
    ...
    const handleCardClick = () => {
      if (isActive) {
        router.push(`/org/overview?organizationId=${item.organizationId}`);
        return;
      }
        ...
        {!loading && !hasError && !emptyState && (
          <section className="space-y-4">
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              ...
              <button
                type="button"
                onClick={() => router.push(appendOrganizationIdToHref("/org/become", currentActive))}
