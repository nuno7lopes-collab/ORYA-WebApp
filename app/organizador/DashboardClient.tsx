...
type ObjectiveTab = "manage" | "promote" | "analyze" | "profile";
type OrgCategory = "EVENTOS" | "PADEL" | "VOLUNTARIADO";

const OBJECTIVE_TABS: ObjectiveTab[] = ["manage", "promote", "analyze", "profile"];
...
const mapTabToObjective = (tab?: string | null): ObjectiveTab => {
  if (OBJECTIVE_TABS.includes((tab as ObjectiveTab) || "manage")) {
    return (tab as ObjectiveTab) || "manage";
  }
  switch (tab) {
    case "overview":
      return "manage";
...
function OrganizadorPageInner({ hasOrganizer }: { hasOrganizer: boolean }) {
  ...
  useEffect(() => {
    ...
    if (tabParam === "overview") {
      router.replace(`/organizador?tab=manage${orgSuffix}`, { scroll: false });
      return;
    }
    if (tabParam === "staff") {
  ...
  const categoryCta = useMemo(() => {
    if (orgCategory === "PADEL") {
      return { label: "Criar torneio de padel", href: "/organizador/eventos/novo?preset=padel" };
    }
    if (orgCategory === "VOLUNTARIADO") {
      return { label: "Criar evento", href: "/organizador/eventos/novo?preset=voluntariado" };
    }
    return { label: "Criar evento", href: "/organizador/eventos/novo" };
  }, [orgCategory]);
  ...
  useEffect(() => {
    ...
    const manageTargets = showPadelHub ? ["eventos", "padel-hub"] : ["eventos", "inscricoes"];
  ...
  }, [scrollSection, activeObjective, showPadelHub]);
  ...
  useEffect(() => {
    ...
    router.replace(`${pathname}?${params.toString()}`, { scroll: false });
  }, [activeObjective, pathname, router, searchParams, sectionParamRaw, showPadelHub]);
    ...
    if (paymentsStatus === "PENDING") {
      return {
        ...
        desc: hasReqs
          ...
          : `Conclui o onboarding no Stripe para começares a receber os pagamentos das tuas ${ticketLabelPluralTitle.toLowerCase()}.`,
  ...
  }, [eventLabelPlural, paymentsStatus, stripeRequirements, ticketLabelPluralTitle]);
  ...
  const exportFinanceCsv = useCallback(() => {
    ...
    const header = ["ID", eventLabelTitle, ticketLabelPluralTitle, "Bruto (€)", "Taxas (€)", "Líquido (€)", "Estado", "Data"];
  ...
  }, [eventLabelTitle, financeData, ticketLabelPluralTitle]);

  const handleExportSalesCsv = useCallback(() => {
    if (!salesSeries?.points?.length || !selectedSalesEvent) return;
    const header = ["Data", ticketLabelPluralTitle, "Bruto (€)", "Desconto (€)", "Taxas (€)", "Líquido (€)"];
    ...
    a.click();
    URL.revokeObjectURL(url);
  }, [salesRange, salesSeries?.points, selectedSalesEvent, ticketLabelPluralTitle]);
  ...
  const activeSection = useMemo(() => {
    const manageSections = showPadelHub ? ["eventos", "padel-hub"] : ["eventos", "inscricoes"];
    ...
    const allowed = baseSections[activeObjective] ?? ["eventos"];
    const candidate =
      normalizedSection ??
      (activeObjective === "analyze"
        ? "financas"
  ...
  return (
    <div className={`${containerClasses} space-y-6 text-white`}>
          ...
          <div className="relative overflow-hidden rounded-3xl border border-white/12 bg-gradient-to-r from-[#0b1226]/80 via-[#101b39]/75 to-[#050811]/90 p-5 shadow-[0_30px_110px_rgba(0,0,0,0.6)] backdrop-blur-3xl">
            ...
            <div className="relative space-y-4">
              <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div className="space-y-1">
                  <p className="text-[11px] uppercase tracking-[0.26em] text-white/70">{eventLabelPluralTitle}</p>
                  <h2 className="text-2xl font-semibold text-white drop-shadow-[0_14px_40px_rgba(0,0,0,0.45)]">Vista geral</h2>
                  <p className="text-sm text-white/80">Seleciona um {eventLabel} e clica em Gerir para aceder a tudo.</p>
                </div>
                <Link href={categoryCta.href} className={cn(CTA_PRIMARY, "text-[12px]")}>
                  {categoryCta.label}
                </Link>
              </div>
            ...
            {eventsError && (
              <div className="rounded-2xl border border-red-500/40 bg-red-500/10 px-4 py-3 text-sm text-red-100 flex items-center justify-between gap-3">
                <div>
                  <p className="font-semibold">Não foi possível carregar os {eventLabelPlural}.</p>
            ...
            {!eventsListLoading && events?.items?.length === 0 && (
              <div className="rounded-2xl border border-dashed border-white/15 bg-white/5 px-4 py-6 text-center text-sm text-white/70 space-y-2">
                <p className="text-base font-semibold text-white">Ainda não tens {eventLabelPlural} criados.</p>
                <p>Começa por criar o teu primeiro {eventLabel} e acompanha tudo a partir daqui.</p>
              </div>
            )}
            ...
            {!eventsListLoading && events?.items && events.items.length > 0 && filteredEvents.length === 0 && (
              <div className="rounded-2xl border border-white/15 bg-white/5 px-4 py-6 text-center text-sm text-white/70 space-y-2">
                <p className="text-base font-semibold text-white">Nenhum {eventLabel} corresponde a estes filtros.</p>
                  ...
                  <div className="overflow-hidden rounded-3xl border border-white/16 bg-gradient-to-br from-white/18 via-[#15284c]/75 to-[#070d19]/92 shadow-[0_34px_110px_rgba(0,0,0,0.62)] backdrop-blur-3xl">
                    <table className="min-w-full text-sm text-white/90">
                      <thead className="bg-white/10 text-left text-[11px] uppercase tracking-wide text-white/75">
                        <tr>
                          <th className="px-4 py-3 font-semibold">{eventLabelTitle}</th>
                          <th className="px-4 py-3 font-semibold">Data</th>
                          <th className="px-4 py-3 font-semibold">Estado</th>
                          <th className="px-4 py-3 font-semibold">Tipo</th>
                          <th className="px-4 py-3 font-semibold">{ticketLabelPluralTitle}</th>
                          <th className="px-4 py-3 font-semibold">Receita</th>
                          <th className="px-4 py-3 text-right font-semibold">Gerir</th>
                        </tr>
                      </thead>
                            ...
                            <tr key={ev.id} className="hover:bg-white/10 transition duration-150">
                              ...
                              <td className="px-4 py-3 text-right text-[11px]">
                                <div className="flex flex-wrap items-center justify-end gap-2">
                                  <Link
                                    href={`/organizador/eventos/${ev.id}`}
                                    className={cn(CTA_PRIMARY, "px-3 py-1 text-[11px]")}
                                  >
                                    Gerir
                                  </Link>
                                  {ev.status === "ARCHIVED" ? (
      ...
      {!isLegacyStandaloneTab && activeObjective === "manage" && activeSection === "inscricoes" && supportsInscricoes && (
      ...
      {!isLegacyStandaloneTab && activeObjective === "profile" && (
          ...
          <div className="rounded-3xl border border-white/12 bg-gradient-to-r from-[#0b1226]/80 via-[#101b39]/75 to-[#050811]/90 px-4 py-4 sm:px-6 sm:py-5 backdrop-blur-2xl shadow-[0_26px_90px_rgba(0,0,0,0.55)]">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="space-y-1">
                ...
                <p className="text-sm text-white/70">
                  Pré-visualiza a tua página pública e edita o que aparece ao público.
                </p>
              </div>
              <div className="flex flex-wrap items-center gap-2 text-[11px] text-white/70">
                {publicProfileUrl ? (
                  ...
                  <span className="rounded-full border border-white/15 bg-white/5 px-3 py-2 text-[11px] text-white/65">
                    Define um @username para teres link público
                  </span>
                )}
              </div>
            </div>
        ...
        <section className={cn("space-y-4", fadeClass)} id="vendas">
          <div className="relative overflow-hidden rounded-3xl border border-white/12 bg-gradient-to-br from-white/8 via-[#0b1124]/75 to-[#050810]/92 p-5 shadow-[0_26px_90px_rgba(0,0,0,0.6)] backdrop-blur-3xl space-y-4">
            ...
            <div className="relative flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div>
                <p className="text-[11px] uppercase tracking-[0.3em] text-white/70">
                  {ticketLabelPluralTitle} &amp; Vendas
                </p>
                <h2 className="text-2xl font-semibold text-white">Vendas por {eventLabel}</h2>
                <p className="text-sm text-white/70">Escolhe um {eventLabel} e vê evolução + compradores.</p>
              </div>
            ...
            <div className="relative flex flex-wrap items-center gap-3">
              ...
              {!eventsList.length && <span className="text-[12px] text-white/65">Sem {eventLabelPlural} para analisar.</span>}
              {selectedSalesEvent && (
          ...
          <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
            {!salesEventId && (
              <div className="col-span-full rounded-2xl border border-dashed border-white/20 bg-black/30 p-4 text-white/70 text-sm">
                Seleciona um {eventLabel} para ver as métricas de vendas.
              </div>
            )}
            {salesLoading && (
              <>
            ...
            {!salesLoading && salesSeries && salesSeries.points?.length === 0 && (
              <div className="col-span-full rounded-2xl border border-dashed border-white/20 bg-black/30 p-4 text-white/70 text-sm">
                Sem dados de vendas neste período. Escolhe outro {eventLabel} ou intervalo.
              </div>
            )}
            {!salesLoading && salesSeries && salesSeries.points?.length !== 0 && (
              <>
                ...
                <div className="rounded-2xl border border-white/12 bg-gradient-to-br from-white/8 via-[#0b1226]/70 to-[#050912]/90 p-3 shadow-[0_14px_45px_rgba(0,0,0,0.5)]">
                  <p className="text-[11px] text-white/60">{ticketSalesLabel}</p>
                ...
                <div className="rounded-2xl border border-white/12 bg-gradient-to-br from-white/8 via-[#0b1226]/70 to-[#050912]/90 p-3 shadow-[0_14px_45px_rgba(0,0,0,0.5)]">
                  <p className="text-[11px] text-white/60">{eventLabelPluralTitle} com vendas</p>
                  <p className="text-2xl font-bold text-white mt-1">{salesKpis.eventsWithSales}</p>
                  <p className="text-[11px] text-white/50">{eventLabelPluralTitle} com pelo menos 1 venda</p>
                </div>
                <div className="rounded-2xl border border-white/12 bg-gradient-to-br from-white/8 via-[#0b1226]/70 to-[#050912]/90 p-3 shadow-[0_14px_45px_rgba(0,0,0,0.5)]">
                  ...
                  <p className="text-[11px] text-white/50">Calculado nos {eventLabelPlural} com capacidade</p>
                </div>
              </>
            )}
          </div>
            ...
            <div className="rounded-2xl border border-white/12 bg-gradient-to-br from-white/6 via-[#0b1224]/60 to-white/0 shadow-inner overflow-hidden px-2 py-3 min-h-[260px]">
              {salesLoading ? (
                <div className="flex w-full items-center gap-3 px-4">
                  ...
                  <span className="text-white/40 text-xs">Escolhe um {eventLabel} para ver a evolução.</span>
                ...
                <span className="text-white/40 text-xs">Sem dados de vendas para este {eventLabel}.</span>
              )}
            </div>
          ...
          <div className="relative overflow-hidden rounded-3xl border border-white/12 bg-gradient-to-br from-white/8 via-[#0a1226]/75 to-[#050912]/90 p-4 space-y-3 shadow-[0_26px_90px_rgba(0,0,0,0.6)]">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-lg font-semibold">{eventLabelPluralTitle} com mais vendas</h3>
                <p className="text-[11px] text-white/60">Top por receita total. Usa como atalho para ver o detalhe.</p>
              </div>
            </div>

            {topEvents.length === 0 && <p className="text-sm text-white/60">Ainda sem {eventLabelPlural} com vendas para ordenar.</p>}
            {topEvents.length > 0 && (
              <div className="overflow-auto">
                <table className="min-w-full text-sm">
                  <thead className="text-left text-[11px] text-white/60">
                    <tr>
                      <th className="py-2 pr-3">{eventLabelTitle}</th>
                      <th className="py-2 pr-3">{ticketLabelPluralTitle}</th>
          ...
          <div className="rounded-3xl border border-white/10 bg-black/40 p-4 space-y-3 shadow-[0_18px_60px_rgba(0,0,0,0.65)]">
            ...
            {!buyersLoading && !salesEventId && (
              <p className="text-sm text-white/60">Escolhe um {eventLabel} para ver compradores.</p>
            )}
            ...
            {!buyersLoading && salesEventId && buyers && buyers.ok !== false && buyersItems.length === 0 && (
              <p className="text-sm text-white/60">Sem compras registadas para este {eventLabel}.</p>
            )}
            {!buyersLoading && salesEventId && buyers && buyers.ok !== false && buyersItems.length > 0 && (
              <div className="overflow-auto">
                <table className="min-w-full text-sm">
                  <thead className="text-left text-[11px] text-white/60">
                    <tr>
                      ...
                      <th className="py-2 pr-3">{ticketLabelTitle}</th>
                      <th className="py-2 pr-3">Estado</th>
      ...
      {!isLegacyStandaloneTab && activeObjective === "analyze" && activeSection === "financas" && (
        <section className={cn("space-y-5", fadeClass)} id="financas">
            ...
            <div className="rounded-3xl border border-white/12 bg-gradient-to-br from-white/10 via-[#0b1124]/70 to-[#050810]/90 backdrop-blur-3xl p-4 space-y-3 shadow-[0_22px_70px_rgba(0,0,0,0.65)]">
              ...
              <div className="grid gap-2 sm:grid-cols-2 text-sm">
                ...
                <div className="rounded-2xl border border-white/12 bg-white/8 p-3">
                  ...
                  <p className="text-[11px] text-white/60">Inclui todos os {eventLabelPlural}.</p>
                </div>
              </div>
              <div className="grid gap-2 sm:grid-cols-2 text-sm">
                <div className="rounded-2xl border border-white/12 bg-white/8 p-3">
                ...
                <div className="rounded-2xl border border-white/12 bg-white/8 p-3">
                  <p className="text-white/70 text-xs">{eventLabelPluralTitle} com vendas</p>
                  <p className="text-xl font-semibold text-white">
          ...
          <div className="rounded-3xl border border-white/12 bg-gradient-to-br from-white/8 via-[#0b1124]/70 to-[#050810]/92 backdrop-blur-3xl p-4 space-y-3 shadow-[0_22px_70px_rgba(0,0,0,0.65)]">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-lg font-semibold text-white">Por {eventLabel}</h3>
                <p className="text-[12px] text-white/65">Bruto, taxas e líquido por {eventLabel}.</p>
              </div>
              <div className="flex items-center gap-2">
                <button
                  type="button"
            ...
            {financeData && financeData.events.length === 0 && (
              <div className="rounded-2xl border border-dashed border-white/15 bg-white/5 px-4 py-4 text-sm text-white/70">
                Sem vendas ainda. Assim que venderes {ticketLabelPluralTitle.toLowerCase()}, verás aqui os totais por {eventLabel}.
              </div>
            )}
            {stripeSuccessMessage && (
              ...
              <div className="overflow-auto">
                <table className="min-w-full text-sm text-white/80">
                  <thead className="text-left text-[11px] uppercase tracking-wide text-white/60">
                    <tr>
                      <th className="px-4 py-3">{eventLabelTitle}</th>
                      <th className="px-4 py-3">{ticketLabelPluralTitle}</th>
    ...
    <section className="space-y-5">
      ...
      >
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="space-y-1">
            ...
            <p className="text-sm text-white/70">Promoções, audiência e ações para encher o {eventLabel}.</p>
          </div>
        </div>
          ...
          <div className={cn("mt-4 space-y-4", fadeClass)}>
            ...
            <div className="rounded-3xl border border-white/12 bg-gradient-to-br from-white/8 via-[#0c162c]/65 to-[#050912]/90 p-4 space-y-3 shadow-[0_24px_90px_rgba(0,0,0,0.55)]">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-[12px] text-white/65">Próximos {eventLabelPlural} com ocupação e ação sugerida.</p>
                </div>
              ...
              {fillTheRoomEvents.length === 0 && (
                <div className="rounded-2xl border border-dashed border-white/15 bg-white/5 px-4 py-4 text-sm text-white/70">
                  Sem {eventLabelPlural} futuros para otimizar. Cria um {eventLabel} ou define datas para ver sugestões.
                </div>
              )}
            ...
            <div className="rounded-3xl border border-white/12 bg-gradient-to-br from-white/8 via-[#101b39]/60 to-[#050912]/90 p-4 shadow-[0_24px_90px_rgba(0,0,0,0.55)]">
              <div className="flex items-center justify-between gap-2">
                <div>
                  ...
                  <p className="text-[12px] text-white/65">
                    {ticketLabelPluralTitle} totais vs. com promo vs. convidados.
                  </p>
                </div>
              ...
              <div className="mt-3 grid gap-3 md:grid-cols-3">
                {[
                  { label: `${ticketLabelPluralTitle} totais`, value: marketingKpis.totalTickets ?? "—" },
                  { label: `${ticketLabelPluralTitle} com promo`, value: marketingKpis.ticketsWithPromo ?? 0 },
          ...
          <div className={cn("mt-4 space-y-3", fadeClass)}>
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-[12px] text-white/65">Copiar e partilhar: textos rápidos por {eventLabel}.</p>
              </div>
            ...
            <div className="rounded-3xl border border-white/10 bg-black/35 p-4 text-sm text-white/70">
              Em breve: kits rápidos para Instagram, WhatsApp e email por {eventLabel}, com botões de copiar.
            </div>
          </div>
        )}
      </div>
      ...
      {eventDialog && (
        <ConfirmDestructiveActionDialog
          ...
          description={
            eventDialog.mode === "delete"
              ? `Esta ação remove o rascunho e ${ticketLabelPluralTitle.toLowerCase()} associadas.`
              : eventDialog.mode === "unarchive"
                ? `O ${eventLabel} volta a aparecer nas listas e dashboards.`
                : `O ${eventLabel} deixa de estar visível para o público. Vendas e relatórios mantêm-se.`
          }
          consequences={
            eventDialog.mode === "delete"
          ...
          confirmLabel={
            eventDialog.mode === "delete" ? "Apagar rascunho" : eventDialog.mode === "unarchive" ? `Reativar ${eventLabel}` : `Arquivar ${eventLabel}`
          }
