...
async function _POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!targetState) {
    return jsonWrap({ ok: false, error: "INVALID_TARGET_STATE" }, { status: 400 });
  }
  ...
  const { match: updated, outboxEventId } = await updatePadelMatch({
    ...
    eventType: "PADEL_MATCH_PENDING_RESET",
    outboxEventType: "PADEL_MATCH_PENDING_RESET",
    data: {
      status: transition.status,
  ...
  await recordOrganizationAuditSafe({
    organizationId: context.match.organizationId,
    actorUserId: auth.user.id,
    action: "PADEL_MATCH_PENDING_RESET",
    metadata: {
      matchId: context.match.id,
      eventId: context.match.eventId,
