...
async function ensureOrganizer(userId: string, eventId: number) {
...
export async function POST(req: NextRequest) {
  ...
  const access = await ensureOrganizer(userId, eventId);
  if (!access.ok) {
  ...
  try {
    ...
    console.error("[organizador/checkin] error", err);
