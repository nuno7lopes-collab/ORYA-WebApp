...
- External senders should work once MX propagated (already OK). Deliveries verified via S3; inbox confirmation pending.
 - IMPORTANT: With MX still pointing to SES, forward-to admin@orya.pt re-enters SES (loop) and is dropped. To deliver into Google Workspace inbox without changing MX, need Gmail API/IMAP injection (credentials required).
