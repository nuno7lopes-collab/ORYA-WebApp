...
async function _GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    console.error("GET /api/org/[orgId]/servicos/[id]/packages error:", err);
...
async function _POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const paidWriteGate = evaluatePaidWriteGate({
      ...
      amountCents: Math.round(priceCentsRaw),
    });
    if (!paidWriteGate.ok) {
      return respondError(
        ctx,
    ...
    console.error("POST /api/org/[orgId]/servicos/[id]/packages error:", err);
