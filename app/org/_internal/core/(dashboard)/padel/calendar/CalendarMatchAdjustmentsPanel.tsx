...
type CalendarCourtItem = { id: number; name: string };
...
export function CalendarMatchAdjustmentsPanel(props: {
  ...
  form: { start: string; end: string; courtId: string };
  onFormChange: (
    patch: Partial<{ start: string; end: string; courtId: string }>,
  ) => void;
  onSave: () => void;
  onCancel: () => void;
  ...
  const sortedMatches = [...props.matches].sort((a, b) => {
    const aStart =
      toDate(a.plannedStartAt ?? a.startTime)?.getTime() ??
      Number.MAX_SAFE_INTEGER;
    const bStart =
      toDate(b.plannedStartAt ?? b.startTime)?.getTime() ??
      Number.MAX_SAFE_INTEGER;
  ...
  const bulkDisabled =
    !props.eventId ||
    props.saving ||
    props.selectedMatchIds.length === 0 ||
    !Number.isFinite(bulkTargetId) ||
    bulkTargetId <= 0;
  return (
        ...
        <>
          <div className="space-y-2 rounded-xl border border-white/12 bg-black/20 p-3">
            ...
            <select
              ...
              onChange={(event) =>
                props.onFormChange({ courtId: event.target.value })
              }
              disabled={disabled}
          ...
          <div className="space-y-2 rounded-xl border border-white/12 bg-black/20 p-3">
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <span className="rounded-full border border-white/20 bg-white/5 px-2 py-1 text-[11px] text-white/70">
                {props.selectedMatchIds.length} selecionado
                {props.selectedMatchIds.length === 1 ? "" : "s"}
              </span>
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    <p className="mt-1 text-white/70">
                      {start
                        ? props.formatZoned(start, props.timezone)
                        : "Sem início"}
                      {end
                        ? ` -> ${props.formatZoned(end, props.timezone)}`
                        : ""}
